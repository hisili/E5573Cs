...
extern struct AX88772B_BULKIN_SIZE_STRU AX88772B_BULKIN_SIZE[8];
extern struct CICADA_FAMILY_HWINIT_STRU CICADA_FAMILY_HWINIT[40];
extern struct CICADA_V2_HWINIT_STRU CICADA_V2_HWINIT[9];
extern struct CICADA_V2_ASIX_HWINIT_STRU CICADA_V2_ASIX_HWINIT[];
extern struct AGERE_FAMILY_HWINIT_STRU AGERE_FAMILY_HWINIT[];
...
axusbnet_probe(struct usb_interface* udev, const struct usb_device_id* prod)
{
    ...
    printk(KERN_INFO"axusbnet_probe Enter\r\n");
    ...
    if (netif_msg_probe(dev))
        devinfo(dev, "register '%s' at usb-%s-%s, %s, %pM",
                ...
                dev->driver_info->description,
                net->dev_addr);
    usb_lan_get_mac(net->dev_addr);	
    usb_set_mac_addr(net);
