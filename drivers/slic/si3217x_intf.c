...
**
** $Id: si3217x_intf.c 3468 2012-05-02 19:07:47Z cdp $
**
** SI3217X_Intf.c
				...
				if (pProslic[k]->deviceId->chipRev == SI3217X_REVB )
				{
					...
					else if(Si3217x_General_Configuration.bomOpt == BO_DCDC_BUCK_BOOST)
					{
						patch = &(SI3217X_PATCH_B_BKBT);
			             ...
			             ||(Si3217x_General_Configuration.bomOpt == BO_DCDC_P_BUCK_BOOST_12V)
					     ||(Si3217x_General_Configuration.bomOpt == BO_DCDC_P_BUCK_BOOST_12V_HV)
					     ||(Si3217x_General_Configuration.bomOpt == BO_DCDC_PMOS_BUCK_BOOST))
					{
    ...
    for (k=0;k<size;k++){
        if ((pProslic[k]->channelEnable)&&(pProslic[k]->channelType == PROSLIC)){
           ...
           pProslic[k]->WriteRAMX(pProslic[k]->pProHWX, pProslic[k]->channel,DCDC_ANA_GAIN,Si3217x_General_Configuration.dcdc_ana_gain);
            ...
            pProslic[k]->WriteRAMX(pProslic[k]->pProHWX,pProslic[k]->channel,ILOOPLPF,0x4EDDB9L); /* 20pps pulse dialing enhancement */
            pProslic[k]->WriteRAMX(pProslic[k]->pProHWX,pProslic[k]->channel,ILONGLPF,0x806D6L); /* 20pps pulse dialing enhancement */
            pProslic[k]->WriteRAMX(pProslic[k]->pProHWX,pProslic[k]->channel,VDIFFLPF,0x10038DL); /* 20pps pulse dialing enhancement */
            pProslic[k]->WriteRAMX(pProslic[k]->pProHWX,pProslic[k]->channel,BAT_HYST,0x83120L); /* reduced to 0.6v */
            pProslic[k]->WriteRAMX(pProslic[k]->pProHWX,pProslic[k]->channel,LKG_STBY_OFFSET,0x4000000L); /* Increase OHT standby leakage */
			/*
			** Overwrite PSR Setting for PSR_DCDC_OVTHRESH_NEW based on BOM option -
			** this will allow for a single patch for all BOM options
			*/
			if(Si3217x_General_Configuration.bomOpt == BO_DCDC_BUCK_BOOST)
			{
				pProslic[k]->WriteRAMX(pProslic[k]->pProHWX,pProslic[k]->channel,PSR_DCDC_OVTHRESH_NEW,0x400000L);
			}
			else if(Si3217x_General_Configuration.bomOpt == BO_DCDC_P_BUCK_BOOST_5V)
			{ /* Obsolete */
				pProslic[k]->WriteRAMX(pProslic[k]->pProHWX,pProslic[k]->channel,PSR_DCDC_OVTHRESH_NEW,0x300000L);
			}
			else if((Si3217x_General_Configuration.bomOpt == BO_DCDC_P_BUCK_BOOST_12V)||(Si3217x_General_Configuration.bomOpt == BO_DCDC_P_BUCK_BOOST_12V_HV))
			{ /* Obsolete */
				pProslic[k]->WriteRAMX(pProslic[k]->pProHWX,pProslic[k]->channel,PSR_DCDC_OVTHRESH_NEW,0x700000L);
			}
			else if(Si3217x_General_Configuration.bomOpt == BO_DCDC_PMOS_BUCK_BOOST)
			{
			...
			{
				pProslic[k]->WriteRAMX(pProslic[k]->pProHWX,pProslic[k]->channel,PSR_DCDC_OVTHRESH_NEW,0xB00000L);
			}
...
int Si3217x_ToneGenSetup (proslicChanType *pProslic, int preset){
    ...
    if(GEN_BUSY_TONE == preset)
    {
        WriteRAM(pProHW, pProslic->channel, OSC1FREQ, slic_parm[g_Country_Type].busy_tone.frequency1);
        WriteRAM(pProHW,pProslic->channel,OSC1AMP,slic_parm[g_Country_Type].busy_tone.amplitude1);
        WriteRAM(pProHW, pProslic->channel, OSC2FREQ, slic_parm[g_Country_Type].busy_tone.frequency2);
        WriteRAM(pProHW,pProslic->channel,OSC2AMP,slic_parm[g_Country_Type].busy_tone.amplitude2);
        WriteReg(pProHW, pProslic->channel, O1TAHI, ((slic_parm[g_Country_Type].busy_tone.burst >> 8) & 0x0FF));       
        WriteReg(pProHW, pProslic->channel, O1TALO, (slic_parm[g_Country_Type].busy_tone.burst & 0x0FF));
        WriteReg(pProHW, pProslic->channel, O1TIHI, ((slic_parm[g_Country_Type].busy_tone.pause >> 8) & 0x0FF));
        WriteReg(pProHW, pProslic->channel, O1TILO, (slic_parm[g_Country_Type].busy_tone.pause & 0x0FF));
    }
    else if(GEN_DIAL_TONE == preset)
    {
        WriteRAM(pProHW, pProslic->channel, OSC1FREQ, slic_parm[g_Country_Type].dial_tone.frequency1);
        WriteRAM(pProHW,pProslic->channel,OSC1AMP,slic_parm[g_Country_Type].dial_tone.amplitude1);
        WriteRAM(pProHW, pProslic->channel, OSC2FREQ, slic_parm[g_Country_Type].dial_tone.frequency2);
        WriteRAM(pProHW,pProslic->channel,OSC2AMP,slic_parm[g_Country_Type].dial_tone.amplitude2);
        WriteReg(pProHW, pProslic->channel, O1TAHI, ((slic_parm[g_Country_Type].dial_tone.burst >> 8) & 0x0FF));       
        WriteReg(pProHW, pProslic->channel, O1TALO, (slic_parm[g_Country_Type].dial_tone.burst & 0x0FF));
        WriteReg(pProHW, pProslic->channel, O1TIHI, ((slic_parm[g_Country_Type].dial_tone.pause >> 8) & 0x0FF));
        WriteReg(pProHW, pProslic->channel, O1TILO, (slic_parm[g_Country_Type].dial_tone.pause & 0x0FF)); 
    }
    else
    {
