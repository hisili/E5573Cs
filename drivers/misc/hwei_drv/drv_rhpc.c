/*
 * ============================== DRV_RHPC.C ===================================
 * Remote Host Procedure Call Intefaces of ASIC Platform Driver.
 * This is a proxy layer for all device driver who has the job that need call
 * modem to perform.
 *
...
void mmi_write_nv(int mmi_result)
{
	...
	memset(aucFacInfo, 0x00, AT_FACTORY_INFO_LEN);
...
/* 成功 写入"ST P" */
	if(1 == mmi_result)
	{
		memcpy(&aucFacInfo[AT_MMI_TEST_FLAG_OFFSET], AT_MMI_TEST_FLAG_SUCC, AT_MMI_TEST_FLAG_LEN);
	}
