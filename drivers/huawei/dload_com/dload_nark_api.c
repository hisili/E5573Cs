...
#if (FEATURE_ON == MBB_DLOAD_SERVER_SIGN)
...
/*******************************************************************
  ...
  函数返回值: DLOAD_OK    : 成功
              DLOAD_ERR : 失败
********************************************************************/
STATIC int32 set_dlpart_flag(uint8 * data_buffer, uint32 data_len)
{
#if (FEATURE_ON == MBB_DLOAD_DOUBLE_PART)
    int32 ret = DLOAD_ERR;
    recovery_type_flag re_flag = {0};

    if((NULL == data_buffer) || (data_len < sizeof(recovery_type_flag)))
    {
        DLOAD_PRINT(KERN_ERR, "input para error, len = 0x%x.", data_len);
        return DLOAD_ERR;
    }

    /*判断格式写入flash*/
    re_flag = *((recovery_type_flag *)data_buffer);
    if(DLOAD_RECOVERY_FLAG_MAGIC_NUM == re_flag.magic_number)
    {
        /*魔鬼数字正确,写入标志*/
        ret = flash_set_recovery_flag(re_flag.new_recovery_flag);
        if(DLOAD_OK != ret)
        {
            DLOAD_PRINT(KERN_ERR, "set flag error, flag = 0x%x.", re_flag.new_recovery_flag);
            return DLOAD_ERR;
        }
        DLOAD_PRINT(KERN_ERR, "write flag 0x%x success.", re_flag.new_recovery_flag);
        return DLOAD_OK;
    }
    else
    {
        DLOAD_PRINT(KERN_ERR, "magic num error, num = 0x%x.", re_flag.magic_number);
        return DLOAD_ERR;
    }
#else
    /*未打开双备份机制直接返回OK*/
...
STATIC ioctl_map_t nark_msg_map[] = 
{
    ...
    {
        /* 设置双备份分区标志*/
        MAP_INDEX(DLOAD_MAIN_MODULE, SET_DOUBLE_PART_FLAG), 
        set_dlpart_flag, 
    },
};
