...
#if (FEATURE_ON == MBB_FEATURE_VSIM_HUK)
#define VSIM_HUK_SECUR_MAX_LEN          (128)   /* 支持输入加密解密数据的最大长度*/
#define VSIM_HUK_CRYPT_MAX_LEN          (144)   /* 加密后数据的最大长度*/
#define VSIM_HUK_ENCODE_MAX_LEN         (512)   /*传输数据最大长度*/
#define VSIM_HUK_MAX_LEN                (16)    /*HUK为128Bits的码流, 长度为16Bytes */
#define VSIM_HUK_256_MAX_LEN            (32)    /*HUK为128Bits的码流, 长度为16Bytes */
#define VSIM_HUK_TO_HASH_MAX_LEN        (32)    /*运算HASH时最大长度*/
#define VSIM_HUK_CBC_IV_MAX_LEN         (16)    /*AES CBC 加密时 向量的最大长度*/
#define VSIM_HUK_AES_128_LEN_BIT        (128)   /*HUK为256Bits的码流, 长度为32Bytes */
#define VSIM_HUK_AES_256_LEN_BIT        (256)   /*HUK为256Bits的码流, 长度为32Bytes */
#define CODEING_ERROR                   (1)     /*编码错误后标志位，用于最终返回ERROR*/
#define CODEING_LEN_NULL                (0)     /*编解码错误后长度返回为0*/

#define VSIM_VIRT_PSIM                  (11)    /*主卡标示*/
#define VSIM_VIRT_SSIM                  (12)    /*子卡标示*/
#define VSIM_STATE_DISABLE              (0)     /*当前卡状态为去激活*/
#define VSIM_KEY_VIRT_PSIM               (11)    /*主卡标示*/
#define VSIM_KEY_VIRT_SSIM               (12)    /*子卡标示*/
#define VSIM_KEY_DEVICE_KEY              (21)    /*Device key标示*/
#define VSIM_KEY_CONTENT_KEY             (22)    /*Content key标示*/

#define NV_VSIM_DIEID_MAXLEN             (64)

#define VSIM_HVSDH_PSIM_NV_ID           (50547) /*主卡参数对应NV ID*/
#define VSIM_HVSDH_SSIM_NV_ID           (50548) /*子卡卡参数对应NV ID*/
#define VSIM_DEVICE_KEY_NV_ID           (50565) /*Device key参数对应NV ID*/
#define VSIM_CONTENT_KEY_NV_ID          (50566) /*Content key参数对应NV ID*/

#define VSIM_HUK_PSIM_LOC               (0)       /*主卡密钥操作bit位*/
#define VSIM_HUK_SSIM_LOC               (4)       /*子卡密钥操作bit位*/
#define VSIM_HUK_APPKEY_LOC             (8)       /*APP密钥操作bit位*/
#define VSIM_HUK_MODEMKEY_LOC           (12)      /*modem密钥操作bit位*/
#endif/*end (FEATURE_ON == MBB_FEATURE_VSIM_HUK)*/
...
{
    SI_PIH_HVADH_ENCRYPT    = 1,        /*加密*/
    SI_PIH_HVADH_DENCRYPT,              /*解密*/
...
{
    SI_PIH_KEY_SERVER_PUBLIC_KEY         = 0x00,             /* 服务器公钥 */
    SI_PIH_KEY_MODEM_PUBLIC_KEY          = 0x01,             /* MODEM侧公钥 */
    SI_PIH_KEY_MODEM_PRIVATE_KEY         = 0x02,             /* MODEM侧私钥 */
...
{
    SI_PIH_AES_TYPE_ECB_128    = 1,      /* ECB 128 算法*/
    SI_PIH_AES_TYPE_ECB_256,             /* ECB 256 算法*/
    SI_PIH_AES_TYPE_CBC_128,             /* CBC 128 算法*/
    SI_PIH_AES_TYPE_CBC_256,             /* CBC 256 算法*/
...
typedef struct
{
    ...
    VOS_UINT8                      aucVal[VSIM_HUK_ENCODE_MAX_LEN+1];/*数据最大为512*/
}SI_PIH_VSIM_HUK_VAL_STRU;


typedef struct
{
    SI_PIH_HVADH_OBJ_ENUM_UINT32            ulobj;             /*加密对象*/
    SI_PIH_HVADH_MODE_ENUM_UINT32           ulmode;            /*加密或者解密*/
    SI_PIH_VSIM_HUK_VAL_STRU                stciperval;        /*源数据*/
} SI_PIH_HVADH_SET_STRU;


enum SI_PIH_REDH_IND_TYPE_ENUM
{
    SI_PIH_REDH_IND_TYPE_HASH_FAIL         = 1,      /* Hash 校验失败*/
    SI_PIH_REDH_IND_TYPE_CARD_KEY_FAIL     = 2,      /* 卡密钥校验失败*/
    SI_PIH_REDH_IND_TYPE_DATA_KEY_FAIL     = 3,      /* 内容密钥校验失败*/
...
typedef struct
{
    VOS_MSG_HEADER
    VOS_UINT32                          ulMsgName;      /* 消息名 */
    SI_PIH_REDH_IND_TYPE_ENUM_UINT32    ulIndType;      /*失败类型*/
