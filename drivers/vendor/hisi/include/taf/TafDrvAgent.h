...
#define AT_TAF_HWLOCKPARALEN    9
#define AT_TAF_HWLOCKOEMTYPE    1
#define AT_TAF_HWLOCKSIMLOCKTYPE 2
#define AT_DRV_AGENT_OEM_SIMLOCK_ENABLE 1
#define AT_DRV_AGENT_HASH_LEN     32
...
1. 请求消息ID按偶数位往上递增。
2. 返回消息ID按奇数位往上递增。
3. 相同消息的请求和返回ID相邻，例如: 请求消息ID为0x010E，原则要求返回消息ID为0x010D
4. 迁移基线过程中，请迁移基线人员检查海思新添加消息ID是否与我们新添加ID重复定义，如有请MBB定义消息ID整体上移。
*********************************************************************************************************/
enum DRV_AGENT_MSG_TYPE_ENUM
{
    ...
    /***********************************************MBB自定义请求消息ID区******************************************************/
    DRV_AGENT_QRY_SIM_INFO_REQ           = 0x0180,
...
/*begin add by wanghaijie for simlock 3.0*/
#if (FEATURE_ON == MBB_SIMLOCK_THREE)
    DRV_AGENT_HWLOCK_SET_CNF             = 0x0185,                         /*C核回复A核设置HWLOCK的消息ID*/
    DRV_AGENT_HWLOCK_QURY_CNF            = 0x0187,                        /*C核回复A核测试HWLOCK的消息ID*/
#endif
/*end   add by wanghaijie for simlock 3.0*/
#endif
...
}DRV_AGENT_HWLOCK_SET_QRY_CNF_STRU;
...
}DRV_AGENT_HWLOCK_TEST_QRY_CNF_STRU;
