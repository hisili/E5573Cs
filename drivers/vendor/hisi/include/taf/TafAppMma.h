...
#if( FEATURE_ON == MBB_WPG_COMMON )
#define TAF_PLMN_RESELETION_MANUAL_AUTO      4     /*PLMN重选：手工-自动*/
#endif
...
/*****************************************************************************
结构名    : MN_MMA_QOSIG_PARA_STRU
结构说明  : AT*QOSIG  查询操作的结果上报结构
*******************************************************************************/
#if(FEATURE_ON == MBB_FEATURE_BOX_FTEN)
typedef struct
{
...
#if (FEATURE_ON == MBB_FEATURE_VSIM_HUK)
#define RSFW_CPRIVATE_KEY_LEN      (256)  /*客户端私钥的长度*/
#define RSFW_CPUBLIC_KEY_LEN       (256)  /*客户端公钥的长度*/
#define RSFW_SPUBLIC_KEY_LEN       (256)  /*服务器端公钥的长度*/

#define HVADH_APP_KEY              (3)    /*APP侧使用的KEY,用于在C核进行加密操作*/
#define HVADH_MODEM_KEY            (4)    /*MODEM侧使用的KEY，用于在C核进行加密操作*/

#define DEVICE_KEY_TYPE            (0)    /*定义DEVICEKEY的类型*/
#define MASTER_KEY_TYPE            (1)    /*定义MASTERKEY的类型*/
#define CONTENT_KEY_TYPE           (2)    /*定义CONTENTKEY的类型*/

#define HUK_KEY_LEN                (1024) /*HUK加密临时变量长度*/
...
typedef struct
{
    VOS_UINT32                    magic_start;                 /* 分区起始Magic Code 0x5B5B5B5B*/
    VSIM_DATA_EVENT               EventType;                   /* 事件类型 */
    union
    {
    ...
    VOS_UINT32                    magic_end;                   /* 分区结束Magic Code 0x5B5B5B5B*/
}TAF_VSIM_DATA_AREA;
