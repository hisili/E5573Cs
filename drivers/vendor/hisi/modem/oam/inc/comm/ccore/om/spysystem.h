...
/*定义了各个监控点的投票值*/
#if (FEATURE_ON == MBB_THERMAL_PROTECT)
#define SPY_VOTE_QOSDOWN_2                  (-7)    /*当前需要二级限速*/
#endif
#if (FEATURE_ON == MBB_THERMAL_PROTECT_SAR)
#define SPY_VOTE_REDUCE_SAR                 (-6)    /*当前需要将SAR*/
#endif
...
#if (FEATURE_ON == MBB_THERMAL_PROTECT)
    SPY_VOTERESULT_DOWNQOS_2,            /*当前需要进行二级限速流控下调*/
#endif   
    SPY_VOTERESULT_PSOFF,                /*当前需要软关机*/
    SPY_VOTERESULT_POWEROFF,             /*当前需要下电*/
#if (FEATURE_ON == MBB_THERMAL_PROTECT_SAR)    
    SPY_VOTERESULT_REDUCE_SAR,           /*当前需要降SAR*/
...
#if (FEATURE_ON == MBB_THERMAL_PROTECT)
    SPY_STATE_POWEROFF          = 3,/*当前处于硬关机状态*/
#endif
#if (FEATURE_ON == MBB_THERMAL_PROTECT_SAR)    
    SPY_STATE_REDUCE_SAR        = 4,/*当前处于降SAR状态*/
#endif
#if (FEATURE_ON == MBB_THERMAL_PROTECT)
    SPY_STATE_QOS_2             = 5,/*当前处于硬关机状态*/
#endif
    SPY_STATE_BUTT
};
...
#if (FEATURE_ON == MBB_THERMAL_PROTECT)
    VOS_UINT32                  ulTempAlarmCount_2;    /*二级限速，告警温度计数器*/
    VOS_UINT32                  ulTempSoftoffResumeCount;  /*软关机恢复温度计数器*/  
#endif
...
typedef struct
{
    TEMP_ENABLE_ENUM_UINT32     enTempEnable;      /*当前系统温度监控降SAR始能开关*/  
    enum SPY_STATE_ENUM ulTempSARState; /*将SAR系统状态:将SAR态和正常态*/
