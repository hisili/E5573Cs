...
#if (FEATURE_ON == MBB_WPG_COMMON)
#if defined (__KERNEL__)
extern struct wake_lock icc_wake_lock;
int g_icc_waketime = 2000;  /*default to wake 2000ms*/
#endif
#endif
...
#define CIEV_SERVICE_FLAG                       (3)       /*+CIEV服务状态上报标示*/
#define AT_CIND_RPT_SWITCH_OFF                  (0)       /*+CIND上报开关关闭*/
#define AT_CIND_RPT_SWITCH_ON                   (1)       /*+CIND上报开关打开*/
#define AT_CMER_RPT_MODE_SWITCH_OFF             (0)       /*+CMER上报MODE开关关闭*/
#define AT_CMER_RPT_MODE_SWITCH_ON              (3)       /*+CMER上报MODE开关打开*/
#define AT_CMER_RPT_SWITCH_OFF                  (0)       /*+CMER上报开关关闭*/
#define AT_CMER_RPT_SWITCH_ON                   (1)       /*+CMER上报开关打开*/
#define TAF_CIND_SRVSTA_NORMAL_SERVICE          (1)       /*+CIND命令返回值表示正常服务 */
#define TAF_CIND_SRVSTA_NO_SERVICE              (0)       /*+CIND命令返回值表示无服务*/
static TAF_PHONE_CHANGED_SERVICE_STATUS   g_oldservicestatus = 0;
...
/*添加AT*QOSIG */
#if(FEATURE_ON == MBB_FEATURE_BOX_FTEN)
    {TAF_PH_QOSIG_VALUE_PARA,           AT_QryParaQosigProc},
#endif
...
#else
                *pcWrite++ = ucLow + 0x37;
#endif/*MBB_FEATURE_SBM_CUSTOMIZE*/
            }
            else
            {
...
/* Added by f62575 for AT Project, 2011-10-04,  End */
    ...
    case MN_CALL_EVT_ALL_RELEASED:
       ...
       /*在当前所有链路的CALL都释放后，需要去使能端口*/
        BSP_USB_SetSwitchPcVoice(VOS_FALSE);
...
)
{
    ...
    *pucAcqOrder = '\0';
    ...
    if ((0 == VOS_StrCmp((VOS_CHAR *)pucAcqOrderBegin, "0302"))
     && (TAF_PH_MAX_LU_RAT_NUM == pstRatOrder->ucRatOrderNum))
    {
        /*接入技术的个数为2且接入优先顺序为L->W,acqorder上报00*/
...
#endif /*FEATURE_ON == MBB_TELSTRA_CUSTOMSIZE*/
#else
    ...
    if (VOS_TRUE == pEvent->OP_MmInfo)
    {
         ...
         || (NAS_MM_INFO_IE_LTZ == (pEvent->stMmInfo.ucIeFlg & NAS_MM_INFO_IE_LTZ)))
        {
            ...
            usLength += (TAF_UINT16)At_PrintMmTimeInfo(ucIndex,
                                                       ...
                                                       (pgucAtSndCodeAddr + usLength));
#if(FEATURE_ON == MBB_WPG_COMMON)
            AT_UpdateCclkInfo(&pEvent->stMmInfo);
#endif
        }
...
)
{
    ...
    g_stAtSysMode.ucRatType    = pstEvent->RatType;
    g_stAtSysMode.ucSysSubMode = pstEvent->ucSysSubMode;
#endif
    return;
}
...
)
{
    ...
    VOS_UINT32                          ulRslt;
#if(FEATURE_ON == MBB_FEATURE_BOX_FTEN)
    TAF_PHONE_CHANGED_SERVICE_STATUS    ciev_service;
#endif
    usLength  = 0;
    enModemId = MODEM_ID_0;
    ...
    if(((AT_CMER_RPT_MODE_SWITCH_ON == g_ucCmerpara.mode) 
        ...
        && (AT_CIND_RPT_SWITCH_ON == g_ucCindpara.service))
    {
        ciev_service = pstEvent->ServiceStatus;
        if (TAF_REPORT_SRVSTA_NORMAL_SERVICE == ciev_service)
        {
            ciev_service = TAF_CIND_SRVSTA_NORMAL_SERVICE;/*1表示 正常服务0为无服务*/
        }
        else
        {
            ciev_service = TAF_CIND_SRVSTA_NO_SERVICE;
        }
        if(g_oldservicestatus != ciev_service) 
        {
            usLength += (TAF_UINT16)At_sprintf(AT_CMD_MAX_LEN,
                                           (TAF_CHAR *)pgucAtSndCodeAddr,
                                           (TAF_CHAR *)pgucAtSndCodeAddr + usLength,
                                           ...
                                           ciev_service,
                                           gaucAtCrLf);
            g_oldservicestatus = ciev_service;
            At_SendResultData(ucIndex,pgucAtSndCodeAddr,usLength);
            usLength = 0;
        }
    }
...
)
{
    ...
    {
#if ( FEATURE_ON == MBB_WPG_COMMON )
        At_FormatSyscfgMbb(pstNetCtx, pEvent, ucIndex);
...
TAF_VOID At_PhRspProc(TAF_UINT8 ucIndex,TAF_PHONE_EVENT_INFO_STRU  *pEvent)
{
...
#endif
#if (FEATURE_ON == MBB_FEATURE_VSIM_HUK)
    case TAF_MMA_EVT_MASTER_KEY_HUK_INFO_IND:
        usLength = AT_PhMasterKeyHukInfoProc((TAF_UINT8*)pEvent, &ulResult);

        /*重置当前AT命令状态，将异步状态至回，并停止定时器*/
        AT_STOP_TIMER_CMD_READY(ucIndex);
        break;
    case TAF_MMA_EVT_DIE_ID_HUK_INFO_IND:
        usLength = AT_PhDieIdHukInfoProc((TAF_UINT8*)pEvent, &ulResult);

        /*重置当前AT命令状态，将异步状态至回，并停止定时器*/
        AT_STOP_TIMER_CMD_READY(ucIndex);
        break;
    case TAF_MMA_EVT_GET_DIE_ID_INFO_IND:
        usLength = AT_PhGetDieIdInfoProc((TAF_UINT8*)pEvent, &ulResult);
    ...
    At_FormatResultData(ucIndex,ulResult);
/*和上层确认607HW需要上报状态给应用*/
#if (FEATURE_OFF == MBB_FEATURE_SBM_CUSTOMIZE) || defined(BSP_CONFIG_BOARD_607HW)
...
TAF_VOID At_PhMsgProc(TAF_UINT8* pData,TAF_UINT16 usLen)
{
    ...
    AT_USB_DEBUG_PRINT_1(ucPhoneEvent);
...
)
{
    ...
    #if defined (__KERNEL__)
    /*收到消息，延迟睡眠*/
    wake_lock_timeout(&icc_wake_lock, (long)msecs_to_jiffies(g_icc_waketime));/*lint !e526 !e628 !e516*/
    #endif
    #endif
...
)
{
    ...
    int                                 i;
    TAF_UINT16                          CharLength;
    TAF_UINT16                          IccIdNumlen; /*MS2172s-818 查询ICCID返回字符串长度*/
#endif
    ...
    usLength += (VOS_UINT16)AT_Hex2AsciiStrLowHalfFirst(AT_CMD_MAX_LEN,
                                                        ...
                                                        stIccId.ucLen);
#if(FEATURE_ON == MBB_FEATURE_ESIM_SWITCH)
    IccIdNumlen = usLength;
    for(i = usLength - 1; i > CharLength; i--)
    {
        if(('F' == *(pgucAtSndCodeAddr + i)) || ('f' == *(pgucAtSndCodeAddr + i))) /*从尾向头检查为"F"或“f”则置为0，发送数据长度减一*/
        {
            * (pgucAtSndCodeAddr + i) = 0;
            IccIdNumlen--;
        }
    else
        {
...
TAF_UINT8 At_IsOplRecPrintable(TAF_PH_USIM_OPL_RECORD * pstOplRec)
{
    ...
    {
        if (( 0xA <= pstOplRec->PLMN[i] ) && ( 0xD != pstOplRec->PLMN[i]))    /*'D'为通配符*/
        {
            return VOS_FALSE;
        }
    }
...
TAF_VOID At_PIHRspProc(TAF_UINT8 ucIndex, SI_PIH_EVENT_INFO_STRU *pEvent)
{
    ...
    switch(pEvent->EventType)
    {
...
#if (FEATURE_ON == MBB_FEATURE_VSIM_HUK)
        case SI_PIH_EVENT_HVADH_SET_CNF:
            usLength += At_SetHvADHCnf(pEvent);
            break;
        ...
        case SI_PIH_EVENT_HVSCONT_SET_CNF:
            usLength += At_SetHvsContCnf(pEvent);
            break;
        case SI_PIH_EVENT_HVSHDH_SET_CNF:
            usLength += AT_HvshDHSetCnf(pEvent);
            break;

        case SI_PIH_EVENT_HVDIEID_QRY_CNF:
            usLength += AT_HvdieidQueryCnf(pEvent);
            break;

        case SI_PIH_EVENT_HVDDH_SET_CNF:
            usLength += AT_HvdDHSetCnf(pEvent);
            break;
#else
...
TAF_VOID At_STKMsgProc(MsgBlock* pMsg)
{
    ...
    {
        AT_BipTestResultReport(pstSTKMsg->Data, (BSP_U32)pstSTKMsg->Datalen);
    }
#endif
    else
    {
...
#if (FEATURE_ON == MBB_WPG_COMMON)
        pstNetCtx->stTimeInfo.ulTimeSeconds = pEvent->stMmInfo.ulTimeSeconds;
#endif
    }
...
)
{
    ...
    usLength += (TAF_UINT16)At_sprintf(AT_CMD_MAX_LEN,(TAF_CHAR *)pgucAtSndCodeAddr,(TAF_CHAR *)pgucAtSndCodeAddr + usLength,"%s:%d",g_stParseContext[ucIndex].pstCmdElement->pszCmdName,stSysInfo.ucSrvStatus);
#if (FEATURE_ON == MBB_WPG_COMMON)
    At_FormatSysinfoExMbb(&usLength, &stSysInfo);
#else
    usLength += (TAF_UINT16)At_sprintf(AT_CMD_MAX_LEN,(TAF_CHAR *)pgucAtSndCodeAddr,(TAF_CHAR *)pgucAtSndCodeAddr + usLength,",%d",stSysInfo.ucSrvDomain);
#endif    
    usLength += (TAF_UINT16)At_sprintf(AT_CMD_MAX_LEN,(TAF_CHAR *)pgucAtSndCodeAddr,(TAF_CHAR *)pgucAtSndCodeAddr + usLength,",%d",stSysInfo.ucRoamStatus);
...
/*****************************************************************************
 函 数 名  : AT_QryParaQosigProc
 功能描述  : 生成AT*QOSIG    命令的返回结果
 输入参数  :
     VOS_UINT8                           ucIndex,
     VOS_UINT8                           OpId,
     VOS_VOID                           *pPara
...
{
    VOS_UINT32                          ulResult;
    MN_MMA_QOSIG_PARA_STRU               stQosigPara;

    /* 初始化 */
    ulResult = AT_OK;
    PS_MEM_CPY(&stQosigPara, pPara, sizeof(MN_MMA_QOSIG_PARA_STRU));
    gstAtSendData.usBufLen = (TAF_UINT16)At_sprintf(AT_CMD_MAX_LEN,
                                                    (TAF_CHAR *)pgucAtSndCodeAddr,
                                                    (TAF_CHAR*)pgucAtSndCodeAddr,
                                                    "%s: %d,%d",
                                                    g_stParseContext[ucIndex].pstCmdElement->pszCmdName,
                                                    (TAF_INT32)stQosigPara.sCpichRscp,
                                                    (TAF_INT32)stQosigPara.sCpichEcNo);
...
#if (FEATURE_ON == MBB_WPG_COMMON )/*修改上层为MP时^spn第一个参数返回b2*/
        usLength += (VOS_UINT16)At_sprintf(AT_CMD_MAX_LEN,(TAF_CHAR *)pgucAtSndCodeAddr,
                                               (VOS_CHAR *)pgucAtSndCodeAddr + usLength,
                                               "%d,%d,",
                                               stAtSPNRslt.DispRplmnMode & 0x03,/*取bI b2*/
...
{
    TAF_PS_CALL_END_CNF_STRU           *pstCallEndCnf;
#if(FEATURE_OFF == MBB_FEATURE_MPDP)
    VOS_UINT32                          ulResult;
...
)
{
    ...
    for(ucTmp = 0; ucTmp < pstGetPrimPdpCtxInfoCnf->ulCidNum; ucTmp++)
    {
        ...
        if(0 == strlen(aucStr))
        {
            PS_MEM_CPY(aucStr,"0.0.0.0",strlen("0.0.0.0"));
        }
#endif
        usLength += (VOS_UINT16)At_sprintf(AT_CMD_MAX_LEN,(VOS_CHAR*)pgucAtSndCodeAddr,(VOS_CHAR*)pgucAtSndCodeAddr + usLength,",\"%s\"", aucStr);
        /* <d_comp> */
...
)
{
    ...
    for(ucTmp = 0; ucTmp < pstUmtsQosInfo->ulCidNum; ucTmp++)
    {
        ...
        usLength += (VOS_UINT16)At_sprintf(AT_CMD_MAX_LEN,(VOS_CHAR*)pgucAtSndCodeAddr,
                                          (VOS_CHAR*)pgucAtSndCodeAddr + usLength,
                                          ",%d",stCgeq.stQosInfo.ucSrcStatisticsDescriptor);
        /* <Signalling Indication> */
        usLength += (VOS_UINT16)At_sprintf(AT_CMD_MAX_LEN,(VOS_CHAR*)pgucAtSndCodeAddr,
                                          (VOS_CHAR*)pgucAtSndCodeAddr + usLength,
                                          ",%d",stCgeq.stQosInfo.ucSignallingIndication);
...
#if (FEATURE_ON == MBB_WPG_COMMON)
    At_FormatRssiInfo(ucIndex, pEvent);
#endif
...
VOS_VOID AT_PhRegRejInfoProc(TAF_UINT8* pData,TAF_UINT16 usLen)
{
    ...
    usLength += (TAF_UINT16)At_sprintf(AT_CMD_MAX_LEN,
                                       ...
                                       gaucAtCrLf);
#if (FEATURE_ON == MBB_MLOG)
    mlog_print("at", mlog_lv_info, "REJINFO plmn: %d%d\n",pstRejInfo->stPlmnId.Mcc,
                              pstRejInfo->stPlmnId.Mcc);
    mlog_print("at", mlog_lv_info, "REJINFO: %d, %d,%d,%d\n",pstRejInfo->ucSrvDomain,
                              pstRejInfo->ulRejCause,pstRejInfo->enRat,pstRejInfo->ucRejType);
#endif
...
VOS_VOID AT_PhMasterKeyHukInfoParaProc(VOS_VOID)
{
    VOS_MemSet(g_cDeviceKey,   0, AT_DEVICE_KEY_MAX_LEN);
    VOS_MemSet(g_cMasterKey,   0, AT_MASTER_KEY_MAX_LEN);
    VOS_MemSet(g_cContentKey,  0, AT_CONTENT_KEY_MAX_LEN);
    VOS_MemSet(g_cMasterData1, 0, AT_MASTER_DATA_MAX_LEN);
    VOS_MemSet(g_cMasterData2, 0, AT_MASTER_DATA_MAX_LEN);
    VOS_MemSet(g_cHash,        0, AT_HASH_MAX_LEN);
    return;
}
...
TAF_UINT16 AT_PhMasterKeyHukInfoProc(TAF_UINT8 *pData, TAF_UINT8 *pResult)
{
    VOS_UINT8                           ucIndex  = 0;
    VOS_UINT32                          ulRslt   = 0;
    TAF_PH_MASTER_KEY_HUK_MSG_INFO_STRU       *pstModemKeyInfo = VOS_NULL_PTR;
    VOS_UINT8  tempKeyWriteToFile[HUK_KEY_LEN];
    VOS_UINT8  tempCPublicHukKey[HUK_KEY_LEN];
    VOS_UINT8  tempCPrivateHukKey[HUK_KEY_LEN];
    VOS_UINT8  tempSPublicHukKey[HUK_KEY_LEN];

    VOS_MemSet(tempKeyWriteToFile, 0,sizeof(tempKeyWriteToFile));
    VOS_MemSet(tempCPublicHukKey,  0,sizeof(tempCPublicHukKey));
    VOS_MemSet(tempCPrivateHukKey, 0,sizeof(tempCPrivateHukKey));
    VOS_MemSet(tempSPublicHukKey,  0,sizeof(tempSPublicHukKey));

    if(VOS_NULL_PTR == pData)
    {
        AT_WARN_LOG("AT_PhMasterKeyHukInfoProc: pData is NULL!");
        goto error_handle;
    }
    pstModemKeyInfo = (TAF_PH_MASTER_KEY_HUK_MSG_INFO_STRU*)pData;
    ulRslt = At_ClientIdToUserId(pstModemKeyInfo->ClientId, &ucIndex);
    if (AT_FAILURE == ulRslt)
    {
        AT_WARN_LOG("AT_PhMasterKeyHukInfoProc: At_ClientIdToUserId FAILURE!\n");
        goto error_handle;
    }
    /*如果加密客户端私钥失败则返回ERROR*/
    if(CODEING_ERROR == pstModemKeyInfo->stMasterHukKey.ucErrorBit)
    {
        vos_printf("AT_PhMasterKeyHukInfoProc : CODEING_ERROR!");
        mlog_print(MLOG_AT, mlog_lv_error, "[VSIM] : AT_PhMasterKeyHukInfoProc : CODEING_ERROR!\n");
        goto error_handle;
    }
    else
    {
        if(pstModemKeyInfo->stMasterHukKey.key_type == MASTER_KEY_TYPE)
        {
            /*将MASTERKEY加密后的秘钥写入NV中*/
            ulRslt = VSim_WriteNV(&pstModemKeyInfo->stMasterHukKey);
            if(0 != ulRslt)
            {
                vos_printf("[AT_PhMasterKeyHukInfoProc] : VSim_WriteNV error!\n");
                mlog_print(MLOG_AT, mlog_lv_error, "[VSIM] : AT_PhMasterKeyHukInfoProc : VSim_WriteNV error!\n");
                goto error_handle;
            }

            mlog_print(MLOG_AT, mlog_lv_info, "[VSIM] : AT_PhMasterKeyHukInfoProc : VSim_WriteNV ok!\n");
        }

#ifdef VSIM_DEBUG_INFO
        vos_printf("pstModemKeyInfo->stMasterHukKey.vsimCpublicKey.aucVal = %s\n", pstModemKeyInfo->stMasterHukKey.vsimCpublicKey.aucVal);
        vos_printf("aucCpublicKeyLen = %d\n", pstModemKeyInfo->stMasterHukKey.vsimCpublicHukKey.ulLen);
#endif
        /*BASE16*/
        At_Base16Encode(pstModemKeyInfo->stMasterHukKey.vsimCpublicKey.aucVal, tempCPublicHukKey, pstModemKeyInfo->stMasterHukKey.vsimCpublicKey.ulLen);
        At_Base16Encode(pstModemKeyInfo->stMasterHukKey.vsimCprivateKey.aucVal, tempCPrivateHukKey, pstModemKeyInfo->stMasterHukKey.vsimCprivateKey.ulLen);
        At_Base16Encode(pstModemKeyInfo->stMasterHukKey.vsimSpublicKey.aucVal, tempSPublicHukKey, pstModemKeyInfo->stMasterHukKey.vsimSpublicKey.ulLen);

#ifdef VSIM_DEBUG_INFO
        vos_printf("tempCPublicHukKey = %s, strlen(tempCPublicHukKey) = %d\n", tempCPublicHukKey, strlen(tempCPublicHukKey));
        vos_printf("tempCPrivateHukKey = %s, strlen(tempCPrivateHukKey) = %d\n", tempCPrivateHukKey, strlen(tempCPrivateHukKey));
        vos_printf("tempSPublicHukKey = %s, strlen(tempSPublicHukKey) = %d\n", tempSPublicHukKey, strlen(tempSPublicHukKey));
#endif
        strcat(tempKeyWriteToFile, tempCPublicHukKey);
        strcat(tempKeyWriteToFile, tempCPrivateHukKey);
        strcat(tempKeyWriteToFile, tempSPublicHukKey);
        if(pstModemKeyInfo->stMasterHukKey.key_type == MASTER_KEY_TYPE)
        {
            /*将数据写入文件*/
            ulRslt = BSP_remove(MASTER_KEY_PATH);  //lint -esym(526, BSP_remove) -esym(628, BSP_remove)
            if(ulRslt < 0)
            {
                vos_printf("MASTER_KEY_PATH BSP_remove fail ......\n");
                mlog_print(MLOG_AT, mlog_lv_error, "[VSIM] : MASTER_KEY_PATH BSP_remove fail!\n");
            }

            if(0 != VSim_WriteFile(MASTER_KEY_PATH, tempKeyWriteToFile, strlen(tempKeyWriteToFile)))
            {
                vos_printf("[VSim_Write] : VSim_WriteFile MASTER_KEY_PATH failed......\n");
                mlog_print(MLOG_AT, mlog_lv_error, "[VSIM] : VSim_WriteFile MASTER_KEY_PATH failed!\n");
                goto error_handle;
            }
            /*写完后将数据清0*/
            VOS_MemSet(g_cMasterKey, 0, AT_MASTER_KEY_MAX_LEN);
        }
        else if(pstModemKeyInfo->stMasterHukKey.key_type == DEVICE_KEY_TYPE)
        {
            /*将数据写入文件*/
            ulRslt = BSP_remove(DEVICE_KEY_PATH);  //lint -esym(526, BSP_remove) -esym(628, BSP_remove)
            if(ulRslt < 0)
            {
                vos_printf("DEVICE_KEY_PATH BSP_remove fail ......\n");
                mlog_print(MLOG_AT, mlog_lv_error, "[VSIM] : DEVICE_KEY_PATH BSP_remove fail!\n");
            }

            if(0 != VSim_WriteFile(DEVICE_KEY_PATH, tempKeyWriteToFile, strlen(tempKeyWriteToFile)))
            {
                vos_printf("[VSim_Write] : VSim_WriteFile DEVICE_KEY_PATH failed......\n");
                mlog_print(MLOG_AT, mlog_lv_error, "[VSIM] : VSim_WriteFile DEVICE_KEY_PATH failed!\n");
                goto error_handle;
            }
            /*写完后将数据清0*/
            VOS_MemSet(g_cDeviceKey, 0, AT_DEVICE_KEY_MAX_LEN);
        }
        else if(pstModemKeyInfo->stMasterHukKey.key_type == CONTENT_KEY_TYPE)
        {
            /*将数据写入文件*/
            ulRslt = BSP_remove(CONTENT_KEY_PATH);  //lint -esym(526, BSP_remove) -esym(628, BSP_remove)
            if(ulRslt < 0)
            {
                vos_printf("CONTENT_KEY_PATH BSP_remove fail ......\n");
                mlog_print(MLOG_AT, mlog_lv_error, "[VSIM] : CONTENT_KEY_PATH BSP_remove fail!\n");
            }

            if(0 != VSim_WriteFile(CONTENT_KEY_PATH, tempKeyWriteToFile, strlen(tempKeyWriteToFile)))
            {
                vos_printf("[VSim_Write] : VSim_WriteFile CONTENT_KEY_PATH failed......\n");
                mlog_print(MLOG_AT, mlog_lv_error, "[VSIM] : VSim_WriteFile CONTENT_KEY_PATH failed!\n");
                goto error_handle;
            }
            /*写完后将数据清0*/
            VOS_MemSet(g_cContentKey, 0, AT_CONTENT_KEY_MAX_LEN);
        ...
        vos_printf("[VSim_Write] : vsim key data write is ok!\n");
        mlog_print(MLOG_AT, mlog_lv_info, "[VSIM] : vsim key data write is ok!\n");
        *pResult = AT_OK;
    }

    return 0;
error_handle:
    AT_PhMasterKeyHukInfoParaProc();
    *pResult = AT_ERROR;
    return 0;
}
...
TAF_UINT16 AT_PhDieIdHukInfoProc(TAF_UINT8 *pData, TAF_UINT8 *pResult)
{
    VOS_UINT8                           ucIndex  = 0;
    VOS_UINT32                          ulRslt   = 0;
    TAF_PH_DIE_ID_HUK_MSG_INFO_STRU       *pstDieIdInfo = VOS_NULL_PTR;
    NV_VSIM_DIEID_STRU                  stDieIdinfo;

    if(VOS_NULL_PTR == pData)
    {
        vos_printf("[AT_PhDieIdHukInfoProc] : pData is NULL!\n");
        *pResult = AT_ERROR;
        return 0;
    }

    pstDieIdInfo = (TAF_PH_DIE_ID_HUK_MSG_INFO_STRU*)pData;
    ulRslt = At_ClientIdToUserId(pstDieIdInfo->ClientId, &ucIndex);

    if (AT_FAILURE == ulRslt)
    {
        vos_printf("[AT_PhDieIdHukInfoProc] : At_ClientIdToUserId error!\n");
        *pResult = AT_ERROR;
        return 0;
    }

    VOS_MemSet(&stDieIdinfo, 0, sizeof(NV_VSIM_DIEID_STRU));

    ulRslt = NV_Read(NV_VSIM_DIEID_INFO, &stDieIdinfo, sizeof(NV_VSIM_DIEID_STRU));

    if(NV_OK != ulRslt)
    {
        vos_printf("[AT_PhDieIdHukInfoProc] : read nv error!\n");
        *pResult = AT_ERROR;
        return 0;
    }

    VOS_MemCpy(stDieIdinfo.aucNum, pstDieIdInfo->stDieID.vsimDieIdHuk.aucVal, pstDieIdInfo->stDieID.vsimDieIdHuk.ulLen);

    stDieIdinfo.ulNumLen = pstDieIdInfo->stDieID.vsimDieIdHuk.ulLen;

    ulRslt = NV_Write(NV_VSIM_DIEID_INFO, &stDieIdinfo, sizeof(NV_VSIM_DIEID_STRU));

    if( NV_OK != ulRslt)
    {
        vos_printf("[AT_PhDieIdHukInfoProc] : NV_Write is error!\n");
        *pResult = AT_ERROR;
        return 0;
    }

    vos_printf("[AT_PhDieIdHukInfoProc] : die id write ok!\n");

    *pResult = AT_OK;
    return 0;
}

CRYPTO_TYPE_NAME vsim_crypto_type_name[CRYPTO_TYPE_MAX] =
{
    {RSA2048_RSA2048,      "0"},
    {AESCBC256_AESCBC256,  "1"},
...
{
    MODEM_ID_ENUM_UINT16               enModemId;
    TAF_PH_DIE_ID_INFO_MSG_INFO_STRU   *pstDieIdInfo = VOS_NULL_PTR;
    VSIM_AES_ENCRYPT_STRU *aes_info;
    VOS_UINT8     ucIndex  = 0;
    VOS_UINT32    ulRslt   = 0;
    VOS_INT32     iRet     = 0;
    VOS_UINT16    usLength;
    VOS_UINT8     aucAsciiImei[TAF_PH_IMEI_LEN + 1];       /*用此变量保存IMEI*/
    VOS_UINT32    crypto_type = CRYPTO_TYPE_MAX;           /*此变量为算法加密和签名类型，变更算法时必须更新此变量*/
    VOS_UINT8     output_data[VSIM_CONTENT_LEN];           /*IMEI|TYPE|SHA256|DIEID|SIGNATURE,长度最大申请为1K*/
    VOS_UINT8     output_data_verify[VSIM_HASH_LEN + 1];   /*用于装备校验字段，计算output_data的HASH*/
    VOS_UINT8     output_data_verify_base16[VSIM_HASH_LEN * 2 + 1]; /*用于装备校验字段，HASH做BASE16编码*/
    VOS_UINT8     aes_key_a_hash_base16[VSIM_HASH_LEN * 2 + 1] = "4717E86EB810886AC32D4972A2AA09194C0154646E6E75A8DB4290BDA53FACD7"; /*KEY_A的SHA值*/
    VOS_UINT8     dieid_base16[VSIM_HASH_LEN * 2 + 1];     /*AES CBC加密的DIEID需要进行BASE16编码*/
    VOS_UINT8     dieid_base16_lower[VSIM_HASH_LEN * 2 + 1]; /*DIEID base16小写存储*/
    VOS_UINT32    index = 0;

    enModemId   = MODEM_ID_0;
    usLength    = 0;
    crypto_type = AESCBC256_AESCBC256;   /*算法类型变更时，此变量必须要更新*/

    if(VOS_NULL_PTR == pData)
    {
        vos_printf("[AT_PhGetDieIdInfoProc] : pData is NULL!\n");
        *pResult = AT_ERROR;
        return 0;
    }

    pstDieIdInfo = (TAF_PH_DIE_ID_INFO_MSG_INFO_STRU*)pData;
    ulRslt = At_ClientIdToUserId(pstDieIdInfo->ClientId, &ucIndex);

    if (AT_FAILURE == ulRslt)
    {
        vos_printf("[AT_PhGetDieIdInfoProc] : At_ClientIdToUserId error!\n");
        *pResult = AT_ERROR;
        return 0;
    }
    ...
    if (VOS_OK != ulRslt)
    {
        vos_printf("[AT_PhGetDieIdInfoProc] : Get modem id fail!\n");
        *pResult = AT_ERROR;
        return 0;
    }

    aes_info = (VSIM_AES_ENCRYPT_STRU *)PS_MEM_ALLOC(WUEPS_PID_AT, sizeof(VSIM_AES_ENCRYPT_STRU));

    if(VOS_NULL_PTR == aes_info)
    {
        vos_printf("[AT_PhGetDieIdInfoProc] : malloc failed!\n");
        *pResult = AT_ERROR;
        return 0;
    }

    /*结构体变量清零操作*/
    PS_MEM_SET(aes_info, 0x0, sizeof(VSIM_AES_ENCRYPT_STRU));
    PS_MEM_SET(output_data, 0x0, sizeof(output_data));
    PS_MEM_SET(output_data_verify, 0x0, sizeof(output_data_verify));
    PS_MEM_SET(output_data_verify_base16, 0x0, sizeof(output_data_verify_base16));
    PS_MEM_SET(dieid_base16, 0x0, sizeof(dieid_base16));
    PS_MEM_SET(dieid_base16_lower, 0x0, sizeof(dieid_base16_lower));

    /* 1.读取NV项获取IMEI */
    ulRslt = AT_GetImeiValue(enModemId, aucAsciiImei);

    if (VOS_OK != ulRslt)
    {
        vos_printf("[AT_PhGetDieIdInfoProc] : Fail to read en_NV_Item_IMEI!\n");
        *pResult = AT_ERROR;
        PS_MEM_FREE(WUEPS_PID_AT, aes_info);
        return 0;
    }

    /* 2.生成DIEID加密的IV */
    ulRslt = VSIM_Get_Rand_Num(aes_info->aes_dieid_iv, VSIM_AES_IV_LEN);
    if (VOS_OK != ulRslt)
    {
        vos_printf("[AT_PhGetDieIdInfoProc] : VSIM_Get_Rand_Num FAIL!\n");
        *pResult = AT_ERROR;
        PS_MEM_FREE(WUEPS_PID_AT, aes_info);
        return 0;
    }

    /*2.1 在此需要对DIEID进行BASE16编码*/
    At_Base16Encode(pstDieIdInfo->stDieIdInfo.vsimDieId.aucVal, dieid_base16, pstDieIdInfo->stDieIdInfo.vsimDieId.ulLen);

    /*2.2 DIEID进行小写转换*/
    for(index = 0; index < (VOS_UINT32)AT_STRLEN(dieid_base16); index++)
    {
        if(dieid_base16[index] >= 'A' && dieid_base16[index] <= 'Z')
        {
            dieid_base16_lower[index] = dieid_base16[index] - 'A' + 'a';
        }
        else
        {
            dieid_base16_lower[index] = dieid_base16[index];
        }
    }

    /* 3.对称密钥KEY_A AESCBC加密 */
    iRet = keya_aes_encrypt_cbc(aes_info->aes_dieid_iv, dieid_base16_lower, AT_STRLEN(dieid_base16_lower),
                         aes_info->aes_dieid, &aes_info->aes_dieid_len);
    if (VOS_OK != iRet)
    {
        vos_printf("[AT_PhGetDieIdInfoProc] : keya_aes_encrypt_cbc FAIL!\n");
        *pResult = AT_ERROR;
        PS_MEM_FREE(WUEPS_PID_AT, aes_info);
        return 0;
    }
    /* 4.DIEID IV补充在DIEID密文前面 */
    PS_MEM_CPY(aes_info->aes_dieid_and_iv, aes_info->aes_dieid_iv, VSIM_AES_IV_LEN);
    PS_MEM_CPY(aes_info->aes_dieid_and_iv + VSIM_AES_IV_LEN, aes_info->aes_dieid, VSIM_AES_DIE_ID_LEN);

    /* 5.密文进行BASE16编码*/
    At_Base16Encode(aes_info->aes_dieid_and_iv, aes_info->aes_dieid_and_iv_base16, VSIM_AES_DIE_ID_LEN + VSIM_AES_IV_LEN);

    /* 6.AES_CONTENT = IMEI+CRYPTO_TYPE+KEYA_HASH+AES_DIEID */

    (void)snprintf(aes_info->aes_content, VSIM_CONTENT_LEN, "%s%s%s%s",
                   aucAsciiImei, vsim_crypto_type_name[crypto_type].name,
                   aes_key_a_hash_base16, aes_info->aes_dieid_and_iv_base16);

    /* 7.SHA256(AES_CONTENT) */
    sha256_hash(aes_info->aes_content, AT_STRLEN(aes_info->aes_content), aes_info->aes_content_sha256);

    /* 8.获取签名IV */
    ulRslt = VSIM_Get_Rand_Num(aes_info->aes_signature_iv, VSIM_AES_IV_LEN);
    if (VOS_OK != ulRslt)
    {
        vos_printf("[AT_PhGetDieIdInfoProc] : VSIM_Get_Rand_Num FAIL!\n");
        *pResult = AT_ERROR;
    ...
    /* 9.计算签名 */
    iRet = keyb_aes_encrypt_cbc(aes_info->aes_signature_iv, aes_info->aes_content_sha256, VSIM_HASH_LEN, aes_info->aes_signature, &aes_info->aes_signature_len);
    if (VOS_OK != iRet)
    {
        vos_printf("[AT_PhGetDieIdInfoProc] : keyb_aes_encrypt_cbc FAIL!\n");
        *pResult = AT_ERROR;
        PS_MEM_FREE(WUEPS_PID_AT, aes_info);
        return 0;
    }
    /* 10.签名前面加上IV */
    PS_MEM_CPY(aes_info->aes_signature_and_iv, aes_info->aes_signature_iv, VSIM_AES_IV_LEN);
    PS_MEM_CPY(aes_info->aes_signature_and_iv + VSIM_AES_IV_LEN, aes_info->aes_signature, aes_info->aes_signature_len);

    /* 11.签名进行BASE16编码 */
    At_Base16Encode(aes_info->aes_signature_and_iv, aes_info->aes_signature_and_iv_base16, VSIM_AES_IV_LEN + aes_info->aes_signature_len);

    /* 12.返回给装备的字段output_data = IMEI|CRYPTO_TYPE|KEYA_HASH|DIEID|SIGNATURE */
    (void)snprintf(output_data, VSIM_CONTENT_LEN, "%s|%s|%s|%s|%s",
                   aucAsciiImei, vsim_crypto_type_name[crypto_type].name,
                   aes_key_a_hash_base16, aes_info->aes_dieid_and_iv_base16,
                   aes_info->aes_signature_and_iv_base16);

    /* 13.返回给装备的字段进行SHA256计算摘要，用于装备侧校验 */
    sha256_hash(output_data, AT_STRLEN(output_data), output_data_verify);

    /* 14.HASH值进行BASE16编码 */
    At_Base16Encode(output_data_verify, output_data_verify_base16, VSIM_HASH_LEN);

    /* 15.返回给装备的字段格式为:CONTENT^HASH，其中HASH为CONTENT的SHA值 */
    usLength += (VOS_UINT16)At_sprintf(AT_CMD_MAX_LEN,
                                       (VOS_CHAR *)pgucAtSndCodeAddr,
                                       (VOS_CHAR *)pgucAtSndCodeAddr + usLength,
                                       "%s:%s,%d,%s",
                                       "GETKEYINFO",
                                       output_data,
                                       AT_STRLEN(output_data),
                                       output_data_verify_base16
                                       );
    gstAtSendData.usBufLen = usLength;
