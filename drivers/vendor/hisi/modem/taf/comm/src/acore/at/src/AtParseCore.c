...
extern VOS_UINT32  gulCustomVersion;
/* 命令表头结点 */
...
VOS_UINT32 atMatchCmdName (VOS_UINT8 ucClientId, VOS_UINT32 CmdType)
{
...
#if (FEATURE_ON == MBB_FEATURE_AT_CMD_FILTER) && (FEATURE_OFF == MBB_FACTORY)
    if (VOS_TRUE == g_bAtDataLocked)
    {
        /*当是升级版本DATALOCK未解锁时，才要判断白名单；解锁后白名单失效*/
        if ((VOS_FALSE == At_IsForbiddenAtCmd(pszCmdName, ucClientId)) /*AT不在漫游白名单中*/
#if ((FEATURE_ON == MBB_FEATURE_GATEWAY) && defined(CUSTOM_LIVEBOX_B2C))
           && (CONFIG_GATEWAY_NONE == GetGatewayWorkMode())
#endif
        )
        {
    ...
    if (VOS_TRUE == g_bAtDataLocked)
    {
        /*当是升级版本DATALOCK未解锁时，才要判断黑名单 ，解锁后黑名单失效*/
        if (VOS_TRUE == At_IsGlobalForbiddenAtCmd(pszCmdName, ucClientId))
        {
            return AT_CMD_NOT_SUPPORT;
        }
    }
...
VOS_VOID At_ReadyClientCmdProc(VOS_UINT8 ucIndex, VOS_UINT8 *pData, VOS_UINT16 usLen)
{
    ...
    /*SIMLOCKEX命令可能有61个参数，特殊处理*/
    if (AT_SUCCESS == At_ProcSimLockExPara(ucIndex, pData, usLen))
    {
        return;
    }
#endif
...
VOS_VOID At_CmdMsgDistr(AT_MSG_STRU *pstMsg)
{
    ...
    AT_USB_DEBUG_PRINT_1(pstMsg->ucType);
