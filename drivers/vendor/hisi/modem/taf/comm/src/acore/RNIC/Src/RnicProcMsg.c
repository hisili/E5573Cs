...
#if (FEATURE_ON == MBB_FEATURE_UNI_PS_CALL)
VOS_VOID RNIC_RegDL_DataCallback(
    VOS_UINT8                           ucRabid,
...
)
{
    ...
    if (0  == *pucSystemAppConfig)
    {
        if ( (VOS_NULL_PTR != pstFilterIpAddr) && (VOS_NULL_PTR != pFunc) )
        {
            /* 注册下行发送函数，ADS调用注册的函数发送下行数据 */
            ADS_DL_RegFilterDataCallback(ucRabid, pstFilterIpAddr, pFunc);
        }
        else
        {
            ADS_DL_DeregFilterDataCallback(ucRabid);
        }
    }
    else
    {
        ...
        {
            /* 注册下行发送函数，ADS调用注册的函数发送下行数据 */
            ADS_DL_RegDlDataCallback(ucRabid, pFunc);
        }
    }
...
)
{
    ...
    VOS_UINT8                           ucRmNetId;
#if (FEATURE_ON == MBB_FEATURE_UNI_PS_CALL)
    ADS_FILTER_IP_ADDR_INFO_STRU        stFilterIpAddrInfo = {0};
#endif
...
#if (FEATURE_ON == MBB_FEATURE_UNI_PS_CALL)
    RNIC_RegDL_DataCallback(ucRabid, &stFilterIpAddrInfo, (RCV_DL_DATA_FUNC)RNIC_RcvAdsDlData);
#else
    /* 注册下行发送函数，ADS调用注册的函数发送下行数据 */
...
#endif
#if (FEATURE_ON == MBB_FEATURE_UNI_PS_CALL)
    RNIC_RegDL_DataCallback(pstRcvInd->ucRabId, (ADS_FILTER_IP_ADDR_INFO_STRU*)VOS_NULL_PTR, (RCV_DL_DATA_FUNC)VOS_NULL_PTR);
#endif
