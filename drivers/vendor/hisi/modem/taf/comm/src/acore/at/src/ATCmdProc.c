...
#if(FEATURE_ON == MBB_FEATURE_BOX_FTEN)
AT_I_TYPE                               gucAtIType      = AT_I_USE5_TYPE;
#endif
...
VOS_VOID AT_MBB_MlogPrintPdpInfo(VOS_UINT8 ucCallId, TAF_PS_CALL_PDP_ACTIVATE_CNF_STRU   *pEvtInfo)
{
    ...
    else
    {
        ...
        mlog_print("at", mlog_lv_info, "IPv6 Bearer Primary DNS %s", aucIpv6PrimDnsStr);
        mlog_print("at", mlog_lv_info, "IPv6 Bearer Secondary DNS %s", aucIpv6SecDnsStr);
        if((TAF_PDP_IPV4V6 == enPdpType))
        {
            mlog_print("at", mlog_lv_info, "IPv4 Bearer Primary DNS 0x%x", pstCallEntity->stIpv4DhcpInfo.ulIpv4PrimDNS);
            mlog_print("at", mlog_lv_info, "IPv4 Bearer Secondary DNS 0x%x", pstCallEntity->stIpv4DhcpInfo.ulIpv4SecDNS);
            mlog_print("at", mlog_lv_info, "IPv4 Bearer Gate Way 0x%x", pstCallEntity->stIpv4DhcpInfo.ulIpv4GateWay);
        }
#endif
    }
