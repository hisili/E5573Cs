...
#if ( FEATURE_ON == MBB_WPG_COMMON )
                /*334这个mcc在表格中有个两个mnc，一个是50，一个是050，
                    所以这里不能直接使用长度判断*/
...
VOS_VOID At_RcvVcMsgSetPortCnfProc(MN_AT_IND_EVT_STRU *pstData)
{
    ...
    if (VOS_TRUE == pstEvent->bSuccess)
    {
        ...
        /*DDSETEX 命令设置切换端口为语音模式，需要使能diag端口让语音数据可以正常畅通*/
