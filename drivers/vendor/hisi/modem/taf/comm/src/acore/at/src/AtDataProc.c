...
AT_PS_RMNET_ID_TAB                       g_astPsRmNetIdTab[] = {{FC_ID_NIC_1, MODEM_ID_0, AT_PS_USER_CID_1},
                                                                {FC_ID_NIC_2, MODEM_ID_0, AT_PS_USER_CID_2},
                                                                {FC_ID_NIC_3, MODEM_ID_0, AT_PS_USER_CID_3},
                                                                {FC_ID_NIC_4, MODEM_ID_0, AT_PS_USER_CID_4},
                                                                {FC_ID_NIC_5, MODEM_ID_0, AT_PS_USER_CID_5},
                                                                };
#else
AT_PS_RMNET_ID_TAB                      g_astPsRmNetIdTab[] = {{FC_ID_NIC_1, MODEM_ID_0, AT_PS_USER_CID_1},
                                                               {FC_ID_NIC_2, MODEM_ID_0, AT_PS_USER_CID_2},
    ...
    if (TAF_PDP_IPV4V6 == g_enAtFirstNdisActPdpType)
    {
        if (TAF_PDP_IPV4V6 == g_enAtNdisActPdpType)
        {
            ...
            if (  ((TAF_PS_CAUSE_SM_NW_UNKNOWN_PDP_ADDR_OR_TYPE == pstEvent->enCause) 
              || (TAF_PS_CAUSE_SM_NW_ACTIVATION_REJECTED_BY_GGSN == pstEvent->enCause)
              || (TAF_PS_CAUSE_SM_NW_ACTIVATION_REJECTED_UNSPECIFIED == pstEvent->enCause)
              || (TAF_PS_CAUSE_SM_NW_SERVICE_OPTION_NOT_SUPPORTED == pstEvent->enCause)
              || (TAF_PS_CAUSE_SM_NW_REQUESTED_SERVICE_NOT_SUBSCRIBED == pstEvent->enCause)))
...
FC_PRI_ENUM_UINT32                  enFCPri;
   ...
   /* 设置FCID与FC Pri的映射关系 */
    ...
    g_stFcIdMaptoFcPri[ulFcId].ulFcPri     = enFCPri;
...
)
{
    ...
    if((TAF_PS_CAUSE_SM_NW_UNKNOWN_PDP_ADDR_OR_TYPE == enCause) 
        || (TAF_PS_CAUSE_SM_NW_ACTIVATION_REJECTED_BY_GGSN == enCause)
...
#if(FEATURE_ON == MBB_FEATURE_MPDP)
        enDefaultFcId = AT_GetDefaultFcID(pstModemPsCtx, ucCallId, ulRmNetId);
#else/* FEATURE_ON == MBB_FEATURE_MPDP */
        /* 获取网卡ID对应的FC ID */
...
)
{
    ...
    {
#if (FEATURE_ON == MBB_WPG_COMMON)
        if ((0 != gastAtParaList[3].usParaLen) && (0 != gastAtParaList[4].usParaLen) && (0 == gastAtParaList[5].ulParaValue))
        {
...
VOS_UINT32 AT_PS_ProcDialUp(VOS_UINT8 ucIndex)
{
...
#if(FEATURE_ON == MBB_FEATURE_MPDP)
    AT_PS_UpdateUserDialDnsInfo(ucIndex, &stUsrDialParam);
#endif
    ...
    /*重新发起拨号，判断拨号使用的CID是否已经被人使用，重新获取拨号CID*/
    stUsrDialParam.ucCid = AT_PS_FindDialCid(ucIndex, &stUsrDialParam);

    if ( AT_PS_CALL_INVALID_CID == stUsrDialParam.ucCid )
    {
