...
PRI_CC_FLAGS	:= -c -LNO:simd -OPT:space_flix=1  -fno-zero-initialized-in-bss -fcommon -fmessage-length=0 -ffunction-sections -DPROC_$(CFG_XTENSA_CORE) -DCONFIG_$(CFG_XTENSA_CORE) \
				-DTENSILICA_PLATFORM -DLPS_RTT --xtensa-system=$(subst \,/,$(CFG_XTENSA_SYSTEM)) --xtensa-core=$(CFG_XTENSA_CORE) --xtensa-params= 
...
MY_PRI_FILE=$(LOCAL_OBJ_PATH)/modem/phy/lt/code/src/pub/tds_mini/jd/checom/tds_che_common.o  $(LOCAL_OBJ_PATH)/modem/phy/lt/code/src/pub/tds_mini/common/tds_comm_fun.o $(LOCAL_OBJ_PATH)/modem/phy/lt/code/src/tds/jd/che/tds_jdspecial.o $(LOCAL_OBJ_PATH)/modem/phy/lt/code/src/tds/jd/che/tds_jdtask.o $(LOCAL_OBJ_PATH)/modem/phy/lt/code/src/tds/jd/jd/tds_jd.o 

COMMON_C_OBJS:=$(filter-out $(MY_PRI_FILE),$(FULL_C_OBJS))
#$(FULL_C_OBJS): $(LOCAL_OBJ_PATH)/%.o : $(BALONG_TOPDIR)/%.c | do_build_before
$(COMMON_C_OBJS): $(LOCAL_OBJ_PATH)/%.o : $(BALONG_TOPDIR)/%.c | do_build_before
	@echo [CC] $@
...
$(MY_PRI_FILE): $(LOCAL_OBJ_PATH)/%.o : $(BALONG_TOPDIR)/%.c | do_build_before
	@echo [CC] $@
	$(Q)-mkdir -p $(patsubst %/,%,$(dir $@))	
	$(Q)$(CC) $(CC_USER_FLAGS)  $(PRI_CC_FLAGS) $(FULL_SRC_INC_PATH) -MD -MF $(LOCAL_OBJ_PATH)/$*.d  -c $< -o $@ 
ifeq ($(CFG_FEATURE_OBJ_DUMP),YES)
	@echo [DUMP] $(LOCAL_OBJ_PATH)/$(notdir $@)
	$(Q)cp -f $@ $(LOCAL_OBJ_PATH)/
endif
