# Created by zhaijiye/00199916, 2012-03-25
...
# compiler config
#***********************************************************
TENSILICA_TOOLS             =$(ROOT_XTENSA_PATH_W)/XtDevTools/install/tools/RD-2012.4-linux
...
CC			:= $(SOURCEANALYZER) $(TENSILICA_TOOLS)/XtensaTools/bin/xt-xcc 
AS			:= $(SOURCEANALYZER) $(TENSILICA_TOOLS)/XtensaTools/bin/xt-xcc 
...
CC_FLAGS	:= -c -OPT:space_flix=1 -Os -fno-zero-initialized-in-bss -fcommon -fmessage-length=0  -DPROC_$(CFG_XTENSA_CORE) -DCONFIG_$(CFG_XTENSA_CORE) \
...
$(LOCAL_IMAGE_FILE) : $(LOCAL_LIBRARY) | do_post_build_before
	@echo [BIN] $@
	@xt-nm --format=posix $(LOCAL_LIBRARY) > $(LOCAL_MAP_FILE)
	@xt-objdump -D $(LOCAL_LIBRARY)    > "$(LOCAL_DUMP_FILE)"
	@xt-objdump -h $(LOCAL_LIBRARY)
	@xt-objcopy -R .debug_aranges $(LOCAL_LIBRARY)
	@xt-objcopy -R .debug_abbrev $(LOCAL_LIBRARY)
	@xt-objcopy -R .debug_pubnames $(LOCAL_LIBRARY)
	@xt-objcopy -R .debug_info $(LOCAL_LIBRARY)
	@xt-objcopy -R .debug_line $(LOCAL_LIBRARY)
	@xt-objcopy -R .xtensa.info $(LOCAL_LIBRARY)
	@xt-objcopy -R .xt.lit $(LOCAL_LIBRARY)
	@xt-objcopy -R .xt.prop $(LOCAL_LIBRARY)
	@xt-objcopy -R .comment $(LOCAL_LIBRARY)
	#@xt-objcopy -O binary $(LOCAL_LIBRARY) $(OBC_LOCAL_MOUDLE_ITCM_NAME) 
	@xt-dumpelf $(DTCM_FLAGS) $(LOCAL_LIBRARY) > $(LOCAL_OBJ_PATH)/$(OBC_LOCAL_MOUDLE_NAME)_dtcm.dram0.tmp
	wine $(ASC_TO_BIN_TOOL) $(LOCAL_OBJ_PATH)/$(OBC_LOCAL_MOUDLE_NAME)_dtcm.dram0.tmp $(LOCAL_MOUDLE_DTCM_NAME)
	@xt-dumpelf $(ITCM_FLAGS) $(LOCAL_LIBRARY) > $(LOCAL_OBJ_PATH)/$(OBC_LOCAL_MOUDLE_NAME)_itcm.iram0.tmp
