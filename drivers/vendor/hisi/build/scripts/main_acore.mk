...
#	$(Q)cp -rf $(BALONG_TOPDIR)/modem/system/external/yaffs2_test/ $(OBB_ANDROID_DIR)/external
	$(Q)cp -rf $(BALONG_TOPDIR)/modem/system/external/wifi/ $(LINUX_KERNEL)/drivers
	$(Q)cp -rf $(BALONG_TOPDIR)/modem/system/external/zarlink_api/  $(LINUX_KERNEL)/sound/soc/balong
endif
#   begin  w00195735 DTS2014101405646   20141013  for cradle wifi compile  test
	@echo build_cradle_seperate
ifeq ($(strip $(CFG_BSP_CONFIG_BOARD_E5575S_210)),YES) 
ifeq ($(strip $(MBB_CRADLE_FEATURE)),FEATURE_ON)
	$(Q)cp -rf $(BALONG_TOPDIR)/config/product/$(OBB_PRODUCT_NAME)/os/autorun/af35/autorun.sh $(BALONG_TOPDIR)/modem/eUAP/device/huawei-mbb/p711/e5575s_210/factory/rootdir/etc
else
	$(Q)cp -rf $(BALONG_TOPDIR)/config/product/$(OBB_PRODUCT_NAME)/os/autorun/e5575s-210/autorun.sh $(BALONG_TOPDIR)/modem/eUAP/device/huawei-mbb/p711/e5575s_210/factory/rootdir/etc
endif
endif
ifeq ($(strip $(CFG_BSP_CONFIG_BOARD_E5575S_320)),YES) 
ifeq ($(strip $(MBB_CRADLE_FEATURE)),FEATURE_ON)
	$(Q)cp -rf $(BALONG_TOPDIR)/config/product/$(OBB_PRODUCT_NAME)/os/autorun/af35/autorun.sh $(BALONG_TOPDIR)/modem/eUAP/device/huawei-mbb/p711/e5575s_320/factory/rootdir/etc
else
	$(Q)cp -rf $(BALONG_TOPDIR)/config/product/$(OBB_PRODUCT_NAME)/os/autorun/e5575s-320/autorun.sh $(BALONG_TOPDIR)/modem/eUAP/device/huawei-mbb/p711/e5575s_320/factory/rootdir/etc
endif
endif
#   end   w00195735 DTS2014101405646  20141013 for cradle wifi compile  test
