...
/******************************************************************************
Function:       chg_set_prechg_threshold
Description:    设定充电芯片对电池的预充电压门限
Input:          预充电电流值
Output:         N/A
Return:         TRUE     : 函数执行成功
                FALSE    : 函数执行失败
Others:         BQ24161特有
******************************************************************************/
boolean chg_set_prechg_threshold(uint32_t pre_vol)
{
    uint8_t reg     = 0;
    uint16_t reg_val = 0;
    int32_t ret     = 0;
    ...
    if (0 != ret)
    {
        chg_print_level_message(CHG_MSG_ERR, "ERROR READ chg_set_prechg_threshold!\n",0,0,0);
        return FALSE;
    }
    else
    {
    ...
    reg_val &= ~(BQ24192_BATLOW_VOL_MASK);
    if(pre_vol == BQ24192_BATLOW_VOL_2V8)
    {   /*  2V8 置位 0 */
        reg_val &= ~(BQ24192_BATLOW_VOL);
    }
    else
    {   /* 其它情况都默认3V  */
        reg_val |= BQ24192_BATLOW_VOL;
    ...
    chg_print_level_message(CHG_MSG_INFO, "CHG_DRV_DBG:chg_set_prechg_threshold() \
        reg %d, value 0x%x after!\n",reg, reg_val,0);
    ret = bq24192_i2c_write(reg, reg_val);
    if (0 != ret)
    {
...
boolean chg_is_charger_present(void)
{
    ...
    {
        chg_print_level_message(CHG_MSG_DEBUG, "CHG_DRV:chg_is_charger_present->  NO present\n");
        return FALSE;
    }
    else
    {
...
boolean chg_chip_init(void)
{
    ...
    int dpm_volt = CHG_DEFAULT_DPM_VOLT;
    if(!is_5577cs_falg)
        dpm_volt = CHG_DEFAULT_DPM_VOLT_H;
    #endif
    ...
    #if defined(BSP_CONFIG_BOARD_E5577S_321) || defined(BSP_CONFIG_BOARD_E5577S_932) \
    || defined(BSP_CONFIG_BOARD_E5577S_324) 
    else if (FALSE == chg_set_dpm_val(dpm_volt))
    {
    ...
    else if (FALSE == chg_set_prechg_threshold(2800))
    {
        /*If error occured, set 15 bit of ret_code.*/
        ret_code |= (1 << 15);
    }
#endif
    else
