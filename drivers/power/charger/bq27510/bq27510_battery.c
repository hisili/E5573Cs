...
#else  /*  e5771S-852 和 e5771h-937 用else分支 */
...
int hisi_bq27510_battery_qmax()
{
    CHECK_DI();
    return bq27510_get_gasgauge_qmax(g_battery_measure_by_bq27510_device);
}
/*
...
static int bq27510_update_firmware(struct i2c_client *client, const char *pFilePath)
{
    ...
    CHG_BATT_ID e5771s_battery_id = CHG_BATT_ID_MAX;
    /* open file */
...
#elif defined(BSP_CONFIG_BOARD_E5771S_852) || defined(BSP_CONFIG_BOARD_E5771H_937)  /* 5771根据电池ID取不同的FW */
    e5771s_battery_id = chg_get_batt_id();
    printk("[%s,%d]:e5771s_battery_id = %d \n",__FUNCTION__,__LINE__,e5771s_battery_id);
    switch(e5771s_battery_id)
    {
        case CHG_BATT_ID_XINGWANGDA:
        case CHG_BATT_ID_FEIMAOTUI:
            buf = bq27510_cmd_buffer_sanyang_ver1_0;
        break;
        default:
            return  1;
    }
...
#if defined(BSP_CONFIG_BOARD_E5_E5770s)
    if(strlen(coul_firmware_update_info.battery_sn) == 0)
    {
        printk("[%s] LINE [%d]No battery SN\n",__FUNCTION__,__LINE__);
        coul_update_status_flag = true;
        return 1;
    }
...
ssize_t bq27510_coul_firmware_update_init(void)
{
    ...
    printk(" bq27510_coul_firmware_update_init version =%d ++++++++++++\r\n",version);
    if(nv_revert_flag || (version < 0))
    {
...
void bq27510_sync_coulupdate_port_control(void)
{
    ...
    {
        printk("[%s]: LINE [%d] not need to delay notify port enum done \r\n",\
                    __FUNCTION__,__LINE__);
        return;
    }
    else
    ...
    printk("[%s]: LINE [%d] coul update success count = %d \r\n",\
                    __FUNCTION__,__LINE__,count);
    bq27510_set_coul_update_start_flag(false);
    bq27510_set_coul_update_status_flag(false);
    printk("[%s]: LINE [%d] start_coul_update_flag = %d \r\n",\
          __FUNCTION__,__LINE__,bq27510_get_coul_update_status_flag());
    printk("[%s]: LINE [%d] coul_update_status_flag = %d \r\n",\
         __FUNCTION__,__LINE__,bq27510_get_coul_update_start_flag());
    return;
}
