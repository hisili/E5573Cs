...
/*
 * linux/drivers/power/bq27510_battery_monitor.c
 *
...
#define BATTERY_PERCENT_OF_TEN       (10)
extern int32_t chg_batt_temp_revise(int32_t batt_temp);
#endif
...
#define BATTERY_PERCENT_SHUTDOWN        (1)
#else
...
static int capacity_changed(struct bq_bci_device_info *di)
{
    ...
    */
    ...
    if ((curr_capacity <= BATTERY_PERCENT_SHUTDOWN) && \
        (!chg_is_batt_in_state_of_emergency()) && (TRUE == is_chg_charger_removed()))
    {   /*电压未达到关机门限时，soc低于关机门限1%,则显示2%*/
        curr_capacity = BATTERY_PERCENT_SHUTDOWN + 1;
        dev_err(di->dev, "no emergency!adjust Battery Capacity to %d Percents\n", \
            curr_capacity);
    }    
    else if(chg_is_batt_in_state_of_emergency() && (curr_capacity > BATTERY_PERCENT_SHUTDOWN))
    {    /*电池达到低电关机门限，迅速将soc平滑至关机门限*/
        curr_capacity = di->prev_capacity - 1;
        dev_err(di->dev, "batt emergency!adjust Battery Capacity to %d Percents\n", \
            curr_capacity);
    }
    else
    {
    ...
    case POWER_SUPPLY_STATUS_FULL:
        if(curr_capacity <= 100 && soc_smooth_flag == 0)
        {
            curr_capacity = di->prev_capacity + 1;
        ...
        else
        {
            ...
            {   /*满充下有耗电，电量向下平滑 */
                if(di->prev_capacity - curr_capacity >= 1)
                {
                    curr_capacity = di->prev_capacity - 1;
                }
            }
            else
            {   /* 无耗电，电量保持上次值。若电量低于100，则下次轮询再把电量平滑到100 */
    ...
    dev_info(di->dev,"Capacity Updated = %d,prev_capacity = %d,charge_status = %d\n",\
                         curr_capacity, di->prev_capacity, di->charge_status);
    if(di->prev_capacity == curr_capacity)
    {
        return 0;
    }
...
#if defined(BSP_CONFIG_BOARD_E5_E5770s) || defined(BSP_CONFIG_BOARD_E5771S_852)
#else
...
#if defined(BSP_CONFIG_BOARD_E5_E5770s) || defined(BSP_CONFIG_BOARD_E5771S_852)
    if ((curr_capacity <= BATTERY_PERCENT_SHUTDOWN) && \
        !chg_is_powdown_charging() && BAT_VOL_3300 <= chg_get_sys_batt_volt() && \
            (TRUE == is_chg_charger_removed()))
    {
        curr_capacity = BATTERY_PERCENT_SHUTDOWN + 1;
        dev_err(di->dev, "no emergency!adjust Battery Capacity to %d Percents\n", \
            curr_capacity);
    }
    else if (chg_is_batt_in_state_of_emergency() && \
        (curr_capacity > BATTERY_PERCENT_SHUTDOWN) && \
            (curr_capacity <= BATTERY_PERCENT_OF_TEN))
    {
        curr_capacity = di->prev_capacity - 1;
        dev_err(di->dev, "batt emergency!adjust Battery Capacity to %d Percents\n", \
            curr_capacity);
...
int bq_charger_event_rcv (unsigned long evt)
{
    ...
    case VCHRG_STOP_CHARGING_EVENT:
        dev_err(di->dev, "%s charger event=%d\n", __func__,VCHRG_STOP_CHARGING_EVENT);
    ...
    case VCHRG_NOT_CHARGING_EVENT:
        dev_err(di->dev, "%s charger event=%d\n", __func__,VCHRG_NOT_CHARGING_EVENT);
...
static void bq_write_temp_to_share_work(struct work_struct *work)
{
    ...
    p_area->chan_out[0].temp_l = temp;
    p_area->chan_out[0].temp_r = ~temp;
...
static int bq_bci_battery_suspend(struct platform_device *pdev,pm_message_t state)
{
    ...
    dev_info(&pdev->dev, "suspend enter!\n");
...
static int bq_bci_battery_resume(struct platform_device *pdev)
{
    ...
    dev_info(&pdev->dev, "resume enter!\n");
