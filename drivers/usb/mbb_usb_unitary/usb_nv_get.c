...
USB_VOID usb_mode_log_lever_init(USB_VOID)
{
    ...
    /*TRUE为不打开no autorun  */
    uint16_t g_nv_autorun = MBB_USB_TRUE;
#endif
...
USB_INT  usb_nv_get(USB_VOID)
{
    ...
    {
        DBG_T(MBB_USB_NV , "Now not support  NV %d! for  DTS2014021309015 \r\n", USB_NV_PROT_OFFSET_ID);
...
#if(FEATURE_ON == FEATURE_NO_AUTORUN)
    /*pnp autorun*/
    ret = usb_read_nv(USB_NV_NO_AUTORUN_ID,
      &(g_nv_autorun), sizeof(g_nv_autorun));
    if (MBB_USB_OK != ret )
    {
        DBG_E(MBB_USB_NV, "%s: NV = %d failed\r\n", __func__, USB_NV_NO_AUTORUN_ID);
        g_nv_autorun = MBB_USB_TRUE;
    }
    else
    {
        DBG_E(MBB_USB_NV, "%s: NV = %d g_nv_autorun=%d \r\n", __func__,
        USB_NV_NO_AUTORUN_ID,g_nv_autorun);
    }
    ...
    if ((MBB_USB_OK != ret)
        ||( MBB_NV_DISENABLE== g_usb_nv_info.usb_security_flag.nv_status))
    {
        ...
        g_usb_nv_info.usb_security_flag.om_enable = 0;
    }
#if(FEATURE_ON == MBB_USB_FTEN_SWITCH)
    /* USB_NV_FTEN_SWITCH_SEL NV50607 */
    memset( &(g_usb_nv_info.usb_ften_switch_sel), 0, sizeof(nv_ften_usb_switch_sel));
    ret = usb_read_nv(USB_NV_FTEN_SWITCH_SEL, &(g_usb_nv_info.usb_ften_switch_sel),
                      sizeof(nv_ften_usb_switch_sel));
    if ((MBB_USB_OK != ret)
        || ( MBB_NV_DISENABLE == g_usb_nv_info.usb_ften_switch_sel.nv_status))
    {
        DBG_T(MBB_USB_NV , "NV %d use default, ret=%d nv_status=%ld\r\n",
            USB_NV_FTEN_SWITCH_SEL, ret, g_usb_nv_info.usb_ften_switch_sel.nv_status);
        g_usb_nv_info.usb_ften_switch_sel.nv_status = 0;
        g_usb_nv_info.usb_ften_switch_sel.usb_sel_flag = 0;
    }
#endif
...
USB_VOID usb_nv_dump(USB_VOID)
{
    ...
    DBG_T(MBB_DEBUG, "|U_NV: om_enable = %d\n",g_usb_nv_info.usb_security_flag.om_enable);
    DBG_T(MBB_DEBUG, "|-------------------------------------------------------\n");
}