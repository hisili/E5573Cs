			      ...
			      const char *buf, size_t count)
{
    ...
    if ( MBB_USB_FALSE == pnp_if_cdrom_can_open())//如果是假关机或者关机状态，直接返回
    {
    ...
    if((curlun->cdrom) && (USB_CONFIG_MBIM == usb_get_config_descripts_number())) {
        rc = fsg_lun_open(curlun, CDROM_OPENSRC_MNT_DIR);
        if (rc == 0){
