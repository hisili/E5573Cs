...
#include "bsp_sram.h"
#include "hi_gpio.h"
static int g_ften_usb_desc_flag = 1;
static int g_ften_usb_enum_flag = 0;
static int g_ften_cur_usb = 0;
static int g_type_of_supply = -1;
#define DETECT_FTEN_USB_ENUM_TIME 15000
#endif

#if (FEATURE_ON == FEATURE_NO_AUTORUN)
extern  uint16_t  g_nv_autorun;
extern int g_no_autorun;
#endif
...
#define RD_DEBUG_ON_LINUX  //because livebox use linux rewind command, we should enable this define
#endif
#endif
...
huawei_dynamic_info_st  g_pnp_info  = {0};
...
static void alloc_string_desc(struct usb_composite_dev* cdev)
{
    int id;
    mbb_usb_nv_info_st* usb_nv_info = usb_nv_get_ctx();
...
#if(FEATURE_ON == MBB_USB_FTEN_SWITCH)
static void alloc_string_desc_ften(struct usb_composite_dev* cdev)
{
    int id;
    mbb_usb_nv_info_st* usb_nv_info = usb_nv_get_ctx();
    ...
    strings_dev[STRING_MANUFACTURER_IDX].id = 3;
    device_desc.iManufacturer = 3;

    strings_dev[STRING_PRODUCT_IDX].id = 2;
    device_desc.iProduct = 2;

    strncpy(manufacturer_string, "Seiko Instruments Inc.", sizeof(manufacturer_string) - 1);

    strncpy(product_string, "TC-T200", sizeof(product_string) - 1);

    strncpy(serial_string, "1234567890ABCDEF", sizeof(serial_string) - 1);

    strings_dev[STRING_SERIAL_IDX].id = 4;
    device_desc.iSerialNumber = 4;
...
static int android_bind(struct usb_composite_dev* cdev)
{
    ...
    {
        alloc_string_desc_ften(cdev);
    }
    else
    {
        alloc_string_desc(cdev);
    }
#else
    {
	alloc_string_desc(cdev);
...
#endif
        DBG_T(MBB_PNP, "DLOAD PCUI COMPLETE\n");
        return PNP_CHECK_DEVICE_STATUS_COMPLETE;
    }
...
USB_INT pnp_status_rewind_before(USB_VOID)
{
    ...
    if (FTEN_USB_DESC_FTEN == get_ften_desc_flag())
    {
        g_pnp_info.portModeIndex = FTEN_USB_SWITCH_MODE;
    }
    else
    {
...
#if (FEATURE_ON == MBB_FEATURE_GATEWAY)
#ifdef CUSTOM_LIVEBOX_B2C
USB_VOID  create_rerwind_command (unsigned char bPCType,  unsigned char bProfile)
{
    g_pnp_info.rewind_param.bPcType = bPCType;
    g_pnp_info.rewind_param.bProFile = bProfile;
}
#endif
#endif
...
#if (FEATURE_ON == MBB_FEATURE_GATEWAY)
#ifdef CUSTOM_LIVEBOX_B2C
    if ( (MEMORY_AXI_APP_DISABLE_MAGIC == GetAppStartFlag())
        && (CTL_APP_WAIT == g_pnp_info.app_start_mode))
    {
        //debug via linux
#ifdef RD_DEBUG_ON_LINUX
        if (LINUX_OS_FLAG == g_pnp_info.rewind_param.bPcType)
        {
            DBG_T(MBB_PNP, "PNP:find disable app, and then rewind to gate fake\n");
            create_rerwind_command (GATEWAY_OS_FLAG, GATEWAY_NDIS_MODE); /*gate --ncm-ndis*/
            //create_rerwind_command (GATEWAY_OS_FLAG, GATEWAT_MODEM_MODE); /*gate --modem*/
            (void)pnp_set_gateway_mode();
            pnp_set_ctl_app_flag(CTL_APP_DISABLE);
            SetAppStartFlag(MEMORY_AXI_APP_START_MAGIC); //清除不启动app标记
            goto switch_mode;
        }
#endif
        //run on livebox flow
        ...
        {
            DBG_T(MBB_PNP, "PNP:find disable app, and then rewind to gate\n");
            (void)pnp_set_gateway_mode();
            pnp_set_ctl_app_flag(CTL_APP_DISABLE);
            SetAppStartFlag(MEMORY_AXI_APP_START_MAGIC);    //清除不启动app标记        
            goto switch_mode;
...
#ifdef CUSTOM_LIVEBOX_B2C
        //第一次切换，模拟为linux工作场景
        create_rerwind_command (LINUX_OS_FLAG, GATEWAY_NDIS_MODE);
        pnp_set_ctl_app_flag(CTL_APP_START);
                                      ...
                                      size_t size, loff_t* ppos )
{
    ...
    switch (mode_buf[APP_WRITE_DATA_SITE])
    {
        ...
        case CTL_APP_DISABLE_FALG:
            pnp_set_ctl_app_flag(CTL_APP_DISABLE);
#if (FEATURE_ON == MBB_FEATURE_GATEWAY)
            SetAppStartFlag(MEMORY_AXI_APP_DISABLE_MAGIC);
#endif
            break;
...
USB_VOID usb_ctl_app_start_flag_init( USB_VOID )
{
    ...
    else if (MEMORY_AXI_APP_START_MAGIC == GetAppStartFlag())
    {
        pnp_set_ctl_app_flag(CTL_APP_START);
    }
    else
    {
        pnp_set_ctl_app_flag(CTL_APP_START);
        SetAppStartFlag(MEMORY_AXI_APP_START_MAGIC); //add init
    }
    SetGatewayWorkMode(CONFIG_GATEWAY_NONE);//add init
#else
    pnp_set_ctl_app_flag(CTL_APP_START);
#endif
#else
...
USB_INT pnp_huawei_vendor_setup(struct usb_gadget* gadget, const struct usb_ctrlrequest* ctrl)
{
    ...
    switch (ctrl->bRequest)
    {
...
#if(FEATURE_ON == FEATURE_NO_AUTORUN)
            DBG_E(MBB_PNP, "++++++++++++ U_PNP:DE recive +++++++++++ \n");
            /*如果NV61为0，去autorun方案激活，收到0xDE/EE/FE时，设置切换标志位*/
            if( MBB_USB_FALSE == g_nv_autorun)
            {
                 g_no_autorun = MBB_USB_TRUE;
            }
#endif
        ...
        case USB_REQ_GET_MSOS_CUSTOM_MAC://mac os service installed
            ...
            break;
        case USB_REQ_GET_MSOS_DEF://windwos service not installed
            if (USB_REQ_TYPE_MSOS_IN == ctrl->bRequestType )
            {
                DBG_I(MBB_PNP, "C0 FE recive\n");
                #if(FEATURE_ON == FEATURE_NO_AUTORUN)
                /*如果NV61为0，去autorun方案激活，收到0xDE/EE/FE时，设置切换标志位*/
                if( MBB_USB_FALSE  == g_nv_autorun)
                {
...
USB_INT usb_get_config_descripts_number(USB_VOID)
{
    struct android_dev* dev = android_get_android_dev();
    if(dev && dev->cdev && dev->cdev->config)
    {
        return (dev->cdev->config->bConfigurationValue);
    }
    else
    {
        return -1; //PC Not begin enumeration
    }
}
#endif

#if(FEATURE_ON == MBB_USB_FTEN_SWITCH)
USB_INT get_ften_enum_flag(USB_VOID)
{
    return g_ften_usb_enum_flag;
}
USB_VOID set_ften_enum_flag(USB_INT flag)
{
    g_ften_usb_enum_flag = flag;
}
USB_INT get_ften_desc_flag(USB_VOID)
{
    return g_ften_usb_desc_flag;
}
USB_VOID set_ften_desc_flag(USB_INT flag)
{
    g_ften_usb_desc_flag = flag;
}
USB_INT get_ften_cur_usb(USB_VOID)
{
    return g_ften_cur_usb;
}
USB_VOID set_ften_cur_usb(USB_INT sel)
{
    g_ften_cur_usb = sel;
}
USB_INT get_ften_supply_type(USB_VOID)
{
    return g_type_of_supply;
}
USB_VOID set_ften_supply_type(USB_INT sel)
{
    g_type_of_supply = sel;
}
USB_INT is_ften_custom_usb(USB_VOID)
{
#if(FEATURE_ON == MBB_FEATURE_BOX_FTEN)
return FTEN_USB_TRUE;
#else
return FTEN_USB_FALSE;
#endif
}
