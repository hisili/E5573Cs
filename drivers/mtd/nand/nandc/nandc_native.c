...
u32 nandc_native_get_spec(struct nandc_host* host, u8* id_data, struct nandc_spec *spec)
{
    ...
    if (!type->pagesize)
    {
        ...
        */
        ...
        /* Nanya, 4k, 256Byte, 256K, 8bit 物料编码:40060683*/
        else if((id_data[0] == NAND_MFR_NANYA) && ((id_data[1] == 0xac) || (id_data[1] == 0xdc))
                 && ((id_data[2] == 0x90)) && ((id_data[3] == 0x26)) && ((id_data[4] == 0x76)))
        {
            /* Calc pagesize */
            writesize = NANDC_SIZE_1K << (extid & 0x03);   /*4K*/
            extid >>= 2;

            /* Calc oobsize */
            oobsize = (8 << (extid & 0x03)) * (writesize >> 8);  /*256BYTES*/
            extid >>= 2;

            /* Calc blocksize. Blocksize is multiples of 64KiB */
            erasesize = ((NANDC_BLOCK_64PAGE * NANDC_SIZE_1K) << (extid & 0x03)); /*64 * 4096*/
        }
		else if((id_data[0] == NAND_MFR_WINBOND) && ((id_data[1] == 0xa1) || (id_data[1] == 0xb1))
                 && ((id_data[2] == 0x80)) && ((id_data[3] == 0x15) || (id_data[3] == 0x55)) && ((id_data[4] == 0x00)))
        {
            /* Calc pagesize */
            writesize = NANDC_SIZE_2K;   /*2K*/
            /* Calc oobsize */
            oobsize = NANDC_SPARE_LENGTH_64BYTE;
            /* Calc blocksize. Blocksize is multiples of 128KiB */
            erasesize = NANDC_BLOCK_64PAGE * NANDC_SIZE_2K; 
        }     
        else
        {
            /* Calc pagesize */
    ...
    if ((cellinfo & NAND_CI_CELLTYPE_MSK) && (maf_id == NAND_MFR_SAMSUNG || maf_id == NAND_MFR_HYNIX))
    {
        host->bbt_options = NAND_BBT_SCANLASTPAGE;
                ...
                (maf_id == NAND_MFR_SAMSUNG || maf_id == NAND_MFR_HYNIX || maf_id == NAND_MFR_TOSHIBA ||
                 maf_id == NAND_MFR_AMD     || maf_id == NAND_MFR_MACRONIX || maf_id == NAND_MFR_MICRON || maf_id == NAND_MFR_NANYA || maf_id == NAND_MFR_WINBOND))
    {
