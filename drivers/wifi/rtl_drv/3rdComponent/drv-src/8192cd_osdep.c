...
static int rtl8192cd_init_sw(struct rtl8192cd_priv *priv)
{
    ...
    if (priv->pshare->rf_ft_var.edca_turbo)
    {
        ...
        priv->pshare->rf_ft_var.enable_1cca = 0;
    #endif /*CONFIG_1RCCA_RF_POWER_SAVING*/
        priv->pshare->rf_ft_var.tx_pwr_ctrl = 0;
        priv->pmib->dot11nConfigEntry.dot11nAMPDUSendSz = 8;
    }
#ifdef BEAMFORMING_SUPPORT
#ifdef UNIVERSAL_REPEATER
...
#endif
	{
        ...
        skb_queue_head_init(&priv->pshare->xmit_skb_queue); // SKB_TASK
...
#if defined(DOT11D) && defined(WLAN_PLATFORM_HUAWEI_COMMON)
...
#if defined(WLAN_PLATFORM_HUAWEI_COMMON) && defined(WIFI_SIMPLE_CONFIG)
	if (priv->pmib->wscEntry.wsc_ie_auto && (OPMODE & WIFI_AP_STATE)) 
    {
        DEBUG_INFO("WSC: auto set beacon IE\n");
        priv->pmib->wscEntry.beacon_ielen = build_beacon_wsc_ie(priv, priv->pmib->wscEntry.beacon_ie);
        DEBUG_INFO("WSC: auto set probe response IE\n");
        priv->pmib->wscEntry.probe_rsp_ielen = build_probe_rsp_wsc_ie(priv, priv->pmib->wscEntry.probe_rsp_ie);
...
//				if (i <tail || i >= head)
...
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
#ifdef MP_TEST
        printk("OPMODE = %p, skb_head/skb_tail=%d/%d\n", OPMODE, priv->pshare->skb_head, priv->pshare->skb_tail);
        if ((OPMODE & (WIFI_MP_STATE|WIFI_MP_CTX_BACKGROUND))==(WIFI_MP_STATE|WIFI_MP_CTX_BACKGROUND)) 
        {
            OPMODE_VAL(OPMODE & ~WIFI_MP_CTX_BACKGROUND);
            ...
            for (i = 0; i < NUM_MP_SKB; i++)
            {
                if (priv->pshare->skb_pool[i])
                {
                    if (priv->pshare->skb_pool[i]->head)
                    {
                        kfree(priv->pshare->skb_pool[i]->head);
                        priv->pshare->skb_pool[i]->head = NULL;
                    }
                    priv->pshare->skb_pool[i] = NULL;
                }
            }
            if (priv->pshare->skb_pool_ptr)
            {
                kfree(priv->pshare->skb_pool_ptr);
                priv->pshare->skb_pool_ptr = NULL;
            }
        }
#endif
	...
	rtw_flush_cmd_queue(priv, NULL);
    ...
    #endif
        {
            ...
            skb_queue_walk_safe(&priv->pshare->xmit_skb_queue, skb, tmp) 
            {
                __skb_unlink(skb, &priv->pshare->xmit_skb_queue);
...
int is_ps_cond_match(struct rtl8192cd_priv *priv)
{
    ...
    {
        printk("channel_select = %d, ss_req_ongoing = %d\n"
            , ((OPMODE & WIFI_WAIT_FOR_CHANNEL_SELECT) ? 1 : 0), priv->ss_req_ongoing);
        return 0;
    }
    ...
    {
        if ((NULL != vxd_priv) && netif_running(vxd_priv->dev))
        {
            printk("sta mode is running, scan = %d, opmode = %08x \n"
                , priv->pshare->bScanInProcess, GET_MIB(vxd_priv)->dot11OperationEntry.opmode);
            if ((WIFI_STATION_STATE != GET_MIB(vxd_priv)->dot11OperationEntry.opmode))
            {
                return 0;
            }
        }
...
void sdio_power_save_timer(unsigned long task_priv)
{
    ...
    unsigned long timer_expires = (jiffies + POWER_DOWN_T0);
	        
    /* ¨®DD¡ìD??¨¬2¨¦ */
    if (!IS_DRV_OPEN(priv) || (NULL == priv->pshare))
    {
        printk("Error, root = %p or netif_running = %d\n", priv, ((NULL == priv) ? 0 : netif_running(priv->dev)));
        return;
    }
    if ((TRUE == priv->pshare->bSurpriseRemoved) || (TRUE == priv->pshare->bDriverStopped))
    {
        printk("Error, bSurpriseRemoved = %d, bDriverStopped = %d\n"
            , priv->pshare->bSurpriseRemoved, priv->pshare->bDriverStopped);
        return;
    }
    printk("ps_disable = %d, total_num = %d, pwr_state = %d, offload_ctl = %d\n"
        , priv->pshare->dis_ps_mode, priv->pshare->total_assoc_num
        , priv->pshare->pwr_state, priv->pshare->offload_function_ctrl);

    if (!priv->pshare->dis_ps_mode /* power save enable */
        || (RTW_STS_WAKEUP == priv->pshare->pwr_state) /* ??D?2¨´¡Á¡Â */
        ) 
    {
        switch (priv->pshare->pwr_state)     
        {
            case RTW_STS_NORMAL:
            {
                if (is_ps_cond_match(priv))
                {
                    ...
                    {
                        priv->pshare->offload_function_ctrl = RTW_BEACON_DLOAD;
                    }
                    printk("starting offload, offload_function_ctrl = %d\n", priv->pshare->offload_function_ctrl);
                }
                break;
            }
            case RTW_STS_SUSPEND:
            {
                if (RTW_BEACON_NORMAL == priv->pshare->offload_function_ctrl)
                {
    ...
    if (timer_expires > 0)
    {
        mod_timer(&priv->pshare->ps_timer, timer_expires);
    }
}
...
{
    struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *) dev_instance;
    int ret = 0;
    
    if (NULL == priv)
    {
        printk("Error, priv is NULL\n");
        return IRQ_NONE;
    }

    printk("WiFi WAKEUP GPIO irq = %d, offload_ctl = %d, pwr_state = %d\n"
        , irq, priv->pshare->offload_function_ctrl, priv->pshare->pwr_state);
...
//		priv->ps_ctrl = 0x11;
        ...
        priv->pshare->autoch_to = SS_AUTOCHANNEL_TO;
        init_timer(&priv->pshare->autoch_timer);
        #if defined(CONFIG_PCI_HCI)
            priv->pshare->autoch_timer.data = (unsigned long) priv;
            priv->pshare->autoch_timer.function = rtl8192cd_autoch_timer;
        #elif defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
            priv->pshare->autoch_timer.data = (unsigned long) &priv->pshare->autoch_timer_event;
            priv->pshare->autoch_timer.function = timer_event_timer_fn;
            INIT_TIMER_EVENT_ENTRY(&priv->pshare->autoch_timer_event, rtl8192cd_autoch_timer, (unsigned long)priv);
        #endif
...
//	if (IS_VXD_INTERFACE(priv) && !GET_ROOT_PRIV(priv)->pmib->miscEntry.func_off) 
...
#endif
	{
        ...
        priv->dev->irq = 0;
            ...
            priv->dev->irq = wlan_request_wakeup_irq(rtl_wakeup_irq_handle, priv->dev->name, priv);
        #endif /* WLAN_PLATFORM_HUAWEI_COMMON */
        printk("dev_irq = %d\n", priv->dev->irq);
    #endif /*SDIO_AP_PS*/
	}
...
*/
...
#endif
	{
        ...
        if (0 != priv->dev->irq)
        {
            printk("remove dev_irq = %d\n", priv->dev->irq);
        #ifdef WLAN_PLATFORM_HUAWEI_COMMON
            wlan_free_irq(priv->dev->irq, priv);
        #else
            free_irq(priv->dev->irq, priv);
        #endif /*WLAN_PLATFORM_HUAWEI_COMMON*/
            priv->dev->irq = 0;
        }
...
#endif // __KERNEL__
{
    ...
    pmib = (struct wifi_mib *)((NULL == wdev->priv) ? NULL : rtl_cache_get(RTL_CACHE_MIB, (vap_idx + 1), sizeof(struct wifi_mib)));
    if (NULL == pmib)
...
#if defined(CONFIG_MEM_USE_CACHE)
    pevent_queue = (DOT11_QUEUE *)rtl_cache_get(RTL_CACHE_DOT11q
                        , ((NULL == wdev->priv) ? 0 : (vap_idx + 2)), sizeof(DOT11_QUEUE));
    if (NULL == pevent_queue)
#endif /* CONFIG_MEM_USE_CACHE */
...
#else
    #if defined(CONFIG_MEM_USE_CACHE)
        pshare = (struct priv_shared_info *)rtl_cache_get(RTL_CACHE_SHARED, 0, sizeof(struct priv_shared_info));
        if (NULL == pshare)
    #endif /* CONFIG_MEM_USE_CACHE */
...
#else
    #if defined(CONFIG_MEM_USE_CACHE)
        pwlan_hdr_poll = (struct wlan_hdr_poll *)rtl_cache_get(RTL_CACHE_SHARED, 1, sizeof(struct wlan_hdr_poll));
        if (NULL == pwlan_hdr_poll)
    #endif /* CONFIG_MEM_USE_CACHE */
...
#else
    #if defined(CONFIG_MEM_USE_CACHE)
        pwlanllc_hdr_poll = (struct wlanllc_hdr_poll *)rtl_cache_get(RTL_CACHE_SHARED, 2, sizeof(struct wlanllc_hdr_poll));
        if (NULL == pwlanllc_hdr_poll)
    #endif /* CONFIG_MEM_USE_CACHE */
...
#else
    #if defined(CONFIG_MEM_USE_CACHE)
        pwlanbuf_poll = (struct wlanbuf_poll *)rtl_cache_get(RTL_CACHE_SHARED, 3, sizeof(struct wlanbuf_poll));
        if (NULL == pwlanbuf_poll)
    #endif /* CONFIG_MEM_USE_CACHE */
...
#else
    #if defined(CONFIG_MEM_USE_CACHE)
        pwlanicv_poll = (struct wlanicv_poll *)rtl_cache_get(RTL_CACHE_SHARED, 4, sizeof(struct wlanicv_poll));
        if (NULL == pwlanicv_poll)
    #endif /* CONFIG_MEM_USE_CACHE */
...
#else
    #if defined(CONFIG_MEM_USE_CACHE)
        pwlanmic_poll = (struct wlanmic_poll *)rtl_cache_get(RTL_CACHE_SHARED, 5, sizeof(struct wlanmic_poll));
        if (NULL == pwlanmic_poll)
    #endif /* CONFIG_MEM_USE_CACHE */
...
#elif  defined(WLAN_PLATFORM_HUAWEI_COMMON)
            sprintf(dev->name, "wlan%d", (vap_idx + 1));
#else
...
register_driver:
    ...
    printk("=====>>EXIT rtl8192cd_init_one (%s) <<=====\n", priv->dev->name);
...
err_out_free:
	...
	if (pmib){
#if defined(CONFIG_MEM_USE_CACHE)
    if (!rtl_cache_check(RTL_CACHE_MIB, pmib))
#endif /* CONFIG_MEM_USE_CACHE */
#ifdef RTL8192CD_VARIABLE_USED_DMEM
    ...
    {
    #if defined(CONFIG_MEM_USE_CACHE)
        if (!rtl_cache_check(RTL_CACHE_DOT11q, pevent_queue))
    #endif /* CONFIG_MEM_USE_CACHE */
        kfree(pevent_queue);
    }
#ifdef CONFIG_RTL_WAPI_SUPPORT
    ...
    {
    #if defined(CONFIG_MEM_USE_CACHE)
        if (!rtl_cache_check(RTL_CACHE_SHARED, pshare))
    #endif /* CONFIG_MEM_USE_CACHE */
        kfree(pshare);
    }
    if (pwlan_hdr_poll)
    {
    #if defined(CONFIG_MEM_USE_CACHE)
        if (!rtl_cache_check(RTL_CACHE_SHARED, pwlan_hdr_poll))
    #endif /* CONFIG_MEM_USE_CACHE */
    ...
    {
    #if defined(CONFIG_MEM_USE_CACHE)
        if (!rtl_cache_check(RTL_CACHE_SHARED, pwlanllc_hdr_poll))
    #endif /* CONFIG_MEM_USE_CACHE */
    ...
    {
    #if defined(CONFIG_MEM_USE_CACHE)
        if (!rtl_cache_check(RTL_CACHE_SHARED, pwlanbuf_poll))
    #endif /* CONFIG_MEM_USE_CACHE */
        kfree(pwlanbuf_poll); 
    }
    if (pwlanicv_poll)
    {
    #if defined(CONFIG_MEM_USE_CACHE)
        if (!rtl_cache_check(RTL_CACHE_SHARED, pwlanicv_poll))
    #endif /* CONFIG_MEM_USE_CACHE */
        kfree(pwlanicv_poll);
    }
    if (pwlanmic_poll)
    {
    #if defined(CONFIG_MEM_USE_CACHE)
        if (!rtl_cache_check(RTL_CACHE_SHARED, pwlanmic_poll))
    #endif /* CONFIG_MEM_USE_CACHE */
        kfree(pwlanmic_poll);
    }
    ...
    printk("=====>>EXIT, Failed! wdev = %p, vap_idx = %p, (rc = %d) <<=====\n", wdev, vap_idx, rc);
...
void rtl8192cd_deinit_one(struct rtl8192cd_priv *priv)
{
...
#if defined(CONFIG_MEM_USE_CACHE)
    if (!rtl_cache_check(RTL_CACHE_MIB, vxd_priv->pmib))
#endif /* CONFIG_MEM_USE_CACHE */
#ifdef RTL8192CD_VARIABLE_USED_DMEM
...
#if defined(CONFIG_MEM_USE_CACHE)
    if (!rtl_cache_check(RTL_CACHE_DOT11q, priv->pevent_queue))
#endif /* CONFIG_MEM_USE_CACHE */
	...
	for (i = 0; i < RTL8192CD_NUM_VWLAN; i++) {
        ...
        #if defined(CONFIG_MEM_USE_CACHE)
            if (!rtl_cache_check(RTL_CACHE_MIB, vap_priv->pmib))
        #endif /* CONFIG_MEM_USE_CACHE */
#ifdef RTL8192CD_VARIABLE_USED_DMEM
...
#if defined(CONFIG_MEM_USE_CACHE)
    if (!rtl_cache_check(RTL_CACHE_MIB, priv->pmib))
#endif /* CONFIG_MEM_USE_CACHE */
#ifdef RTL8192CD_VARIABLE_USED_DMEM
...
#ifndef PRIV_STA_BUF
#if defined(CONFIG_MEM_USE_CACHE)
    if (!rtl_cache_check(RTL_CACHE_SHARED, priv->pshare->pwlan_hdr_poll))
#endif /* CONFIG_MEM_USE_CACHE */
	kfree(priv->pshare->pwlan_hdr_poll);

#if defined(CONFIG_MEM_USE_CACHE)
    if (!rtl_cache_check(RTL_CACHE_SHARED, priv->pshare->pwlanllc_hdr_poll))
#endif /* CONFIG_MEM_USE_CACHE */
	kfree(priv->pshare->pwlanllc_hdr_poll);

#if defined(CONFIG_MEM_USE_CACHE)
    if (!rtl_cache_check(RTL_CACHE_SHARED, priv->pshare->pwlanbuf_poll))
#endif /* CONFIG_MEM_USE_CACHE */
	kfree(priv->pshare->pwlanbuf_poll);

#if defined(CONFIG_MEM_USE_CACHE)
    if (!rtl_cache_check(RTL_CACHE_SHARED, priv->pshare->pwlanicv_poll))
#endif /* CONFIG_MEM_USE_CACHE */
	kfree(priv->pshare->pwlanicv_poll);

#if defined(CONFIG_MEM_USE_CACHE)
    if (!rtl_cache_check(RTL_CACHE_SHARED, priv->pshare->pwlanmic_poll))
#endif /* CONFIG_MEM_USE_CACHE */
...
#if defined(CONFIG_MEM_USE_CACHE)
    if (!rtl_cache_check(RTL_CACHE_SHARED, priv->pshare))
#endif /* CONFIG_MEM_USE_CACHE */
