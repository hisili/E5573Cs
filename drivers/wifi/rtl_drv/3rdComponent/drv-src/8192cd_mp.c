...
#endif
			{
                if (HT_CHANNEL_WIDTH_20_40 == priv->pshare->CurrentChannelBW)
                    val_read = 0x7400;
                else
                    val_read = 0x7c00;
...
void dump_mp_rx_tx(struct rtl8192cd_priv *priv)
{
    unsigned int val_read = 0;
    val_read = PHY_QueryRFReg(priv, 0, 0x0, bMask20Bits, 1);
    printk("RF 0x0=0x%X, skb_head/skb_tail=%d/%d\n", val_read, priv->pshare->skb_head, priv->pshare->skb_tail);
    
    printk("%d%%  rx %s%d (ss %d %d )(FA %d)(CCA %d)\n",
            priv->pshare->mp_rssi,
    ...
    printk("Tx OK:%d, Tx Fail:%d, Rx OK:%lu, CRC error:%lu\n",
...
void mp_ctx(struct rtl8192cd_priv *priv, unsigned char *data)
{
    ...
    static unsigned long s_tx_pkt_count = 0;
    static unsigned long s_start_jiffies = 0;
    static unsigned long s_dump_info_jiffies = 0;
/*
...
*/
    ...
    if (val)
    {
        if (OPMODE & WIFI_MP_CTX_BACKGROUND)
        {
            if (((OPMODE & WIFI_MP_CTX_ST)
                || !(OPMODE & (WIFI_MP_CTX_PACKET | WIFI_MP_CTX_CCK_CS))) && (priv->net_stats.tx_packets > 0))
            {
                printk("Error, for OPMODE = %p!", OPMODE);
                return;
            }
    ...
    if (background) 
    {
        ...
        for (i = 0; i < NUM_MP_SKB; i++)
        {
            ...
            if (NULL == priv->pshare->skb_pool[i]->head)
            {
                printk("Allocate pshare->skb_pool[%d]->head Fail!\n", i);
                priv->pshare->skb_pool[i] = NULL;
                for (i = 0; i < NUM_MP_SKB; i++)
                {
                    if (priv->pshare->skb_pool[i])
        ...
        i = 0;
        printk("free_xmit_queue = %d, free_xmitbuf_queue = %d"
            , priv->pshare->free_xmit_queue.qlen, priv->pshare->free_xmitbuf_queue.qlen);
    ...
    if (!tx_from_isr)
    {
     ...
     if (background) 
     {
        ...
        s_dump_count = 0;
        s_tx_pkt_count = 0;
        s_start_jiffies = jiffies;
        s_dump_info_jiffies = jiffies + HZ; /* ?a那?∩辰車?dump那㊣?? */
        dump_mp_rx_tx(priv);
	...
	while (1)
	{
        ...
        else if (CIRC_SPACE(priv->pshare->skb_head, priv->pshare->skb_tail, NUM_MP_SKB) <= 0)
        {
        ...
        else
        {
            ...
            congestion_handle:
                ...
                #endif // CONFIG_PCI_HCI
                    SMP_UNLOCK(flags);
                    printk("tx_from_isr = %d", tx_from_isr);
                    return;
                }
                else
                {
            ...
            else if ((1 == priv->net_stats.tx_packets) && (WIFI_MP_CTX_BACKGROUND
                     == (OPMODE & (WIFI_MP_CTX_PACKET | WIFI_MP_CTX_BACKGROUND | WIFI_MP_CTX_ST | WIFI_MP_CTX_CCK_CS)))
                    )
            {
            #define CHECK_TX_MODE_CNT   40
                ...
                {
                    check_cnt = 0;
                    while ((check_cnt < CHECK_TX_MODE_CNT)
                           && (0x02 != PHY_QueryRFReg(priv, RF92CD_PATH_A, 0x00, 0x70000, 1))
                          )
                    {
                        delay_ms(1);
                        ++check_cnt;
                ...
                if ( priv->pshare->mp_antenna_tx & ANTENNA_B )
                {
                    check_cnt = 0;
                    while ((check_cnt < CHECK_TX_MODE_CNT)
                           && (0x02 != PHY_QueryRFReg(priv, RF92CD_PATH_B, 0x00, 0x70000, 1))
                          )
                    {
                        delay_ms(1);
                        ++check_cnt;
            ...
            {
                if (time_after(jiffies, s_dump_info_jiffies))
                {
                    dump_mp_rx_tx(priv);

                    s_dump_count++;
                    s_dump_info_jiffies = jiffies + HZ;
                    if (s_dump_count == 4)                     {
                        reg_dump(priv, "all");
                    }
                }
...
*/
    ...
    if (OPMODE & WIFI_MP_CTX_BACKGROUND)
    {
        unsigned long  loop_jiffies_timeout = 0;
        loop_jiffies_timeout = jiffies + msecs_to_jiffies(30000); /* ?-?﹞??DD米?℅?∩車3?那㊣那㊣3∟ */
        s_tx_pkt_count -= ((priv->pshare->skb_tail + NUM_MP_SKB - priv->pshare->skb_head) & (NUM_MP_SKB - 1));

        printk("Stop continuous TX, OPMODE = %p, skb_head = %d, skb_tail=%d, free_xmit_qlen = %d, free_xmitbuf_qlen = %d" \
            ", total_pkt = %d, total_time = %d(ms)\n"
            , OPMODE, priv->pshare->skb_head, priv->pshare->skb_tail
            , priv->pshare->free_xmit_queue.qlen, priv->pshare->free_xmitbuf_queue.qlen
            , s_tx_pkt_count, jiffies_to_msecs(jiffies - s_start_jiffies)
        );
		SAVE_INT_AND_CLI(flags);
		SMP_LOCK(flags);
        ...
        while (priv->pshare->skb_head != priv->pshare->skb_tail)
        {
            if (time_after(jiffies, loop_jiffies_timeout))
            {
                printk("Exit, loop after check timeout\n");
                break;
            }
            if (((++i) % 10) == 0)
            {
                printk("OPMODE = %p, skb_head = %d, skb_tail=%d, free_xmit_qlen = %d, free_xmitbuf_qlen = %d\n"
                    , OPMODE, priv->pshare->skb_head, priv->pshare->skb_tail
                    , priv->pshare->free_xmit_queue.qlen, priv->pshare->free_xmitbuf_queue.qlen
                );
        ...
        printk("Exit, OPMODE = %p, skb_head = %d, skb_tail=%d, free_xmit_qlen = %d, free_xmitbuf_qlen = %d\n"
            , OPMODE, priv->pshare->skb_head, priv->pshare->skb_tail
            , priv->pshare->free_xmit_queue.qlen, priv->pshare->free_xmitbuf_queue.qlen
        );
    }
...
//	RTL_W32(RCR, _ENMARP_ | _NO_ERLYRX_ | _RX_DMA64_);
    ...
    snprintf(data, data_len, "Rx reseult: ok=%ld,lost=%ld,elapsed time=%ld",
...
//clear filter:
        ...
        /* BEGEN  DTS2014061606613  lifeng WX199172   modified */
        if (0x00 == priv->pshare->mp_filter_SA[0] && 0x00 == priv->pshare->mp_filter_SA[1] 
            && 0x00 == priv->pshare->mp_filter_SA[2] && 0x00 == priv->pshare->mp_filter_SA[3] 
            && 0x00 == priv->pshare->mp_filter_SA[4] && 0x00 == priv->pshare->mp_filter_SA[5])
        {
            OPMODE_VAL(OPMODE & (~WIFI_MP_ARX_FILTER));
        }
        else
        ...
        /* END  DTS2014061606613  lifeng WX199172   modified */
