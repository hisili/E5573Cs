...
#define EXTERN
    ...
    EXTERN void __rtl_cache_free(const char *func, int line);
    EXTERN void * __rtl_cache_get(const char *func, int line
        , rtl_cache_type cache_type, unsigned int cur_idx, unsigned int buf_len);
    EXTERN int __rtl_cache_check(const char *func, int line, rtl_cache_type cache_type, void *buf);
    EXTERN int __rtl_mem_in_cache(const char *func, int line, void *buf);
    #define rtl_cache_free()                    __rtl_cache_free(__FUNCTION__, __LINE__)
    #define rtl_cache_get(type, idx, len)     __rtl_cache_get(__FUNCTION__, __LINE__, type, idx, len)
    #define rtl_cache_check(type, buf)   __rtl_cache_check(__FUNCTION__, __LINE__, type, buf)
    #define rtl_mem_in_cache(buf)  __rtl_mem_in_cache(__FUNCTION__, __LINE__, buf)
...
EXTERN struct channel_list *get_channel_list(int is_5g, int region);
...
#endif
EXTERN __MIPS16 __IRAM_IN_865X unsigned int get_skb_priority(struct rtl8192cd_priv *priv, struct sk_buff *skb);
EXTERN unsigned int get_tx_rate(struct rtl8192cd_priv *priv, struct stat_info *pstat);
...
EXTERN void rtl8192cd_autoch_timer(unsigned long task_priv);
#endif
...
EXTERN int build_beacon_wsc_ie(struct rtl8192cd_priv *priv, unsigned char *data);
EXTERN int build_probe_rsp_wsc_ie(struct rtl8192cd_priv *priv, unsigned char *data);
#endif
...
#ifdef CONFIG_RTL_FLOW_CTRL
    int check_rx_flow_ctrl(struct rtl8192cd_priv *priv);
#endif /*CONFIG_RTL_FLOW_CTRL*/
...
__MIPS16
    ...
    void fastip_attach_wifi(struct rtl8192cd_priv *priv);
    void fastip_detach_wifi(struct rtl8192cd_priv *priv);
...
#ifdef WLAN_PLATFORM_HUAWEI_COMMON
EXTERN void set_offload_wakeup_pin(struct rtl8192cd_priv *priv, int is_high);
#endif
...
EXTERN int is_ps_cond_match(struct rtl8192cd_priv *priv);
#endif
...
#endif
#ifdef WLAN_PLATFORM_HUAWEI_COMMON
EXTERN int get_country_channel(char *country, char *buf, int buf_len);
#endif
...
EXTERN int rtk_start_simple_config(struct rtl8192cd_priv *priv);
EXTERN int rtk_start_parse_packet(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo);
EXTERN int check_simple_config_security(struct rtl8192cd_priv *priv, struct bss_desc *bss);
EXTERN int rtk_stop_simple_config(struct rtl8192cd_priv *priv);
EXTERN int rtk_start_connect_target(struct rtl8192cd_priv *priv);
EXTERN int rtk_sc_sync_vxd_to_root(struct rtl8192cd_priv * priv);
EXTERN int rtk_clean_profile_value();
#undef EXTERN
