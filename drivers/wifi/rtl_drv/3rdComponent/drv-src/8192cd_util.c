...
/* 引用wlan_if模块申请到的连续内存 */
extern wlan_mem_cache_st g_wlan_mem_cache;
#endif
...
struct channel_list *get_channel_list(int is_5g, int region)
{
    int list_size = 0;
    struct channel_list *list_chan = NULL;
    if (is_5g)
    {
        list_chan = reg_channel_5g_full_band;
        list_size = sizeof(reg_channel_5g_full_band) / sizeof(reg_channel_5g_full_band[0]);
    }
    else
    {
        list_chan = reg_channel_2_4g;
        list_size = sizeof(reg_channel_2_4g) / sizeof(reg_channel_2_4g[0]);
    }
    
    region--;
    if (region < 0 || region >= list_size)
    {
        return NULL;
    }
    else
    {
        return &(list_chan[region]);
    }
}
			...
			if (pstat->ht_cap_len) {
                    ...
                    (priv->pmib->dot11BssType.net_work_type & (WIRELESS_11N | WIRELESS_11G)))
                {
                    if (pstat->ht_cap_buf.ht_cap_info & cpu_to_le16(_HTCAP_40M_INTOLERANT_))
                    {
                        if (OPMODE & WIFI_AP_STATE)
                        {
                            unsigned int force_20_sta_old = priv->force_20_sta;
                        #ifdef STA_EXT
                            if (pstat->aid <= FW_NUM_STAT)
...
#ifdef WLAN_EVENT_WPA
    if (!IS_VXD_INTERFACE(priv) && !(priv->pmib->dot1180211AuthEntry.dot11EnablePSK && act))
#else
    if (!IS_VXD_INTERFACE(priv))
#endif
    {
        WLAN_USER_EVENT user_evt = {USER_WIFI_UPDATE_STA_LIST, 0};
        (void)wlan_event_report(&user_evt);
    }
#endif /* WLAN_PLATFORM_HUAWEI_COMMON */
    printk("root_priv = %d, funt = %s, assoc_num%s(%d) in %02X%02X%02X%02X%02X%02X\n"
        , IS_ROOT_INTERFACE(priv), ((NULL == func) ? "NULL" : func)
        , (act ? "++" : "--")
        , priv->assoc_num
...
void event_indicate(struct rtl8192cd_priv *priv, unsigned char *mac, int event)
{
...
#ifdef LINUX_2_6_27_
	{
        ...
        if (!DOT11_IsEmptyQueue(priv->pevent_queue))
    #endif /*WLAN_PLATFORM_HUAWEI_COMMON*/
...
void update_intel_sta_bitmap(struct rtl8192cd_priv *priv, struct stat_info *pstat, int release)
{
    ...
    if (release)
    {
        ...
        if (pstat->aid > FW_NUM_STAT)
        {
            priv->pshare->intel_sta_bitmap_ext &= ~BIT(pstat->aid - FW_NUM_STAT - 1);
        }
        else
    #endif
    #ifdef CONFIG_RTL_88E_SUPPORT
        if ((GET_CHIP_VER(priv) == VERSION_8188E) && (pstat->aid > 32))
        {
            priv->pshare->intel_sta_bitmap_88e_ext &= ~BIT(pstat->aid - 32 - 1);
        }
        else
    #endif
            priv->pshare->intel_sta_bitmap &= ~BIT(pstat->aid - 1);
    } 
    else 
    {   // join
    #ifdef STA_EXT
...
void checkBandwidth(struct rtl8192cd_priv *priv)
{
    ...
    _40m_stamap = priv->pshare->_40m_staMap;
#ifdef STA_EXT
    _40m_stamap |= priv->pshare->_40m_staMap_ext;
#endif
	...
	if( priv->pmib->dot11nConfigEntry.dot11nCoexist) {
        u1Byte IGI = RTL_R8(0xc50) & 0x7f; /* 获取检测干扰寄存器的值 */
        unsigned int intel_sta_bitmap = 0;

        intel_sta_bitmap = priv->pshare->intel_sta_bitmap;
#ifdef CONFIG_RTL_88E_SUPPORT
        intel_sta_bitmap |= priv->pshare->intel_sta_bitmap_88e_ext;
#endif
#ifdef STA_EXT
        intel_sta_bitmap |= priv->pshare->intel_sta_bitmap_ext;
#endif
...
#endif
            && (IGI >= 0x30)) /* 根据抗干扰寄存器调整带宽选择门限 */
        {
            switch_20m = 1;
        }
        else if (((FA_counter > priv->pshare->rf_ft_var.bws_Thd) && (IGI >= 0x32))
    #if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
            || (IGI >= 0x42) /* 根据抗干扰寄存器调整带宽选择门限 */
    #endif
        )
        {
        ...
        } else if(_40m_stamap && (!priv->ss_req_ongoing) && (priv->pshare->CurrentChannelBW == HT_CHANNEL_WIDTH_20) &&
    ...
    if (do_switch) {
        printk("switch band CurrentChannelBW %d", priv->pshare->CurrentChannelBW);
...
int set_ap_ps_mode(struct rtl8192cd_priv *priv, unsigned char *data)
{
    ...
    priv->pshare->dis_ps_mode = _atoi(data, 10);
    /* BEGIN PN: DTS2014050608052,Modified by zwx179749, 2014/05/07*/
    /* 处于自动发beacon状态则唤醒 */
    if (priv->pshare->dis_ps_mode)
    {
        SDIO_AP_WAKEUP(priv);
    }
...
void txrpt_handler_8812(struct rtl8192cd_priv *priv, struct tx_rpt *report, struct stat_info	*pstat)
{
	...
	if(!pstat)
		return;
#ifdef STA_EXT
	if( pstat->aid && (pstat->aid < RTL8812_NUM_STAT-1))	
#else
...
void RF_MIMO_check_timer(unsigned long task_priv)
{
    ...
    if ((rf_mimo_mode != MIMO_2T2R) && (rf_mimo_mode != MIMO_1T2R))
        return;
...
#define dprintf printk
	...
	if (mode == 0x01)	{
        ...
        MIMO_mode_switch(priv, 5);
        priv->pshare->rf_ft_var.rf_mode = 1;
    } else if (mode == 0x06)         {
        MIMO_mode_switch(priv, 6);
        priv->pshare->rf_ft_var.rf_mode = 1;
	} else {
...
int rtk_start_simple_config(struct rtl8192cd_priv *priv)
{
	int i=0;
	if(priv->pmib->dot11StationConfigEntry.sc_debug)
		panic_printk("Start simple config now!\n");
	rtk_clean_profile_value();
	if(netif_running(priv->dev))// && (priv->simple_config_status == 0))
	{
#ifdef UNIVERSAL_REPEATER	
		if(IS_VXD_INTERFACE(priv))
		{
			...
			mod_timer(&priv->ss_timer, jiffies + 1); 			   
		}
#endif
	}
...
int rtk_start_connect_target(struct rtl8192cd_priv *priv)
{
	int i=0;
#ifdef UNIVERSAL_REPEATER		
	if(IS_VXD_INTERFACE(priv))
...
int rtk_sc_sync_vxd_to_root(struct rtl8192cd_priv * priv)
{
...
#if defined(CONFIG_MEM_USE_CACHE)
    /* *******************缓存数据存储区域定义*************************** */
    typedef struct _rtl_mem_cache_
    {
        // mib 
        void *mib[2];       /* wlan0/wlan1/WiFi0 - RTL8192CD_SPEEDUP_PRIV_COUNT */
        
        // shared
        void *shared[6];    /* pshared/pwlan_hdr_poll/pwlanllc_hdr_poll/pwlanllc_hdr_poll/pwlanicv_poll/pwlanmic_poll */

        void *dot11q[3];    /* wlan0/wlan1/WiFi0 priv->pevent_queue */
        void *recv[1];      /* 接收缓存, 已优化合并成一块 NR_RECVBUFF */
        void *xmit[16];     /* 发送缓存的最大块数 NR_XMITBUFF */
        void *cfgfile[4];   /* MAX_CONFIG_FILE_SIZE，备PHY_ConfigBBWithParaFile调用 */
    }rtl_mem_cache_st;
    static rtl_mem_cache_st g_rtl_cache = {0};

    /******************内存申请列表***************************/
    typedef struct _rtl_cache_item_
    {
        const rtl_cache_type type;
    ...
    #ifndef ARRAY_SIZE
        #define ARRAY_SIZE(x)  (sizeof(x) / sizeof(x[0])) /* 获取数组大小 */
    #endif
    #define SET_CACHE_ITEM(type, field)  {type, #field, &g_rtl_cache.field, ARRAY_SIZE(g_rtl_cache.field)}
    static rtl_cache_item_st g_rtl_cache_list[] =
    {
        SET_CACHE_ITEM(RTL_CACHE_MIB, mib),
        SET_CACHE_ITEM(RTL_CACHE_SHARED, shared),
        SET_CACHE_ITEM(RTL_CACHE_DOT11q, dot11q),
        SET_CACHE_ITEM(RTL_CACHE_RECV, recv),
        SET_CACHE_ITEM(RTL_CACHE_XMIT, xmit),
        SET_CACHE_ITEM(RTL_CACHE_CFGFILE, cfgfile),
    };

    /*****************************************************************************
     函数名称  : rtl_get_cache_item
     功能描述  : 获取缓存项
     输入参数  : cache_type: 缓存类型
     输出参数  : NA
     返 回 值  : rtl_cache_item_st: 缓存项
    *****************************************************************************/
    static __always_inline rtl_cache_item_st * rtl_get_cache_item(rtl_cache_type cache_type)
    {
         rtl_cache_item_st *pitem = NULL;
         for (pitem = g_rtl_cache_list; (pitem - g_rtl_cache_list) < ARRAY_SIZE(g_rtl_cache_list); pitem++)
         {
            if (cache_type == pitem->type)
            {
                return pitem;
            }
         }
    ...
    /*****************************************************************************
     函数名称  : rtl_cache_get
     功能描述  : 获取CACHE缓存
     输入参数  : rtl_cache_type: 缓存类型; buf_len: 缓存长度
     输出参数  : NA
     返 回 值  : void *: 指向缓存的指针
    *****************************************************************************/
    void * __rtl_cache_get(const char *func, int line
                , rtl_cache_type cache_type, unsigned int cur_idx, unsigned int buf_len)
    {
        rtl_cache_item_st *pitem = NULL;

        pitem = rtl_get_cache_item(cache_type);
        if (NULL == pitem)
        {
            rtl_printk(WLAN_LOG_DRV_LOW, func, line, "[error] cache_type %d\n" , cache_type);
            return NULL;
        }

        //索引检查
        if (cur_idx >= pitem->size)
        {
            rtl_printk(WLAN_LOG_DRV_LOW, func, line
                , "[error] (%s cache) cur_idx = %d out of array %d\n", pitem->name, cur_idx, pitem->size);
            return NULL;
        }

        if (RTL_CACHE_RECV == cache_type)
        {
            pitem->pbuf[cur_idx] = g_wlan_mem_cache.recv;
            rtl_printk(WLAN_LOG_DRV_LOW, func, line, "_rtl_cache_get cur_idx = %d, recv = %p \n", cur_idx, g_wlan_mem_cache.recv);
        }
        else if (RTL_CACHE_CFGFILE == cache_type)
        {
            pitem->pbuf[cur_idx] = g_wlan_mem_cache.cfgfile[cur_idx];
            rtl_printk(WLAN_LOG_DRV_LOW, func, line, "_rtl_cache_get cur_idx = %d , &cfgfile = %p \n", cur_idx, g_wlan_mem_cache.cfgfile[cur_idx]);
        }
        else
        {
            rtl_printk(WLAN_LOG_DRV_LOW, func, line, "_rtl_cache_get cache_type = %s \n", pitem->name);
        }

        //申请并初始化缓存
        if (NULL == pitem->pbuf[cur_idx])
        {
            pitem->pbuf[cur_idx] = kmalloc(buf_len, /*GFP_KERNEL*/GFP_ATOMIC);
            rtl_printk(WLAN_LOG_DRV_LOW, func, line
                , "[alloc] (%s cache) new = %p, cur_idx = %d of array %d, buf_len = %u\n"
                , pitem->name, pitem->pbuf[cur_idx], cur_idx, pitem->size, buf_len);
        }
        else
        {
            rtl_printk(WLAN_LOG_DRV_LOW, func, line
                , "[cache] (%s cache) mem = %p, cur_idx = %d of array %d, buf_len = %u\n"
                , pitem->name, pitem->pbuf[cur_idx], cur_idx, pitem->size, buf_len);
        }

        return pitem->pbuf[cur_idx];
    }
    /*****************************************************************************
     函数名称  : sdio_cache_free
     功能描述  : 释放CACHE缓存
     输入参数  : NA
     输出参数  : NA
     返 回 值  : NA
    *****************************************************************************/
    void __rtl_cache_free(const char *func, int line)
    {
        u32 idx = 0;
        rtl_cache_item_st *pitem = NULL;

        rtl_printk(WLAN_LOG_DRV_LOW, func, line, "[kfree] static cache\n");

        for (pitem = g_rtl_cache_list; (pitem - g_rtl_cache_list) < ARRAY_SIZE(g_rtl_cache_list); pitem++)
        {
            rtl_printk(WLAN_LOG_DRV_LOW, func, line, "_rtl_cache_free type = %s \n", pitem->name);
            for (idx = 0; idx < pitem->size; idx++)
            {
                if (NULL != pitem->pbuf[idx])
                {
                    if ((RTL_CACHE_RECV == pitem->type) || (RTL_CACHE_CFGFILE == pitem->type))
                    {
                        pitem->pbuf[idx] = NULL;
                        continue;
                    }
                    ...
                    {
                        printk("[kfree] (%s cache) mem = %p, cur_idx = %d of array %d\n"
                            , pitem->name, pitem->pbuf[idx], idx, pitem->size);
                        kfree(pitem->pbuf[idx]);
                        pitem->pbuf[idx] = NULL;
                    }
                }
            }
    ...
    /*****************************************************************************
     函数名称  : rtl_cache_check
     功能描述  : 检查申请的buffer是否属于CACHE缓存
     输入参数  : cache_type:缓存数据类型，buf:需要检查的缓存
     输出参数  : NA
     返 回 值  : BOOT
    *****************************************************************************/
    int __rtl_cache_check(const char *func, int line, rtl_cache_type cache_type, void *buf)
    {
        u16 idx = 0;
        rtl_cache_item_st *pitem = NULL;

        pitem = rtl_get_cache_item(cache_type);
        if ((NULL == buf) || (NULL == pitem))
        {
            rtl_printk(WLAN_LOG_DRV_LOW, func, line, "[error] cache_type %d, buf = %p\n" , cache_type, buf);
            return FALSE;
        }

        for (idx = 0; idx < pitem->size; idx++)
        {
            if (buf == pitem->pbuf[idx])
            {
                break;
            }
        }
        ...
        /*
        rtl_printk(WLAN_LOG_DRV_LOW, func, line
            , " [find] (%s cache) mem = %p, cur_idx = %d of array %d\n"
            , pitem->name, buf, idx, pitem->size);
        */

        return (idx != pitem->size);
    }
    /*****************************************************************************
     函数名称  : rtl_mem_in_cache
     功能描述  : 检查申请的buffer是否属于CACHE缓存
     输入参数  : x:需要检查的缓存地址
     输出参数  : NA
     返 回 值  : BOOT
    *****************************************************************************/
    int __rtl_mem_in_cache(const char *func, int line, void *buf)
    {
        u32 idx = 0;
        rtl_cache_item_st *pitem = NULL;
        for (pitem = g_rtl_cache_list; (pitem - g_rtl_cache_list) < ARRAY_SIZE(g_rtl_cache_list); pitem++)
