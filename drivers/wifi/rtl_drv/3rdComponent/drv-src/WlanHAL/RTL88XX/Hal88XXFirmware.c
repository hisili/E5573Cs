...
#endif			
			{
                if ((rf_mimo_mode == MIMO_1T1R) || (rf_mimo_mode == MIMO_1T2R))
				{
					if (bCurTxBW40MHz)
					{
						if(rssi_level == 1)
...
)
{
    ...
    printk("bcn: [%x] [%x] nr_ap=:%d\n", H2CCommand[0], H2CCommand[1], numOfAP);
}
...
)
{
   ...
   ret = FillH2CCmd88XX(Adapter, H2C_88XX_WAKEUP_PIN, 3, H2CCommand);
   
   printk("Exit, gpio wakeup=%x, ret = %d\n", H2CCommand[0], ret);
}
...
)
{
    ...
    printk("ap_offload = 0x%02x%02x%02x%02x\n", H2CCommand[2], H2CCommand[1], H2CCommand[0], H2C_88XX_AP_OFFLOAD);
