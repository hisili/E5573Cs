...
int DOT11_EnQueue(unsigned long task_priv, DOT11_QUEUE *q, unsigned char *item, int itemsize)
{
    ...
    #define DOT11_IS_VAILD_EVENT(type) ( \
            (type == DOT11_EVENT_WSC_ASSOC_REQ_IE_IND) \
        ||  (type == DOT11_EVENT_WSC_PIN_IND) \
        ||  (type == DOT11_EVENT_WSC_SET_MY_PIN) \
        ||  (type == DOT11_EVENT_WSC_SPEC_SSID) \
        ||  (type == DOT11_EVENT_WSC_SPEC_MAC_IND) \
        ||  (type == DOT11_EVENT_EAP_PACKET) \
        ||  (type == DOT11_EVENT_WSC_PROBE_REQ_IND) \
    )
    if (!DOT11_IS_VAILD_EVENT(item[0]))
    {
    ...
    if (DOT11_IsFullQueue(q)) 
    {
            ...
            DEBUG_ERR("Queue Full to Drop event %d\n", q->ItemArray[q->Tail].Item[0]);
        #endif /*0*/
    }
...
int rtl8192cd_ioctl_priv_daemonreq(struct net_device *dev, struct iw_point *data)
{
	...
	switch(req.EventId)
	{
        ...
        case DOT11_EVENT_REQUEST:
            ...
            {
                DOT11_NO_EVENT no_event = {DOT11_EVENT_NO_EVENT, 0};
                if (copy_to_user((void *)data->pointer, &no_event, sizeof(DOT11_NO_EVENT)))
                {
