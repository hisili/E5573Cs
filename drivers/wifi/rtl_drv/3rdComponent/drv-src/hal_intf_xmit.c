...
void rtw_handle_xmit_fail(struct rtl8192cd_priv *priv, struct tx_insn *txcfg)
{
    ...
    if (pxmitbuf)
    {
        ...
        if ((txcfg == pxmitbuf->agg_start_with) && pxmitbuf->use_hw_queue)
        {
            ...
            /* BEGIN PN: DTS2014121902376,Added by h00231374, 2015/01/24 */
	...
	if (txcfg->fr_type == _SKB_FRAME_TYPE_)
	{
		...
		if (txcfg->wdsIdx >= 0) {
			...
			if (txcfg->aggre_en < FG_AGGRE_MSDU_FIRST) {
				priority = get_skb_priority(priv, pskb);
			...
			)
		{
			...
			if (txcfg->aggre_en < FG_AGGRE_MSDU_FIRST) {
#ifdef CONFIG_RTK_MESH
				priority = get_skb_priority3(priv, pskb, txcfg->is_11s);
#else
				priority = get_skb_priority(priv, pskb);
#endif
#ifdef RTL_MANUAL_EDCA
...
void rtl8192cd_hal_fill_fwinfo(struct rtl8192cd_priv *priv, struct tx_insn* txcfg, struct tx_desc  *pdesc, unsigned int frag_idx)
{
    ...
    if (bRtsEnable && (!priv->pshare->rf_ft_var.edca_turbo))
    {
	...
	if (txcfg->need_ack) {
		...
		if (txcfg->q_num == MANAGE_QUE_NUM) {
			...
			if (priv->pshare->rf_ft_var.low_retry_rssi_thd && txcfg->pstat->rssi
                && (txcfg->pstat->rssi < priv->pshare->rf_ft_var.low_retry_rssi_thd)) /* ?¨´?YD?o????¨¨?¡¥¨¬?¦Ì¡Â????¡ä?¡ä?¨ºy */
            {
...
int rtl8192cd_signin_txdesc(struct rtl8192cd_priv *priv, struct tx_insn* txcfg, struct wlan_ethhdr_t *pethhdr)
{
    ...
    if (0 == txcfg->frg_num) 
    {
        ...
        mem_dump("[signin_txdesc]txcfg", (unsigned char*)txcfg, sizeof(struct tx_insn));
    }
...
int rtl8192cd_firetx(struct rtl8192cd_priv *priv, struct tx_insn* txcfg)
{
    ...
    {
        printk("0 == txcfg->frg_num\n");
        mem_dump("[firetx]txcfg", (unsigned char*)txcfg, sizeof(struct tx_insn));
        mem_dump("[firetx]phdr", (unsigned char*)txcfg->phdr, txcfg->hdr_len);
        dump_stack();
    }
    local_bh_disable();
