...
{							
		...
		if(pDM_Odm->SupportICType & (ODM_RTL8188E|ODM_RTL8192E|ODM_RTL8723B))
		{
			...
			if(pDM_Odm->SupportICType & (ODM_RTL8188E|ODM_RTL8192E))
			{
                if(pDM_Odm->cck_agc_report_type == 0 && (pDM_Odm->SupportICType & ODM_RTL8192E) )
                {
                    ...
                    {
                        case 7:
                            rx_pwr_all = pDM_Odm->rx_pwr_7  - 2*(VGA_idx); 
                            break;
                        case 6:
                            rx_pwr_all = pDM_Odm->rx_pwr_6 -2*(VGA_idx); 
                            break;
                        case 5:
                            rx_pwr_all = pDM_Odm->rx_pwr_5 - 2*(VGA_idx); 
                            break;
                        case 4:
                            rx_pwr_all = pDM_Odm->rx_pwr_4 - 2*(VGA_idx); 
                            break;
                        case 3:							
                          //  if (VGA_idx <21 )
                                 rx_pwr_all = pDM_Odm->rx_pwr_3 - 2*(VGA_idx); 
                         //   else 
                          //       rx_pwr_all = pDM_Odm->rx_pwr_3 - 2*(VGA_idx)-2; 
							
                            break;
                        case 2:
                            rx_pwr_all = pDM_Odm->rx_pwr_2 - 2*(VGA_idx);
                            break;
                        case 1:
                            rx_pwr_all = pDM_Odm->rx_pwr_1 -2*(VGA_idx);
                            break;
                        case 0:
                            rx_pwr_all = pDM_Odm->rx_pwr_0 -2*(VGA_idx);
                            break;
                        default:
	...
	else //2 is OFDM rate
	{
        ...
        if (pDM_Odm->SupportICType & (ODM_RTL8188E|ODM_RTL8192E))
        {
             rx_pwr_all += pDM_Odm->ofdmRssiOffset;
        }
