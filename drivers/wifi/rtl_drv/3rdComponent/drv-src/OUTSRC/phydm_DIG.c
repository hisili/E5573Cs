	...
	)
{
    ...
    if( pDM_Odm->BoardType & ODM_BOARD_EXT_LNA )
         pDM_DigTable->CurCCK_CCAThres = 0xcd;
	else
	...
	if(pDM_Odm->bLinked)
	{
		if(pDM_Odm->RSSI_Min > 25) {
	        if ( pDM_Odm->ExtLNA )
			     ...
			     CurCCK_CCAThres = 0x87; 	 
		} else if((pDM_Odm->RSSI_Min <= 25) && (pDM_Odm->RSSI_Min > 10))
			CurCCK_CCAThres = 0x83;
		else
		{
            if(FalseAlmCnt->Cnt_Cck_fail > 1000) 
                 CurCCK_CCAThres = pDM_Odm->cck_cca_hi;
            else 
                 CurCCK_CCAThres = pDM_Odm->cck_cca_low;	
		}
	}
	else
	{
	...
	)
{
	...
	if(pDM_DigTable->CurCCK_CCAThres!=CurCCK_CCAThres)		//modify by Guo.Mingzhi 2012-01-03
	{
        ...
        if ( pDM_Odm->BoardType & ODM_BOARD_EXT_LNA )
        {
            if ( CurCCK_CCAThres == pDM_Odm->cck_cca_low )
                ODM_Write1Byte(pDM_Odm, 0xa0e, pDM_Odm->lna_rx_hi);
            else	
                ODM_Write1Byte(pDM_Odm, 0xa0e, pDM_Odm->lna_rx_low);
        }
	}
