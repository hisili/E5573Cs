	...
	)
{
	...
	BOOLEAN			path_a_on,path_b_on,path_c_on,path_d_on;

	path_a_on	= (pDM_Odm->path_select & BIT0)? 1:0;
	path_b_on	= (pDM_Odm->path_select & BIT1)? 1:0;
	path_c_on	= (pDM_Odm->path_select & BIT2)? 1:0;
	path_d_on	= (pDM_Odm->path_select & BIT3)? 1:0;	

	//phydm_set_TX_path(path);
}
...
)
{
	...
	{	
		...
		H2C_Parameter[1] =  pDM_PathDiv->ant_candidate_1; 
		H2C_Parameter[2] =  pDM_PathDiv->ant_candidate_2; 
		H2C_Parameter[3] =  pDM_PathDiv->ant_candidate_3; 
	...
	)
{
	...
	if(pPktinfo->bPacketToSelf || pPktinfo->bPacketMatchBSSID)
	{
		if(pPktinfo->DataRate > ODM_RATE11M)
		{
			...
			{
				#if RTL8814A_SUPPORT
				//pDM_PathDiv->PathC_Sum[pPktinfo->StationID]+=pPhyInfo->RxMIMOSignalStrength[2];
				//pDM_PathDiv->PathC_Cnt[pPktinfo->StationID]++;

				//pDM_PathDiv->PathD_Sum[pPktinfo->StationID]+=pPhyInfo->RxMIMOSignalStrength[3];
				//pDM_PathDiv->PathD_Cnt[pPktinfo->StationID]++;
