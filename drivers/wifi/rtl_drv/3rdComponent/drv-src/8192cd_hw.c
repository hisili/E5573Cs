...
#include "data_TxPowerTrack_AP_88E.c"
VAR_MAPPING(TxPowerTrack_AP_88E, TxPowerTrack_AP_88E);
				  ...
				  unsigned int BitMask, unsigned int Data)
{
    ...
    {
        static unsigned int s_RegAddr = 0;
        static unsigned int s_Data = 0;
        if ((RegAddr != s_RegAddr) || Data != s_Data)
        {
            s_RegAddr = RegAddr;
            s_Data = Data;
            DEBUG_INFO("rf: reg=0x%08X val=0x%08X\n", RegAddr, Data);
        }
    }
#if defined(CONFIG_RTL_8812_SUPPORT)
	if (GET_CHIP_VER(priv) == VERSION_8812E) {
...
#if defined(CONFIG_MEM_USE_CACHE)
       mem_ptr = (unsigned char *)rtl_cache_get(RTL_CACHE_CFGFILE, cache_pool_idx, MAX_CONFIG_FILE_SIZE);
       if (NULL == mem_ptr)
...
#if defined(CONFIG_MEM_USE_CACHE)
        if (!rtl_cache_check(RTL_CACHE_CFGFILE, mem_ptr))
#endif /* CONFIG_MEM_USE_CACHE */
		kfree(mem_ptr);
		return -1;
	}
...
#endif
#ifdef CONFIG_RTL_88E_SUPPORT
static char TXPWR_TRACKING_NAME_88E[][16] =
{
    "delta_N",
    "delta_P",
    "therm_N",
...
void input_tracking_value_88E(struct rtl8192cd_priv* priv, int offset, int num, int value)
{
    ...
    switch (offset)
    {
        ...
        case DELTASWING_N:
            offset = (offset % 2);
            priv->pshare->deltaSwingTableIdx_88E[offset][num] = value;
            break;
        case THERMAL_P:
        case THERMAL_N:
            offset = (offset % 2);
            priv->pshare->thermalThreshold_88E[offset][num] = value;
            break;
        default:
            break;
    }
...
static int get_tracking_table_88E(struct rtl8192cd_priv* priv, unsigned char* line_head)
{
    unsigned char* next;
    int base, idx;
    int num = 0;
    ...
    for (offset = 0; offset < TXPWR_TRACKING_NAME_NUM_88E; offset++)
    {
        if (!memcmp(line_head, TXPWR_TRACKING_NAME_88E[offset], strlen(TXPWR_TRACKING_NAME_88E[offset])))
        { break; }
    }
    printk(" offset %d  \n", offset);
    if (offset >= TXPWR_TRACKING_NAME_NUM_88E)
    { return offset; }
    ...
    while (1)
    {
        ...
        if (next)
        {
            ...
            printk("num#%d = %d \n", num, _convert_2_pwr_tracking((char*)&next[idx], base));
            input_tracking_value_88E(priv, offset, num, _convert_2_pwr_tracking((char*)&next[idx], base));
...
void check_tracking_table_88E(struct rtl8192cd_priv* priv)
{
    int tmp = 0;
    int tmp2 = 0;
    ...
    for (tmp = 0; tmp < 2; tmp++)
    {
        for (tmp2 = 1; tmp2 < index_mapping_NUM_88E; tmp2 ++)
        {
            if (priv->pshare->deltaSwingTableIdx_88E[tmp][tmp2] < priv->pshare->deltaSwingTableIdx_88E[tmp][tmp2 - 1])
            { priv->pshare->deltaSwingTableIdx_88E[tmp][tmp2] = priv->pshare->deltaSwingTableIdx_88E[tmp][tmp2 - 1]; }
        }
    }
    ...
    for (tmp = 0; tmp < 2; tmp++)
    {
        for (tmp2 = 1; tmp2 < index_mapping_NUM_88E; tmp2 ++)
        {
            if (priv->pshare->thermalThreshold_88E[tmp][tmp2] < priv->pshare->thermalThreshold_88E[tmp][tmp2 - 1])
            { priv->pshare->thermalThreshold_88E[tmp][tmp2] = priv->pshare->thermalThreshold_88E[tmp][tmp2 - 1]; }
        }
    }
}

int PHY_ConfigTXPwrTrackingWithParaFile_88E(struct rtl8192cd_priv* priv)
{
    int read_bytes, num;
    unsigned char* mem_ptr, *line_head, *next_head;

    memset(priv->pshare->deltaSwingTableIdx_88E, 0, (2 * index_mapping_NUM_88E));
    memset(priv->pshare->thermalThreshold_88E, 0, (2 * index_mapping_NUM_88E));
    ...
    read_bytes = (u4Byte)(data_TxPowerTrack_AP_88E_end - data_TxPowerTrack_AP_88E_start);
    *((pu4Byte)((pu1Byte)&next_head)) = (u4Byte)data_TxPowerTrack_AP_88E_start;
    ...
    while (1)
    {
        ...
        if (line_head[0] == '/')
        { continue; }
        printk(" get_tracking_table_88E will in \n");
        num = get_tracking_table_88E(priv, line_head);

    }
    printk(" check_tracking_table_88E will in \n");
    check_tracking_table_88E(priv);

    kfree(mem_ptr);
    printk(" PHY_ConfigTXPwrTrackingWithParaFile_88E will out \n");
    return 0;
}
#endif
...
//_TXPWR_REDEFINE in MP Tool, 3 Groups: 36-99 100-148 149-165
    ...
    #if defined(CONFIG_MEM_USE_CACHE)
        mem_ptr_tmp = (unsigned char *)rtl_cache_get(RTL_CACHE_CFGFILE, 0, MAX_CONFIG_FILE_SIZE);
        if (NULL == mem_ptr_tmp)
    #endif /* CONFIG_MEM_USE_CACHE */
    ...
    #if defined(CONFIG_MEM_USE_CACHE)
        if (!rtl_cache_check(RTL_CACHE_CFGFILE, mem_ptr_tmp))
    #endif /* CONFIG_MEM_USE_CACHE */
		kfree(mem_ptr_tmp);
    ...
    #if defined(CONFIG_MEM_USE_CACHE)
        mem_ptr = (unsigned char *)rtl_cache_get(RTL_CACHE_CFGFILE, 0, MAX_CONFIG_FILE_SIZE);
        if (NULL == mem_ptr)
    #endif /* CONFIG_MEM_USE_CACHE */
		if((mem_ptr = (unsigned char *)kmalloc(MAX_CONFIG_FILE_SIZE, GFP_ATOMIC)) == NULL) {
			printk("PHY_ConfigBBWithParaFile(): not enough memory\n");
...
#if defined(CONFIG_MEM_USE_CACHE)
        mem_ptr = (unsigned char *)rtl_cache_get(RTL_CACHE_CFGFILE, 1, MAX_CONFIG_FILE_SIZE);
        if (NULL == mem_ptr)
#endif /* CONFIG_MEM_USE_CACHE */
	if ((mem_ptr = (unsigned char *)kmalloc(MAX_CONFIG_FILE_SIZE, GFP_ATOMIC)) == NULL) {
...
int PHY_ConfigMACWithParaFile(struct rtl8192cd_priv *priv)
{
	...
	{
#if defined(CONFIG_MEM_USE_CACHE)
            mem_ptr = (unsigned char *)rtl_cache_get(RTL_CACHE_CFGFILE, 2, MAX_CONFIG_FILE_SIZE);
            if (NULL == mem_ptr)
#endif /* CONFIG_MEM_USE_CACHE */
		if ((mem_ptr = (unsigned char *)kmalloc(MAX_CONFIG_FILE_SIZE, GFP_ATOMIC)) == NULL) {
			printk("PHY_ConfigMACWithParaFile(): not enough memory\n");
...
//			ODM_RF_CALIBRATION		|
...
#if defined(HIGH_POWER_EXT_PA) && defined(HIGH_POWER_EXT_LNA)
    if (priv->pshare->rf_ft_var.use_ext_pa && priv->pshare->rf_ft_var.use_ext_lna) { // For 8814A 
	...
	{		
		ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_GPA, 0x55);
		ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_APA, 0x55);
		ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_GLNA, 0x55);
		ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_ALNA, 0x55);	
	}
	else
	{
		ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_GPA, 0x00);
...
#endif
    ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_EXT_LNA_RSSI_OFFSET, 
        priv->pshare->rf_ft_var.rssi_offset);
    ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_EXT_LNA_OFDM_RSSI_OFFSET, 
        priv->pshare->rf_ft_var.ofdm_rssi_offset);
#ifdef HIGH_POWER_EXT_LNA
		if(priv->pshare->rf_ft_var.use_ext_lna){
			// 9. u1Byte ExtLNA
			if ((GET_CHIP_VER(priv) == VERSION_8192C) || (GET_CHIP_VER(priv) == VERSION_8188C)||
                ...
                ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_EXT_LNA_RX_PWR_0, 
                    priv->pshare->rf_ft_var.ofdm_rssi_offset);
                ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_EXT_LNA_RX_PWR_7, 
                    priv->pshare->rf_ft_var.rx_pwr_7);
                ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_EXT_LNA_RX_PWR_6, 
                    priv->pshare->rf_ft_var.rx_pwr_6);
                ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_EXT_LNA_RX_PWR_5, 
                    priv->pshare->rf_ft_var.rx_pwr_5);
                ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_EXT_LNA_RX_PWR_4, 
                    priv->pshare->rf_ft_var.rx_pwr_4);
                ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_EXT_LNA_RX_PWR_3, 
                    priv->pshare->rf_ft_var.rx_pwr_3);					
                ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_EXT_LNA_RX_PWR_2, 
                    priv->pshare->rf_ft_var.rx_pwr_2);
                ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_EXT_LNA_RX_PWR_1, 
                    priv->pshare->rf_ft_var.rx_pwr_1);
                ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_EXT_LNA_RX_PWR_0, 
                    priv->pshare->rf_ft_var.rx_pwr_0);
                ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_EXT_LNA_CCK_CCA_HI, 
                    priv->pshare->rf_ft_var.cck_cca_hi);
                ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_EXT_LNA_CCK_CCA_LOW, 
                    priv->pshare->rf_ft_var.cck_cca_low);
                ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_EXT_LNA_RX_GAIN_HI, 
                    priv->pshare->rf_ft_var.lna_rx_hi);
                ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_EXT_LNA_RX_GAIN_LOW, 
                    priv->pshare->rf_ft_var.lna_rx_low);
			}else{
				ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_EXT_LNA, TRUE);
				ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_5G_EXT_LNA, TRUE);
			}
...
void set_1cca_enable(struct rtl8192cd_priv *priv, int enable_1cca)
{
    if (NULL == priv)
    {
        return;
    }
    if(IS_TESLF_LOCK_ENABLE())  /*进行天线锁口测试时，直接返回*/
    {
        return;
    }
    if (enable_1cca == priv->pshare->rf_ft_var.enable_1cca)
    {
        return;
    }
    ...
    if (-1 != enable_1cca)
    {
        ...
        priv->pshare->rf_ft_var.enable_1cca = enable_1cca;
    }
    printk("switch 1cca enable to %d\n", priv->pshare->rf_ft_var.enable_1cca);

    if (priv->pshare->rf_ft_var.enable_1cca)
    {
...
*/
	...
	if (GET_CHIP_VER(priv) == VERSION_8188E) { 
        ...
        {
            /* set [11:22] all bits 1*/
            RTL_W32(AFE_XTAL_CTRL, RTL_R32(AFE_XTAL_CTRL) | 0x7FF800);
        }
...
//	RTL_W8(IMR+4, 0x3f);
	...
	if (priv->pshare->rf_ft_var.pwr_track_file) {
        ...
        if (GET_CHIP_VER(priv) == VERSION_8188E) 
        {
           printk(" PHY_ConfigTXPwrTrackingWithParaFile_88E will in \n");
           PHY_ConfigTXPwrTrackingWithParaFile_88E(priv);
        } 
#endif
...
/*END PN:   DTS2014121803084, Added by zwx235250, 20150108*/
#ifdef CONFIG_1RCCA_RF_POWER_SAVING
    set_1cca_enable(priv, -1);
#endif /*CONFIG_1RCCA_RF_POWER_SAVING*/
...
/*CamResetAllEntry                                     */
/*******************************************************/
#if defined(MULTI_MAC_CLONE) && defined(CONFIG_WLAN_HAL_8192EE)
...
void set_offload_wakeup_pin(struct rtl8192cd_priv *priv, int is_high)
{
#ifndef CONFIG_WLAN_HAL_8192EE
    #define WAKEUP_CTL_GPIO_NUM (7)
#endif
    ...
    printk("set wakeup pin %s LEVE\n", (is_high ? "HIGH" : "LOW"));
    
    if (is_high)
    {
        /* pull high */
    #if defined(CONFIG_WLAN_HAL_8192EE)
        RTL_W8(0x4e, (RTL_R8(0x4e) | (BIT(6))));
        RTL_W16(0x64, (RTL_R16(0x64) | (BIT(13))));
    #else
        RTL_W32(GPIO_PIN_CTRL, (RTL_R32(GPIO_PIN_CTRL) & ~BIT(WAKEUP_CTL_GPIO_NUM + 8)));
    #endif
    }
    else
    {
...
int offloadTestFunction(struct rtl8192cd_priv *priv, unsigned char *data)
{
    ...
    if (mode == 0x1)
    {
        printk("offload_function_ctrl = %d\n", priv->pshare->offload_function_ctrl);
        if (0 == priv->pshare->offload_function_ctrl)
        {
    ...
    if (mode == 0x6)
    {
        printk("manual wakeup offload \n");
        irqreturn_t rtl_wakeup_irq_handle(int irq, void *dev_instance);
        rtl_wakeup_irq_handle(0, priv);
    }
#endif
    ...
    if (mode == 0x7)
    {
        set_offload_wakeup_pin(priv, 1);
    }

    if (mode == 0x8)
    {
        set_offload_wakeup_pin(priv, 0);
    }
#endif /*WLAN_PLATFORM_HUAWEI_COMMON*/
...
int MIMO_mode_switch(struct rtl8192cd_priv *priv, int mode)
{
    ...
    if ((rf_mimo_mode != MIMO_2T2R) && (rf_mimo_mode != MIMO_1T2R) && (!IS_TESLF_LOCK_ENABLE()))   /*天线锁口测试时不返回*/
		return 0;
#ifdef MP_TEST
	if (((OPMODE & WIFI_MP_STATE) || priv->pshare->rf_ft_var.mp_specific))
		return 0;
#endif	
	if(IS_TESLF_LOCK_ENABLE())
	{
		priv->pmib->dot11RFEntry.tx2path=0;
		PHY_SetBBReg(priv, 0x90c, 0x80000000 , 0x1);

		if ( mode == 5 ) {
		    printk("cliW: set path A: %x\n", rFPGA0_TxInfo);

		    PHY_SetBBReg(priv, rFPGA0_TxInfo, 0x0000000f, 0x1);
		    PHY_SetBBReg(priv, 0x80c, 0x0000000f, 0x1);
		...
		if ( mode == 6 ) {
		    printk("cliW: set path B\n");

	        PHY_SetBBReg(priv, rFPGA0_TxInfo, 0x0000000f, 0x2);
		    PHY_SetBBReg(priv, 0x80c, 0x0000000f, 0x2);

	        PHY_SetBBReg(priv, 0xc04, 0x000000ff , 0x22);
	        PHY_SetBBReg(priv, 0xd04, 0x0000000f , 0x2);
	        PHY_SetBBReg(priv, 0x90c, 0x000000ff , 0x22);
	        PHY_SetBBReg(priv, 0x90c, 0x0ff00000 , 0x22);
	        PHY_SetBBReg(priv, 0xd00, 0x00180000, 0);

		    RTL_W8(0xa07, 0x45);//0x40);

		}

		priv->pshare->rf_ft_var.rf_mode = MIMO_1T1R;
	...
	if ((mode == MIMO_1T1R) && (priv->pshare->rf_status!= MIMO_1T1R)) 	{
		...
		if (GET_CHIP_VER(priv) == VERSION_8192E) {
		    ...
		    if ( pathB && IS_TESLF_LOCK_ENABLE()) {
				printk("===== enter the pathB===\n");
			    PHY_SetBBReg(priv, 0xc04, 0x000000ff , 0x22);
                PHY_SetBBReg(priv, 0xd04, 0x0000000f , 0x2);
                PHY_SetBBReg(priv, 0x90c, 0x000000ff , 0x22);
                PHY_SetBBReg(priv, 0x90c, 0x0ff00000 , 0x22);
...
#ifdef CONFIG_RTL_88E_SUPPORT
	if (GET_CHIP_VER(priv) == VERSION_8188E) {
        if (hwinfo[EEPROM_XTAL_88E] > 0 && hwinfo[EEPROM_XTAL_88E] < 0x3f)
        {
            priv->pmib->dot11RFEntry.xcap = (hwinfo[EEPROM_XTAL_88E]);	//[7:0]
        }
        else
        {
            priv->pmib->dot11RFEntry.xcap = 0x20;
        }
