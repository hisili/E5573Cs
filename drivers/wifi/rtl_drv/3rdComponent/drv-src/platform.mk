...
# _PC_ , _BRLADLIGHT_ , _PUMA6_ , _MNDSPEED_ , _POWERPC_,  ARM_PLATFORM_HUAWE

#default
ifeq ($(CONFIG_PLATFORM),)
    CONFIG_PLATFORM := ARM_PLATFORM_HUAWE
endif
...
ifeq ($(CONFIG_PLATFORM),ARM_PLATFORM_HUAWE)
   EXTRA_CFLAGS += -D_LITTLE_ENDIAN_ -DSMP_SYNC
   ARCH := arm
   KSRC ?= ${ANDROID_KERN_DIR}

   #wifi feature macro
   EXTRA_CFLAGS += $(CFG_WIFI_EXTRA_CFLAGS)
   #huawei platform MBB features macro
   ifneq ($(CFG_MBB_FEATURE_MACRO_FILE),)
        include $(CFG_MBB_FEATURE_MACRO_FILE)
        ifneq ($(CFG_MBB_FEATURE_MACRO_NAME),)
            EXTRA_CFLAGS += $($(CFG_MBB_FEATURE_MACRO_NAME))
        endif
   endif
