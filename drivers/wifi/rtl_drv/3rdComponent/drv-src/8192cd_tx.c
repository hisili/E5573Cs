...
#else
__MIPS16 __IRAM_IN_865X unsigned int get_skb_priority(struct rtl8192cd_priv *priv, struct sk_buff *skb)
#endif
{
				...
				struct net_device *dev, struct net_device *wdsDev, struct tx_insn *txcfg)
{
    ...
    if (0 == txcfg->frg_num) 
    {
        ...
        mem_dump("[SlowPath]txcfg", (unsigned char*)txcfg, sizeof(struct tx_insn));
    }
#endif
...
#ifdef WLAN_PLATFORM_HUAWEI_COMMON
    if(wlan_check_arp_spoofing(priv->dev, skb)) 
    {
        dev_kfree_skb_any(skb);
        RESTORE_INT(x);
        SMP_UNLOCK_XMIT(x);
        _DEBUG_ERR("TX DROP: Detected ARP spoofing!\n");
        return ret;
    }
#endif //WLAN_PLATFORM_HUAWEI_COMMON

#ifndef CFG_XMIT_SKB_QUEUE
    ret = __rtl8192cd_start_xmit(skb, skb->dev, TX_NORMAL);
#else
    ret = 0;
    if (skb_queue_len(&priv->pshare->xmit_skb_queue) <= 1024)
    {
        __skb_queue_tail(&priv->pshare->xmit_skb_queue, skb);
    } 
    else 
    {
        dev_kfree_skb_any(skb);
...
//	int priority;
...
#ifdef CONFIG_RTK_MESH
                pri = get_skb_priority3(priv, skb, txcfg->is_11s);
#else
                pri = get_skb_priority(priv, skb);
#endif
    ...
    {
        pktpri = get_skb_priority(priv, skb);
...
#ifdef CONFIG_PCI_HCI
#ifdef CONFIG_RTK_MESH
		(get_skb_priority3(priv, skb, txcfg->is_11s) == ptxsc_entry->pktpri) &&
#else		
		(get_skb_priority(priv, skb) == ptxsc_entry->pktpri) &&
#endif
#endif
		(FRAGTHRSLD > 1500))
	{
