...
#ifdef RTL_CHANEL_COPY_FORM_BCM
...
//	{364,"IR ", 3, 3}, /*IRAN*/
	...
	//{410,"KR ", 3, 3}, /*NORTH KOREA*/   º«¹úÔõÃ´Ìí¼Ó
	//{408,"KP ", 3, 3}, /*KOREA REPUBLIC*/
	{399,"KR ", 3, 3}, /*SOUTH KOREA*/
	{414,"KW ", 3, 3}, /*KUWAIT*/
	{415,"LA ", 3, 3}, /*LAOS*/
	{422,"LB ", 3, 3}, /*LEBANON*/
	{428,"LV ", 3, 3}, /*LATVIA*/
	...
	{634,"QA ", 3, 3}, /*QATAR*/
    /*BEGIN DTS2015120807375 qwx283034 2015-12-15 modified*/
    {642,"RO ", 3, 3}, /*ROMANIA*/
    /*END DTS2015120807375 qwx283034 2015-12-15 modified*/
	...
	{858,"UY ", 3, 3}, /*URUGUAY*/
	{860,"UZ ", 3, 3}, /*UZBEKISTAN*/
	{862,"VE ", 3, 3}, /*VENEZUELA*/
	{704,"VN ", 3, 3}, /*VIET NAM*/
	{887,"YE ", 3, 3}, /*YEMEN*/
	{716,"ZW ", 3, 3}, /*ZIMBABWE*/
};
#else /*realtek default*/
static const COUNTRY_IE_ELEMENT countryIEArray[] =
{
	/*
	 format: countryNumber | CountryCode(A2) 
...
#endif /* RTL_CHANEL_COPY_FORM_BCM */
...
int  map_country_to_reg(struct rtl8192cd_priv *priv)
{
    ...
    {
       if (0 == memcmp(CStringPtr, countryIEArray[i].countryA2, 2))
       {
    ...
    printk("domain:%d cn: %s found: %d\n"
            , priv->pmib->dot11StationConfigEntry.dot11RegDomain
            , priv->pmib->dot11dCountry.dot11CountryString
            , (i != country_num)
            );

    return ((i != country_num) ? 0 : (-1));
}

#define STRING_COUNTRY_CODE_LEN     (2) 
int get_country_channel(char *country, char *buf, int buf_len)
{
    int idx = 0, str_len = 0, begin = 0;
    char countryCode[STRING_COUNTRY_CODE_LEN + 1] = {0};
    struct channel_list *cur_list = NULL;

    if (NULL == buf || buf_len <= 0)
    {
        return str_len;
    }
    if ((NULL == country) || (STRING_COUNTRY_CODE_LEN != strlen(country)))
    {
        str_len += snprintf(&(buf[str_len]), (buf_len - str_len), "country code must be %d chars\n", STRING_COUNTRY_CODE_LEN);
        return str_len;
    }
    
 
    memcpy(countryCode, (void *)country, STRING_COUNTRY_CODE_LEN);
    for (idx = 0; idx < STRING_COUNTRY_CODE_LEN; idx++)
    {
        if (countryCode[idx] >= 'a' && countryCode[idx] <= 'z')
        {
            countryCode[idx] = (countryCode[idx] - 'a' + 'A');
        }
    }
    

    for (idx = 0; idx < (sizeof(countryIEArray) / sizeof(countryIEArray[0])); idx++)
    {
        if (0 == memcmp(countryCode, countryIEArray[idx].countryA2, STRING_COUNTRY_CODE_LEN))
        {
            break;
        }
    }
    
    if (idx == (sizeof(countryIEArray) / sizeof(countryIEArray[0])))
    {
        str_len += snprintf(&(buf[str_len]), (buf_len - str_len), " [%s] not support\n", countryCode);
    }
    else
    {
        cur_list = get_channel_list(0, countryIEArray[idx].G_Band_Region);
        if (NULL == cur_list)
        {
            cur_list = get_channel_list(0, DOMAIN_FCC);
        }
        str_len += snprintf(&(buf[str_len]), (buf_len - str_len), " [%c%c] code: %d, region: %d, channel: "
                ,countryIEArray[idx].countryA2[0], countryIEArray[idx].countryA2[1]
                , countryIEArray[idx].countryNumber, countryIEArray[idx].G_Band_Region);
        if (NULL != cur_list)
        {
            begin = 0;
            for (idx = begin + 1; idx < cur_list->len; idx++)
            {
                if (cur_list->channel[idx] == (cur_list->channel[idx - 1] + 1))
                {
                    continue;
                }
                str_len += snprintf(&(buf[str_len]), (buf_len - str_len), "%d-%d,"
                    , cur_list->channel[begin], cur_list->channel[idx]);
                begin = idx;
            }
            str_len += snprintf(&(buf[str_len]), (buf_len - str_len), "%d-%d,"
                    , cur_list->channel[begin], cur_list->channel[idx - 1]);
        }
        buf[str_len - 1] = '\n';
    }
