...
#endif /*!defined(WLAN_PLATFORM_HUAWEI_COMMON)*/
...
#endif			
{
	...
	PRINT_SINGL_ARG("    keep_alive_time", priv->pmib->dot11OperationEntry.keep_alive_time, "%d");
	return pos;
}
...
//	if ((GET_CHIP_VER(priv)==VERSION_8192D) && (priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_5G)) {
    ...
    PRINT_ONE("    force_1t2r: ", "%s", 0);
    PRINT_ONE(priv->pshare->rf_ft_var.force_1t2r, "%d", 0);
    PRINT_ONE(priv->pshare->rf_ft_var.force_1t2r_already, " (state:%d)", 1);
#endif
...
#endif
{
	...
	PRINT_ONE("    edca_turbo: ", "%s", 0);
	PRINT_ONE(priv->pshare->rf_ft_var.edca_turbo, "%d", 1);
#ifdef CONFIG_1RCCA_RF_POWER_SAVING
	PRINT_ONE("    1rcca_ps_active: ", "%s", 0);
	PRINT_ONE(priv->pshare->rf_ft_var.enable_1cca, "%d", 1);
#endif

    PRINT_ONE("    mp_tx_head/tail: ", "%s", 0);
    PRINT_ONE(priv->pshare->skb_head, "%d", 0);
    PRINT_ONE(priv->pshare->skb_tail, "/%d", 1);
...
#endif			
{
	...
	PRINT_SINGL_ARG("    wsc_ie_auto: ", priv->pmib->wscEntry.wsc_ie_auto, "%d");
...
/* Hotspot 2.0 Release 1 */
    ...
    PRINT_SINGL_ARG("    no_rts: ", pstat->no_rts, "%d");
    CHECK_LEN_B;
	PRINT_ONE("", "%s", 1);
