...
#define int16 short
    ...
    fastip_t  __attribute__ ((aligned(32))) *wifi_fastip_handle[RTL8192CD_NUM_VWLAN + 2]  = {NULL}; /* 最大支持接口数 */

    #define fastip_get_netif_index(priv) (IS_VXD_INTERFACE(priv) ? 0 : (priv->vap_id + 2)) /* 获取网络设备索引 */
    void fastip_attach_wifi(struct rtl8192cd_priv *priv)
    {
        ...
        dev = priv->dev;
        ifidx = fastip_get_netif_index(priv);
        printk("wifi fastip netif index = %d, devname %s\n", ifidx, dev->name);
        if(NULL != wifi_fastip_handle[ifidx]) 
        {
            printk("wifi fastip handle have been used!\n");
            return;
        }
        wifi_fastip_handle[ifidx] = _fastip_attach(dev->name, &msglevel, fastip_detach_wifi, &ifidx);
        if (wifi_fastip_handle[ifidx])
        {
            printk("wifi_fastip_handle attach ok !!!!!! cih = 0x%x \n", wifi_fastip_handle[ifidx]);
        }
        if (fastip_dev_register(wifi_fastip_handle[ifidx], dev, FALSE, TRUE) != FASTIP_OK) 
        {
            printk("wifi wifi_dev_register() failed\n");
        }
        else
        {
            printk("wifi wifi register ok\n");
        }
    }
    ...
    void fastip_detach_wifi(struct rtl8192cd_priv *priv)
    {
        ...
        if (NULL != wifi_fastip_handle[ifidx])
        {
            printk("wifi_fastip_handle %s unregiseter:%d handle:%x\n", dev->name
                , ifidx, wifi_fastip_handle[ifidx]);
            fastip_dev_unregister(wifi_fastip_handle[ifidx], dev);
            wifi_fastip_handle[ifidx] = NULL;
        }
        return;
    }
    
    int fastip_forward_wifi(struct rtl8192cd_priv *priv, struct sk_buff *skb)
    {
        int ifidx = 0;
        
        if (NULL == priv || NULL == skb)
        {
            return FASTIP_ERROR;
        }
        ...
        if (!FASTIP_ENAB(wifi_fastip_handle[ifidx]))
        {
            return (FASTIP_ERROR);
        }
    
        /* try fastip first */
        if (fastip_forward(wifi_fastip_handle[ifidx], skb, skb->dev) != FASTIP_ERROR)
        {
            return (FASTIP_OK);
        }
...
#if defined(WLAN_PLATFORM_BALONG_V3)
{
    ...
    sd = &per_cpu(softnet_data, get_cpu());
    len = skb_queue_len(&sd->input_pkt_queue);
    put_cpu();
    if (len > CFG_FC_RX_SKB_QLEN_SIZE) 
    {
        si_meminfo(&si);
        printk("cliW: qu len:%d t_m:%d f_m:%d\n",  len, _CK(si.totalram), _CK(si.freeram) );
        return 1;
    }
}
#endif
...
/* Hotspot 2.0 Release 1 */
    ...
    if (fastip_forward_wifi(priv, pskb) != FASTIP_ERROR)            
    {
        return;
    }
    skb_pull_inline(pskb, ETH_HLEN);
#endif
	#if defined(_BROADLIGHT_FASTPATH_)
		send_packet_to_upper_layer(pskb);
	#elif defined(__LINUX_2_6__) && defined(RX_TASKLET) && !defined(CONFIG_RTL8672)&& !defined(NOT_RTK_BSP)&& !defined(__LINUX_3_10__)
...
//			if (get_free_memory() < FREE_MEM_LOWER_BOUND) {
        ...
        #ifdef WLAN_PLATFORM_HUAWEI_COMMON
            if (wlan_check_arp_spoofing(priv->dev, pskb))
            {
                priv->ext_stats.rx_data_drops++;
                DEBUG_ERR("RX DROP: MCAST Detected ARP spoofing!\n");
                return FAIL;
            }
        #endif /* WLAN_PLATFORM_HUAWEI_COMMON */
...
mcast_netif_rx:
		...
		else
		{
            ...
            {
                priv->ext_stats.rx_data_drops++;
                DEBUG_ERR("RX DROP: UNICAST Detected ARP spoofing!\n");
                return FAIL;
            }
        #endif /* WLAN_PLATFORM_HUAWEI_COMMON */
			dst_pstat = get_stainfo(priv, da);
