...
//在此添加E5573CS各子项目硬件ID，\include\drv\drv_version.h
    #define HW_VER_PRODUCT_E5573CS_609    (0X00000804)
    #define HW_VER_PRODUCT_E5573CS_322    (0X00000900)
    #define HW_VER_PRODUCT_E5573CS_931    (0X00000901)
    #define HW_VER_PRODUCT_E5673S_609     (0X00000902)
    #define HW_VER_PRODUCT_E5573CS_609_B    (0X00000905)
    #define HW_VER_PRODUCT_E5573CS_322_B    (0X00000906)
    #define HW_VER_PRODUCT_E5573CS_931_B    (0X00000907)
    #define HW_VER_PRODUCT_E5673S_609_B     (0X00000908)
    #define HW_VER_PRODUCT_E5573CS_323_B    (0X00000807)
    #define HW_VER_PRODUCT_E5573BS_322      (0X00000409)
    #define HW_VER_PRODUCT_E5573CS_933      (0X00000109)
    #define HW_VER_PRODUCT_E5573CS_509      (0X00000009)
...
#define SIOCEFUSE_SYNC 		0x8b9c
    ...
    #define SIOC_COUNTRY_CHANNEL        0x8B89
    #define SIOC_FIRMWARE_INFO          0x8B99
    #define SIOC_REFRESH_ACL_TABLE      0x8B90
    #define SIOCREAD_STA                0x8B9f
#endif
...
#endif
#ifdef WLAN_PLATFORM_HUAWEI_COMMON
    { SIOC_COUNTRY_CHANNEL, IW_PRIV_TYPE_CHAR | 40, IW_PRIV_TYPE_CHAR | 128, "channel" },
    { SIOC_FIRMWARE_INFO, IW_PRIV_TYPE_CHAR | 40, IW_PRIV_TYPE_CHAR | 128, "fw_info" },
    { SIOC_REFRESH_ACL_TABLE, IW_PRIV_TYPE_CHAR | 40, 0, "refresh_acl" },
    { SIOCREAD_STA, IW_PRIV_TYPE_CHAR | 128, IW_PRIV_TYPE_CHAR | 1024, "sta_info" },
#endif
...
#endif //#ifdef CONFIG_OFFLOAD_FUNCTION
#ifdef CONFIG_AUTOCH_TIMER
    { SIOCAUTOCHTIMER, IW_PRIV_TYPE_CHAR | 128, 0, "autoch_to" },
#endif /* CONFIG_AUTOCH_TIMER */
...
static struct iwpriv_arg mib_table[] = {
...
#ifdef WLAN_PLATFORM_HUAWEI_COMMON
    {"ch_mask",       INT_T,      _OFFSET(dot11RFEntry.dot11ch_mask), _SIZE(dot11RFEntry.dot11ch_mask), (unsigned int)(-1)},
    {"ant_lock",       INT_T,      _OFFSET(dot11RFEntry.tserlf_lock_enable), _SIZE(dot11RFEntry.tserlf_lock_enable), (unsigned int)(0)},
	...
	{"keep_alive_time",    INT_T,		_OFFSET(dot11OperationEntry.keep_alive_time), _SIZE(dot11OperationEntry.keep_alive_time), 0},
#ifdef __ECOS
...
#ifdef WLAN_PLATFORM_HUAWEI_COMMON
    {"cost_ie_attr", INT_T,     _OFFSET(miscEntry.cost_ie_attr), _SIZE(miscEntry.cost_ie_attr), -1},
#endif
    ...
    {"max_xmitbuf_agg",    INT_T,      _OFFSET(miscEntry.max_xmitbuf_agg), _SIZE(miscEntry.max_xmitbuf_agg), 16},
#endif
#ifdef CONFIG_SDIO_HCI
	{"max_rx_agg_sz",         INT_T,      _OFFSET(miscEntry.max_rx_agg_sz),      _SIZE(miscEntry.max_rx_agg_sz), 0x0f},
	{"max_recvbuf_agg",       INT_T,      _OFFSET(miscEntry.max_recvbuf_agg),    _SIZE(miscEntry.max_recvbuf_agg), 1},
    {"max_handle_xmitbuf",    INT_T,      _OFFSET(miscEntry.max_handle_xmitbuf), _SIZE(miscEntry.max_handle_xmitbuf), 5},
    {"max_handle_recvbuf",    INT_T,      _OFFSET(miscEntry.max_handle_recvbuf), _SIZE(miscEntry.max_handle_recvbuf), 10},
    {"max_tx_agg",            INT_T,      _OFFSET(miscEntry.max_tx_agg),        _SIZE(miscEntry.max_tx_agg), 10},
#endif
	...
	{"pin",			PIN_IND_T, 0, 0},
#ifdef WLAN_PLATFORM_HUAWEI_COMMON
	{"wsc_ie_auto",	INT_T,	_OFFSET(wscEntry.wsc_ie_auto), _SIZE(wscEntry.wsc_ie_auto), 0},
#endif
	/* WPS2DOTX   */
...
/* Release 1 */
	...
	{"adaptivity_enable",		RFFT_T,	_OFFSET_RFFT(adaptivity_enable), _SIZE_RFFT(adaptivity_enable), 1},
#else
    {"adaptivity_enable",		RFFT_T,	_OFFSET_RFFT(adaptivity_enable), _SIZE_RFFT(adaptivity_enable), 1},
#endif
    ...
    {"tx_sched_qlen",   RFFT_T, _OFFSET_RFFT(tx_sched_qlen), _SIZE_RFFT(tx_sched_qlen), 4},
...
#endif
    {"rssi_offset",         RFFT_T, _OFFSET_RFFT(rssi_offset), _SIZE_RFFT(rssi_offset), 0},
    {"ofdm_rssi_offset",    RFFT_T, _OFFSET_RFFT(ofdm_rssi_offset), _SIZE_RFFT(ofdm_rssi_offset), 0},
#ifdef HIGH_POWER_EXT_LNA
    {"use_ext_lna",     RFFT_T, _OFFSET_RFFT(use_ext_lna), _SIZE_RFFT(use_ext_lna), 1},
    {"ext_lna_gain",    RFFT_T, _OFFSET_RFFT(ext_lna_gain), _SIZE_RFFT(ext_lna_gain), 16},
    {"rx_pwr_7",        RFFT_T, _OFFSET_RFFT(rx_pwr_7), _SIZE_RFFT(rx_pwr_7), -30},
    {"rx_pwr_6",        RFFT_T, _OFFSET_RFFT(rx_pwr_6), _SIZE_RFFT(rx_pwr_6), -48},
    {"rx_pwr_5",        RFFT_T, _OFFSET_RFFT(rx_pwr_5), _SIZE_RFFT(rx_pwr_5), -32},
    {"rx_pwr_4",        RFFT_T, _OFFSET_RFFT(rx_pwr_4), _SIZE_RFFT(rx_pwr_4), -26},
    {"rx_pwr_3",        RFFT_T, _OFFSET_RFFT(rx_pwr_3), _SIZE_RFFT(rx_pwr_3), -12},
    {"rx_pwr_2",        RFFT_T, _OFFSET_RFFT(rx_pwr_2), _SIZE_RFFT(rx_pwr_2), 5},
    {"rx_pwr_1",        RFFT_T, _OFFSET_RFFT(rx_pwr_1), _SIZE_RFFT(rx_pwr_1), 15},
    {"rx_pwr_0",        RFFT_T, _OFFSET_RFFT(rx_pwr_0), _SIZE_RFFT(rx_pwr_0), 10},
    {"cck_cca_hi",      RFFT_T, _OFFSET_RFFT(cck_cca_hi), _SIZE_RFFT(cck_cca_hi), 0xcd},
    {"cck_cca_low",     RFFT_T, _OFFSET_RFFT(cck_cca_low), _SIZE_RFFT(cck_cca_low), 0x40},
    {"lna_rx_hi",       RFFT_T, _OFFSET_RFFT(lna_rx_hi), _SIZE_RFFT(lna_rx_hi), 0x2f},
    {"lna_rx_low",      RFFT_T, _OFFSET_RFFT(lna_rx_low), _SIZE_RFFT(lna_rx_low), 0x1a},
#endif
...
//	{"antSw_dump",			RFFT_T, _OFFSET_RFFT(antSw_dump), _SIZE_RFFT(antSw_dump), 0},
    ...
    {"edca_turbo",       RFFT_T, _OFFSET_RFFT(edca_turbo), _SIZE_RFFT(edca_turbo), 0},

#ifdef CONFIG_1RCCA_RF_POWER_SAVING
    {"1cca_enable",       RFFT_T, _OFFSET_RFFT(enable_1cca), _SIZE_RFFT(enable_1cca), 1},
...
#endif
	{"low_retry_rssi_thd",		RFFT_T, _OFFSET_RFFT(low_retry_rssi_thd), _SIZE_RFFT(low_retry_rssi_thd), 20},
...
//	{"mp_dig_enable",	RFFT_T, _OFFSET_RFFT(mp_dig_enable), _OFFSET_RFFT(mp_dig_enable), 0},
    ...
    {"DCbackoff",	ODM_DM_1UT,	_OFFSET_ODM_DM(DCbackoff), _SIZE_ODM_DM(DCbackoff), 0},// for odm_Adaptivity()
...
#endif
#if defined (WLAN_SET_PRODUCE_MIB)
#define INDEX_MAPPING_EXTLNA_NUM 1
...
#define IS_SPLIT_CHAR(c) ((' ' == (c)) || ('\t' == (c)))

#include "data_Produce_MIB_Value.c"
VAR_MAPPING(Produce_MIB_Value, Produce_MIB_Value);
static int _convert_2_mib_value(char*s, int base)
{
    int k = 0;
    int value_flag=0;
...
static unsigned char* get_digit_mib(unsigned char** data)
{
...
static int set_mib_default(struct iwpriv_arg *p_mib, int mib_size, unsigned char* line_head)
{
    unsigned char* next;
    int value = 0, val_len = 0;
    int num = 0;
    int offset = 0;
    unsigned char* swim;

    for (offset = 0; offset < mib_size; offset++)
    {
        val_len = strlen(p_mib[offset].name);
        if ((!memcmp(line_head, p_mib[offset].name, val_len)) 
            && (IS_SPLIT_CHAR(line_head[val_len])) 
            )
        {
            if ( (BYTE_T == p_mib[offset].type) || (INT_T == p_mib[offset].type)
	         || (RFFT_T == p_mib[offset].type)
            #if defined(CONFIG_RTK_MESH)
                || (WORD_T == p_mib[offset].type)
            #endif /* CONFIG_RTK_MESH */
            #if defined(USE_OUT_SRC)
                || (ODM_DM_1UT == p_mib[offset].type) || (ODM_DM_1UT == p_mib[offset].type)
            #endif /* USE_OUT_SRC */
                )
            {
                // remove mib key
                swim = line_head + val_len;
		   break;
            }
        }
    }
    ...
    while (1)
    {
        ...
        if (next)
        {
            value = _convert_2_mib_value((char*)&next[0], 10); 
            if (value != p_mib[offset].Default)
            {
                printk("num= %d, mib_key = %s, value from def %d to %d \n"
                        , num, p_mib[offset].name, p_mib[offset].Default, value);
                p_mib[offset].Default = value;
            }
            num++;
            return true;
        }
        ...
        if (num >= INDEX_MAPPING_EXTLNA_NUM)
        { break; }

        next = get_digit_mib(&next);
...
int init_produce_mib(struct iwpriv_arg *p_mib, int mib_size)
{
    int read_bytes;
    unsigned char* mem_ptr, *line_head, *next_head;
    int arg_num = sizeof(mib_table)/sizeof(struct iwpriv_arg);
    read_bytes = (u4Byte)(data_Produce_MIB_Value_end - data_Produce_MIB_Value_start);
    char mem_buf[read_bytes+1];
    memset(mem_buf, 0, read_bytes+1);
    *((pu4Byte)((pu1Byte)&next_head)) = (u4Byte)data_Produce_MIB_Value_start;

    memcpy(mem_buf, next_head, read_bytes);

    next_head = mem_buf;
   // printk(" while (1) will in \n");
    while (1)
    {
        line_head = next_head;
        ...
        if (line_head[0] == '/')
        { continue; }
        (void)set_mib_default(p_mib, arg_num,line_head);
    }
    return 0;
}
#endif /* WLAN_SET_PRODUCE_MIB */
#if (FEATURE_ON == MBB_WIFI_CHIP_REGULAR)
 struct rtl_mib_type{
    char name[32];  /* mib name */
    int value;  /* mib  value */
...
enum mib_name_index{
    rssi_offset_index = 0,
    ofdm_rssi_offset_index,
    adaptivity_enable_index,
    trswitch_index,
#ifdef HIGH_POWER_EXT_LNA
    use_ext_lna_index,
    ext_lna_gain_index,
...
static struct rtl_mib_type g_mib_table[]={
    {"rssi_offset",0},
    {"ofdm_rssi_offset",0},
    {"adaptivity_enable",2},
    {"trswitch",0},
...
void modify_mib_default_value_based_hw_id()
{
    uint32 product_hw_id = (uint32)wlan_get_product_wifi_hw_id();
    printk("== the product hw id =%d\n",product_hw_id);
    switch (product_hw_id) {
        case HW_VER_PRODUCT_E5573CS_609:
        case HW_VER_PRODUCT_E5573CS_609_B:
                g_mib_table[rssi_offset_index].value = 7;
                g_mib_table[ofdm_rssi_offset_index].value = 3;
            break;
        case HW_VER_PRODUCT_E5573CS_322:
        case HW_VER_PRODUCT_E5573CS_322_B:
            {
                g_mib_table[rssi_offset_index].value = 8;
                g_mib_table[ofdm_rssi_offset_index].value = 4;
                break;
            }
         case HW_VER_PRODUCT_E5573BS_322:
            {
                g_mib_table[rssi_offset_index].value = 8;
                g_mib_table[ofdm_rssi_offset_index].value = 4;
                break;
            }
          case HW_VER_PRODUCT_E5573CS_509:
            {
                g_mib_table[rssi_offset_index].value = 7;
                g_mib_table[ofdm_rssi_offset_index].value = 3;
                break;
            }
        case HW_VER_PRODUCT_E5573CS_933:
            {
                g_mib_table[rssi_offset_index].value = 8;
                g_mib_table[ofdm_rssi_offset_index].value = 4;
                break;
            }
        case HW_VER_PRODUCT_E5573CS_931:
        case HW_VER_PRODUCT_E5573CS_931_B:
        {
                g_mib_table[rssi_offset_index].value = 8;
                g_mib_table[ofdm_rssi_offset_index].value = 3;
                break;
            }
        case HW_VER_PRODUCT_E5673S_609:
        case HW_VER_PRODUCT_E5673S_609_B:
            {
                g_mib_table[rssi_offset_index].value = 8;
                g_mib_table[ofdm_rssi_offset_index].value = 3;
                break;
            }
        case HW_VER_PRODUCT_E5573CS_323_B:
            {
                g_mib_table[rssi_offset_index].value = 8;
                g_mib_table[ofdm_rssi_offset_index].value = 3;
                break;
...
void init_produce_mib_based_hw_id(struct iwpriv_arg *p_mib, int mib_size)
{
    int i = 0;
    int j =0;
    int mib_name_len = 0;
    modify_mib_default_value_based_hw_id();
    for(i = 0; i < max_mib_index; i++)
    {
        for (j = 0; j < mib_size; j++)
        {
            mib_name_len = strlen(p_mib[j].name);
            if(!memcmp(p_mib[j].name, g_mib_table[i].name, mib_name_len))
            {
                p_mib[j].Default = g_mib_table[i].value;
                break;
            }
        }
    }
...
#if defined (WLAN_SET_PRODUCE_MIB)
    {
        init_produce_mib(mib_table, arg_num);
    }
#endif
#if (FEATURE_ON == MBB_WIFI_CHIP_REGULAR)
    init_produce_mib_based_hw_id(mib_table, arg_num);
#endif
...
#ifdef WLAN_PLATFORM_HUAWEI_COMMON
static int acl_refresh_cmd(struct rtl8192cd_priv *priv)
{
    int idx = 0;
    unsigned long flags = 0;
    struct list_head *plist = NULL;
    struct wlan_acl_node *paclnode = NULL;
    
    if (NULL == priv || NULL == priv->pmib || NULL == priv->dev || !netif_running(priv->dev))
    {
        return 0;
    }

    SMP_LOCK_ACL(flags);
    plist = &priv->wlan_acl_list;
    while(!list_empty(plist))
    {
        paclnode = list_first_entry(plist, struct wlan_acl_node, list);
        if (NULL != paclnode)
        {
            list_del_init(&paclnode->list);
            list_add_tail(&paclnode->list, &priv->wlan_aclpolllist);
    ...
    for (idx = 0; idx < priv->pmib->dot11StationConfigEntry.dot11AclNum; idx++)
    {
        paclnode = list_first_entry(&priv->wlan_aclpolllist, struct wlan_acl_node, list);
        list_del_init(&paclnode->list);
        
        memcpy((void *)paclnode->addr, priv->pmib->dot11StationConfigEntry.dot11AclAddr[idx], 6); 
        paclnode->mode = (unsigned char)priv->pmib->dot11StationConfigEntry.dot11AclMode; 

        list_add_tail(&paclnode->list, &priv->wlan_acl_list);
    }
    
    SMP_UNLOCK_ACL(flags);
    return 0;
}
int read_sta_info(struct rtl8192cd_priv *priv, char *buf, int buf_size)
{
    int len = 0, num = 0;
    struct stat_info *pstat = NULL;
	...
	unsigned long flags = 0;
#endif
    if (buf_size <= 0 || NULL == priv || !netif_running(priv->dev))
    {
        return len;
    }
    len += snprintf((buf + len), (buf_size - len), "-- STA info table -- (active: %d)\n", priv->assoc_num);
    SMP_LOCK_ASOC_LIST(flags);
    phead = &priv->asoc_list;
    if (list_empty(phead))
    {
    ...
    plist = phead->next;
    while ((plist != phead) && (len <= (buf_size - 1)))
    {
        num++;
        pstat = list_entry(plist, struct stat_info, asoc_list);
        len += snprintf((buf + len), (buf_size - len), " %d: stat_info...\n", num);
        len += snprintf((buf + len), (buf_size - len), "    hwaddr: %02x%02x%02x%02x%02x%02x\n"
            , pstat->hwaddr[0], pstat->hwaddr[1], pstat->hwaddr[2], pstat->hwaddr[3], pstat->hwaddr[4], pstat->hwaddr[5]);/*MAC*/
        len += snprintf((buf + len), (buf_size - len), "    rssi: %d dbm\n", pstat->rssi-100);
        len += snprintf((buf + len), (buf_size - len), "    link_time: %d sec\n\n", pstat->link_time);
        plist = plist->next;
    }
    SMP_UNLOCK_ASOC_LIST(flags);
    return len;
...
#ifdef CONFIG_AUTOCH_TIMER
static int rtl8192cd_autochannel_timer(struct rtl8192cd_priv *priv, unsigned char *data)
{

    if (!IS_ROOT_INTERFACE(priv))
    {
        printk("Error, not root interface!\n");
        return -1;
    }

    priv->pshare->autoch_to = _atoi((char *)data, 10);

    printk("auto_timer: data = %s, autoch_to = %d\n", data, (int)priv->pshare->autoch_to);

    if (timer_pending(&priv->pshare->autoch_timer))
    {
        del_timer_sync(&priv->pshare->autoch_timer);
    }

    if (netif_running(priv->dev) && priv->pshare->autoch_to > 0)
    {
...
int rtl8192cd_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
...
#endif
    static unsigned char tmpbuf1[4096];
#ifdef RTK_WOW
    ...
    printk("recv ioctl iface = %.16s, cmd = %08X\n", wrq->ifr_ifrn.ifrn_name, cmd);
#ifdef SDIO_AP_PS
    if ((cmd == RTL8192CD_IOCTL_WRITE_REG)
        || (cmd == RTL8192CD_IOCTL_WRITE_RF_REG)
        )
    {
        SDIO_AP_WAKEUP(priv);
    }
...
#if defined(WLAN_PLATFORM_HUAWEI_COMMON)
    case SIOC_COUNTRY_CHANNEL:
    {
        memset(tmpbuf, 0, sizeof_tmpbuf);
        (void)ioctl_copy_from_user(tmpbuf, (void *)wrq->u.data.pointer, wrq->u.data.length);
        wrq->u.data.length = get_country_channel(tmpbuf, tmpbuf, sizeof_tmpbuf);
        (void)ioctl_copy_to_user((void *)wrq->u.data.pointer, tmpbuf, wrq->u.data.length);
        ret = 0;
        break;
    }
    case SIOC_FIRMWARE_INFO:
    {
        memset(tmpbuf, 0, sizeof_tmpbuf);
        wrq->u.data.length = snprintf(tmpbuf, sizeof_tmpbuf
            , "\n[FirmWare] - version %d.%d (%02x-%02x %02x:%02x)\n"
            , priv->pshare->fw_version, priv->pshare->fw_sub_version
            , priv->pshare->fw_date_month, priv->pshare->fw_date_day, priv->pshare->fw_date_hour, priv->pshare->fw_date_minute
            );
        wrq->u.data.length += snprintf((tmpbuf + wrq->u.data.length), (sizeof_tmpbuf - wrq->u.data.length)
            , "[%s] - version %d.%d.%d (%s)\n"
            , DRV_NAME, DRV_VERSION_H, DRV_VERSION_L, DRV_SUB_VERSION, DRV_RELDATE
            );
        (void)ioctl_copy_to_user((void *)wrq->u.data.pointer, tmpbuf, wrq->u.data.length);
        ret = 0;
        break;
    }
    case SIOC_REFRESH_ACL_TABLE:
    {
        ret = acl_refresh_cmd(priv);
        break;
    }
    case SIOCREAD_STA:
    {
        ret = read_sta_info(priv, tmpbuf, sizeof_tmpbuf);
        if (ret <= 0)
        {
            ret = -1;
        }
        else if (0 == ioctl_copy_to_user((void *)wrq->u.data.pointer, tmpbuf, ret))
        {
            wrq->u.data.length = ret;
            ret = 0;
        }
...
/*--------------P2P related ioctl----------------------------------end*/
	...
	case MP_START_TEST:
    #ifdef SDIO_AP_PS
        set_ap_ps_mode(priv, (unsigned char *)"1");
#endif
	...
	case MP_STOP_TEST:
    #ifdef SDIO_AP_PS
        set_ap_ps_mode(priv, (unsigned char *)"0");
#endif
		mp_stop_test(priv);
		ret = 0;
		break;
...
#ifdef CONFIG_AUTOCH_TIMER
    case SIOCAUTOCHTIMER:
    {
        if ((wrq->u.data.length > sizeof_tmpbuf) || copy_from_user(tmpbuf, (void *)wrq->u.data.pointer, wrq->u.data.length))
        {
            break;
        }
        ret = rtl8192cd_autochannel_timer(priv, tmpbuf);
        break;
    }
#endif /* CONFIG_AUTOCH_TIMER */
	case SIOCANTSTS:
