...
void rtl8192es_free_recv_priv(struct rtl8192cd_priv *priv)
{
	...
	if (precvpriv->recvbuf_mem) {
        ...
        if (!rtl_cache_check(RTL_CACHE_RECV, precvpriv->recvbuf_mem))
...
int rtl8192es_init_recv_priv(struct rtl8192cd_priv *priv)
{
    ...
    {
        printk("Error, pxmitbuf->pallocated_buf = %p is NOT NULL!\n", precvpriv->recvbuf_mem);
    }
#if defined(CONFIG_MEM_USE_CACHE)
    precvpriv->recvbuf_mem = (u8 *)rtl_cache_get(RTL_CACHE_RECV, 0, MAX_RECVBUF_MEM_SZ);
    if (NULL == precvpriv->recvbuf_mem)
#endif /* CONFIG_MEM_USE_CACHE */
...
#ifdef RECVBUF_DEBUG
	precvpriv->recvbuf_mem_end = precvpriv->recvbuf_mem + MAX_RECVBUF_MEM_SZ -1;
