...
#define DRV_SUB_VERSION 5
#define DRV_RELDATE		"2014-11-20"
...
#define EVENT_LOG
    ...
    #define CFG_FC_RX_SKB_QLEN_SIZE (128)
    #define CFG_FC_TX_FREE_QLEN_SIZE (100)
...
// Kconfig will conflict if the same CONFIG name is used for two times	
...
// FW Test Function
//-------------------------------------------------------------
#if defined(CONFIG_WLAN_HAL) || defined(CONFIG_RTL_88E_SUPPORT)
    #ifdef SDIO_AP_PS
...
#endif
#define TXPWR_TRACKING_NAME_NUM_88E 4
enum txpwr_tracking_offset_88E
{
    DELTASWING_N, 
    DELTASWING_P, 
    THERMAL_N, 
...
#define CU_Intval       200
    ...
    #ifdef WIFI_WPAS
        #define SS_AUTO_CHNL_TO RTL_MILISECONDS_TO_JIFFIES(100)
    #else
        #define SS_AUTO_CHNL_TO RTL_MILISECONDS_TO_JIFFIES(200)
    #endif
#define SS_AUTO_CHNL_NHM_TO		RTL_MILISECONDS_TO_JIFFIES(100)
#endif
#ifndef SS_AUTOCHANNEL_TO
    #define SS_AUTOCHANNEL_TO (5 * 60) /* ??¨¨??¡¥¨¬?¡Á??¡¥D?¦Ì¨¤?????????a5¡¤??¨® */
#endif
...
#ifdef CONFIG_WLAN_HAL_8192EE
    #ifndef WLAN_PLATFORM_HUAWEI_FACTORY
#define CONFIG_1RCCA_RF_POWER_SAVING
#endif
#endif /*CONFIG_WLAN_HAL_8192EE*/

#define CONFIG_MEM_USE_CACHE
// SDIO TX
#define CONFIG_XMITBUF_TXAGG
#ifndef CONFIG_RTL_88E_SUPPORT
...
#endif
#if defined(CONFIG_XMITBUF_TXAGG_ADV) && !defined(CONFIG_XMITBUF_TXAGG)
#error "CONFIG_XMITBUF_TXAGG_ADV must define CONFIG_XMITBUF_TXAGG mode"
#endif
...
#define SDIO_AP_WAKEUP(priv)
#if 1
    #include "wlan_if.h"
    static void rtl_printk(int flag, const char *func, int line, const char *fmt, ...)
    {
        va_list args = {0};

        if (WLAN_LOG_LEVE < flag)
        {
            return;
        }
        ...
        {
            pr_err("***realtek*** [%s:%d]: ", (func ? func : ""), line);
            va_start(args, fmt);
            vprintk(fmt, args);
            va_end(args);
        }
    ...
    #define printk(fmt, ...) do { \
        rtl_printk(WLAN_LOG_DRV_LOW, __FUNCTION__, __LINE__, fmt, ##__VA_ARGS__); \
        if (WLAN_LOG_LEVE < WLAN_LOG_DRV_LOW) \
        { \
            break; \
        } \
        if (WLAN_LOG_FILE & WLAN_LOG_SIGN) \
        { \
            wlan_log_fprintf("***realtek*** [%s:%d]: "fmt, __FUNCTION__, __LINE__, ##__VA_ARGS__); \
        } \
    }while(0)
    #ifdef SDIO_AP_PS
        #undef SDIO_AP_WAKEUP
        #define SDIO_AP_WAKEUP(priv) do { \           
            if (IS_DRV_OPEN(GET_ROOT(priv)) && (NULL != priv->pshare) \
                && (TRUE != priv->pshare->bSurpriseRemoved) && (TRUE != priv->pshare->bDriverStopped) \
                && (0 != priv->pshare->offload_function_ctrl) && (RTW_STS_WAKEUP != priv->pshare->pwr_state)) \
            { \
                printk("pwr_state = %d, offload_ctl = %d, set wifi to wakeup ap\n" \
                    , priv->pshare->pwr_state, priv->pshare->offload_function_ctrl); \
                wlan_set_driver_lock(true); \
                priv->pshare->pwr_state = RTW_STS_WAKEUP; \
                mod_timer(&priv->pshare->ps_timer, jiffies); \
            } \
       }while(0)
    #endif /*SDIO_AP_PS*/
    #define ASSERT_PRIV_RUNNING(priv)   do { \
        if (NULL == priv) \
        { \
           printk("Error, priv is NULL\n"); \
           return (-EINVAL); \
        } \
        if (!IS_DRV_OPEN(priv) || !netif_running(priv->dev) || (NULL == priv->pshare)) \
        { \
            printk("Error, netif (%s) running = %d, pshare = %p\n" \
                , priv->dev->name, netif_running(priv->dev), priv->pshare); \
            return (-EINVAL); \
        } \
        if ((TRUE == priv->pshare->bSurpriseRemoved) || (TRUE == priv->pshare->bDriverStopped)) \
        { \
            printk("Error, netif (%s), bSurpriseRemoved = %d, bDriverStopped = %d\n" \
                , priv->dev->name, priv->pshare->bSurpriseRemoved, priv->pshare->bDriverStopped); \
            return (-EINVAL); \
        } \
    }while(0)
