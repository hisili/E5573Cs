...
while [ "$num" -lt 5 ]; do
    ...
    if [ "$(ls /sys/bus/platform/drivers/msm_sdcc/msm_sdcc.2/mmc_host/ | grep mmc)" != "" ]; then
...
echo "[wi-fi]: mmc device:$(ls /sys/bus/platform/drivers/msm_sdcc/msm_sdcc.2/mmc_host/ | grep mmc)"
