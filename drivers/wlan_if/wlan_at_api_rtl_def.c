...
#define WLAN_AT_MODE_SUPPORT    "2,3,4"   /* 支持的模式(cw:0/a:1/b:2/g:3/n:4/ac:5) */
#define WLAN_AT_BAND_SUPPORT    "0,1"       /* 支持的带宽(20M:0/40M:1/80M:2/160M:3) */
#if defined(FEATURE_HUAWEI_MBB_RTL8189)
    #define WLAN_AT_TSELRF_SUPPORT  "0"           /* 支持1*1单天线 */
#elif defined(FEATURE_HUAWEI_MBB_RTL8192)
    #define WLAN_AT_TSELRF_SUPPORT  "0,1"         /* 支持2*2双天线 */
#endif

#define MAX_POW_INDEX    63           /* 最大功率index值 */
...
/*****************************************************************************
 函数名称  : STATIC void WlanRegInforToDig(const char *nptr, const int byte_count, unsigned long *value)
 功能描述  : 寄存器查询字符串转换为数字类型
 输入参数  : nptr:字符串指针，byte_count:字符串长度，value：字符转换后的数字类型
 输出参数  : NA
 返 回 值  : NA
*****************************************************************************/
STATIC void WlanRegInforToDig(const char *nptr, const int byte_count, unsigned long *value)
{
    ...
    {
        byte_value = 0;
        byte_value = (unsigned long)wlan_strtoi(nptr, NULL, WLAN_BASE10); /*获取10进制数*/
...
/*****************************************************************************
 函数名称  : STATIC int WlanReadRegInfor(const char *cmd, char *value)
 功能描述  : 获取命令执行的结果字符串
 输入参数  : cmd：需要执行的命令，value：字符命令的长度
 输出参数  : NA
 返 回 值  : 成功返回value的长度，失败返回0
*****************************************************************************/
                    ...
                    , WLAN_AT_WIBAND_TYPE band, uint32 channel, WLAN_AT_TSELRF_TYPE wtselrf)
{
    ...
    {
        p_buf += (strlen(info_key));  /* 根据信道查询power值 */
    }
    else
    {
    ...
    *reg_val = (uint32)wlan_strtoi(p_buf, NULL, WLAN_BASE16); /*获取16进制数*/
    ...
    if (efuse_key == g_wical_pwr_param_set[(int)wtselrf].diff_th20_ofdm)
    {
        ...
        {
            *reg_val = (uint32)(0x0F & *reg_val); /* 仅低4位表示11g的power增量 */
        }
        else if (AT_WIBAND_20M == band)
        {
            *reg_val = (uint32)((0xF0 & *reg_val) >> 4); /* 高4位表示20M的power增量 */
        }
    }
#endif
   ...
   if(MAX_POW_INDEX >= ht40_index)
   {
        switch (g_wlan_at_data.WiMode.mode)
        {
            case AT_WIMODE_80211b:
            {
                ...
                {
                    pow_index = curr_index + (lWifiPower - (s_target_power.dot11b / 10)) * 2; /* 参见MP文档 */
                }
                break;
            }
            case AT_WIMODE_80211g:
...
STATIC int32 ATSetWifiTX(WLAN_AT_FEATURE_TYPE onoff)
{
    ...
    case WLAN_AT_TX_FULL_DUTY_RATIO:
    {
...
#ifdef FEATURE_HUAWEI_MBB_RTL8192
        /*通过调用WlanReadRegInfor保存c14,c1c,a07寄存器*/
        ...
        tmp_reg_value = g_b_a07_value | 0xF; //0xF:低四位置1
...
STATIC int32 ATGetWifiInfo(WLAN_AT_WIINFO_STRU *params)
{
    ...
    if (!IS_TSELRF_VAILD(params->member.group)) /* 双天线，支持索引到1 */
    {
        return (AT_RETURN_FAILURE);
    }
    ...
    case AT_WICALDATA_ABS_IDX_POW:
    {
        switch (params->mode)
        {
            ...
            case AT_WIMODE_80211g:
            {
                ...
                {
                    g_wlan_at_data.WiCal.power[params->group].diffofdm = (uint8)(0x0F & (data+1)); /* 仅取低4位 */
				}
                else
                {
                    g_wlan_at_data.WiCal.power[params->group].diffofdm = (uint8)(0x0F & data); /* 仅取低4位 */
                }
                break;
            }
            case AT_WIMODE_80211n:
                ...
                switch(params->bandwidth)
                {
                    case AT_WIBAND_20M:
                    {
                        ...
                        g_wlan_at_data.WiCal.power[params->group].diffofdm = (
                            (g_wlan_at_data.WiCal.power[params->group].diffofdm & 0x0F) /* 高4位清零 */
                            | ((0x0F & data) << 4)); /* 20M的数据存放于OFDM的高4位 */
                    #else
                        g_wlan_at_data.WiCal.power[params->group].diffht20 = (uint8)(0x0F & data); /* 仅取低4位 */
                    #endif
                        break;
                    }
                    case AT_WIBAND_40M:
...
/*****************************************************************************
函数名称  : wlan_at_init_rtl_def
功能描述  : rtl8192芯片装备接口初始化函数
输入参数  : NA
输出参数  : NA
返 回 值  : int
*****************************************************************************/
