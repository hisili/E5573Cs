...
{
    static int s_reg_buk3_flags = 0;
    if (en)
    {
        s_reg_buk3_flags |= (int)mod;
    }
    else
    {
        s_reg_buk3_flags &= (~(int)mod);
    ...
    regu_pr_debug("[%s:%d] buk3_flags = %p, BUCK3_ONOFF_FLAG = %p, buk3 from %d to %d\n"
        , ((NULL != func) ? func : ""), line, (void *)s_reg_buk3_flags
        , &BUCK3_ONOFF_FLAG, BUCK3_ONOFF_FLAG, (s_reg_buk3_flags != 0));

    (void)pr_err("----ebul----[%s:%d] buk3_flags = %p, BUCK3_ONOFF_FLAG = %p, buk3 from %d to %d\n"
        , ((NULL != func) ? func : ""), line, (void *)s_reg_buk3_flags
        , &BUCK3_ONOFF_FLAG, BUCK3_ONOFF_FLAG, (s_reg_buk3_flags != 0));

    /*根据状态位裁决是否允许BUK3下电*/
    BUCK3_ONOFF_FLAG = (0 != s_reg_buk3_flags);
}
#endif
...
{
#if (FEATURE_ON == MBB_COMMON)
    BSP_BUK3_ENABLE(BUK3_MOD_USB, 1);
#else
    BUCK3_ONOFF_FLAG = 1;
#endif
...
#if (FEATURE_ON == MBB_COMMON)
    BSP_BUK3_ENABLE(BUK3_MOD_USB, 0);
#else
    BUCK3_ONOFF_FLAG = 0;
#endif
